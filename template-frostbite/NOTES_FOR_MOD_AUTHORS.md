# Notes for Mod Authors - template-frostbite (template)

Packaging rules for template-frostbite (template) mods, so Vortex installs them to the right place.
...
| Frosty Mods | a `.fbmod` file | `FrostyModManager\Mods\XXX` |
| XXX Plugin | a file with the `.exe` extension and a file with one of these extensions: `.dll` | `FrostyModManager\Plugins` |
| XXX Key | - | - |
...
Installs to: `FrostyModManager\Mods\XXX`
