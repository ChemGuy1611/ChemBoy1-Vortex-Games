...
| `prereleaseTag`      | opt-in                | `'<tag>'` -> fetch one fixed release directly via `/releases/tags/<tag>` (rolling tag, e.g. UE4SS `experimental-latest`). Pick the tag that upstream _moves_, not one that merely sounds current — see the warning below.                                                                                                                                                     |
...
- **semver orders `X.Y.Z-N` below a bare `X.Y.Z`.** Ordering within a series is correct (`5.4.23-6` > `5.4.23-5`, `3.1.0-6` > `3.1.0-5`), which is all these upstreams produce. An upstream that ships a bare version _after_ a numbered one needs `trackByAssetDate` instead — that is the escape hatch for sources that do not order by semver.
- **`semver.coerce()` rejects components with leading zeros** (`2026.02.01` -> `null`), so such versions reach the caller's `0.0.0` floor. A source that versions this way needs a game-specific transform applied _before_ this ladder, not after — `game-dragonagetheveilguard`'s `normalizeFrostyVersion` (`2026.02.01.0` -> `26.2.1`) is the reference case.
...
The same field name exists in seven of the eight downloader modules. Only the way each one _reaches_ the pinned release differs, because their hosts do:
...
| `fcmodding_downloader.js`    | _not supported_                  | — the host keeps only the current build and one prior, so any pin 404s within a release or two (`FCMODDING_API.md`) |
...
- Installed identity differs from the pin — **including not installed at all** — and the module resolves the _pinned_ release, never the latest one.
- The notification says "pinned version available", not "update available". The wording has to cover a user who is _ahead_ of the pin as well as behind it; installing the pin from that state is a deliberate downgrade, and the dialog says so.
- The pin overrides every "which release" strategy the module owns: `allowPrerelease` and `prereleaseTag` here, newest-file selection in the companions. Each combination logs a warning. Selection _within_ the pinned release is untouched — `fileArchivePattern` / `filePattern` / `fileType` still pick the right asset out of it.
...
- `autoInstall` stays orthogonal: the pin says _which_ version, `autoInstall` says _whether_ anything installs unattended.
...
- `nexusCategoryId` (default `1`, MAIN) selects the category, and accepts an array. **Do not assume the current build is in MAIN.** HFW Mod Manager's page has no MAIN file at all: its current release sits in MISCELLANEOUS while every superseded build is in OLD_VERSION or ARCHIVED. MAIN (1), OPTIONAL (3) and MISCELLANEOUS (5) are the only categories a file can be _uploaded_ as, so `[1, 3, 5]` means "any current build" and still excludes UPDATE (2), OLD_VERSION (4) and ARCHIVED (7) — the states a file falls into once it is superseded, and precisely what a requirement must never install. Check the page's listing before choosing.
...
- **Failures open the page.** Any error on this route notifies _and_ opens the mod page's files tab, so a user whose account cannot auto-download still lands where the file is.
...
- **GitHub or Nexus.** A requirement comes from a GitHub release asset by default, or from a Nexus mod page when it sets `nexusModId` (see **Nexus-hosted requirements**). Requirement objects carrying the _old_ Nexus fields (`modId`/`fileFilter`/`modUrl`, from the webpack-bundle era) are still ignored — those are not the current field names. Every other host has its own companion module, listed at the top of this document.
...
- **Foreign Nexus ids are cleared at install.** Vortex md5-matches every finished download against Nexus, game-agnostically, so a shared third-party dependency that is byte-identical to a copy somebody re-uploaded for an unrelated game gets that upload's `modId`/`fileId` stamped onto the mod. Vortex's update check then resolves `<managed game>/mods/<foreign modId>` — an unrelated mod — and offers it as an update for the requirement. The install therefore deletes `attributes.modId` and `attributes.fileId`, using the **singular** `setModAttribute` with an `undefined` value (the mods reducer deletes on that; the plural `setModAttributes` merges and cannot delete). `checkModVersion` bails on a non-numeric `modId`, which makes the deletion a reliable kill switch on stock Vortex. Do not remove these as redundant. Background: `VORTEX_MOD_METADATA.md`, upstream issue Nexus-Mods/Vortex#21979.
- **The download declares its origin before the install pipeline reads it.** `installDownload` dispatches `setDownloadModInfo(dlId, 'source', 'website')` _before_ emitting `start-install-download`. Placement is the whole point: `InstallManager` re-reads the download from live state immediately before running the attribute extractors, and `processAttributes` gates its entire Nexus fetch on `modInfo.source === 'nexus'` — so a value written here is the one it sees, and writing it after the install is too late to prevent anything. It also permanently exempts the download from the startup "downloads missing meta" sweep, which only re-queries entries whose `modInfo.source` is undefined. This does race `queryInfo`, which `finalizeDownload` fires without awaiting: on stock Vortex the md5 lookup usually lands afterwards and stamps `'nexus'` back over it. It holds outright on an already-downloaded archive (that lookup ran long ago), and on a Vortex carrying the #21979 fix it is authoritative — that fix returns early when a download already declares a non-Nexus source, which is exactly this signal. The seven companion modules do the same thing at the same point.
- **The Nexus-page route is exempt from both guards.** A requirement with `nexusModId` passes `nexusSourced: true`, which skips the source declaration _and_ the id clear. That route is the one place where an md5 match against Nexus is correct — the file really did come from that page — so the ids are genuine and worth keeping, and leaving the download Nexus-sourced lets the install pick up the page's real author, picture and description. Nothing runs away with those ids: the mod is still stamped `source: 'website'`, and the bulk update check filters on `source === 'nexus'`, so this module remains the only thing updating it.
- **Source attribution + version.** A successful install sets `source: 'website'` and `url` to the requirement's human page — the repo page derived from `githubUrl` (`https://api.github.com/repos/{owner}/{repo}` -> `https://github.com/{owner}/{repo}`), or the mod page for a Nexus requirement — which Vortex renders as a clickable "Source" link in the mod details panel. It also records the `version` attribute: the archive-derived version on the already-downloaded shortcut path (no extra GitHub request), or `latestAssetVersion(requirement, asset)` on a fresh download (the same value used in the update-check dialog). When the shortcut path cannot resolve a version (no resolver, or the `''`/`'0.0.0'` sentinel for a versionless archive), the attribute is left unset rather than stamped with a bogus floor — the next forced update stamps the real release version. The `source` stamp is unconditional while `url` is not: a file this module fetched came from a GitHub release or a Nexus mod _page_, never a Nexus mod _file_, and must not be left looking like one even when there is no page to link to. The same id is written to the download entry's `modInfo.source`, which shares the mod-source namespace — `'website'` is a registered id, `'other'` is not (the id behind the "Other" label is `unsupported`, and an unregistered id leaves the Source column blank).
