...
const selectUDF = async (api) => {
  //user select folder
  const launcherSettings = path.join(
  ...
  await vfs.ensureDirWritableAsync(path.join(directory, "Mods"));
  ...
  await fsp.writeFile(launcherSettings, launcherData, { encoding: "utf8" });
...
function installPk4(api, files) {
  ...
  return api
    .showDialog(
      "question",
      "Choose Install for .pk4 Files",
      {
        text:
          "The mod you are installing contains .pk4 files." +
          `Select if these .pk4 files should be installed to the "SP" (SinglePlayer) folder or "MP" (MultiPlayer) folder.`,
      },
      [{ label: CANC_BUT }, { label: SP_BUT }, { label: MP_BUT }],
    )
    .then((result) => {
          ...
          async (error, dlid) => {
            ...
            try {
              ...
              files = files
                .filter(
                  (file) =>
                    path.basename(file).includes(MODMANAGER_STRING) &&
...
async function browseForDownloadFunction(
  ...
  return api.showErrorNotification(
    `${MOD_NAME} is already installed. Please remove the exisitng mod if you want to try again.`,
    undefined,
    { allowReport: false },
  );
} //*/
...
async function deserializeLoadOrder(context) {
  ...
  try {
    ...
    modFiles = modFiles.filter((file) =>
      PLUGINS_EXTS_FILTER.includes(path.extname(file).toLowerCase()),
    );
...
async function validate(context, prev, current) {
  ...
  for (const entry of current) {
    try {
      await fsp.stat(path.join(dataPath, entry.id));
    } catch {
...
async function deserializeLoadOrder(context) {
  ...
  //Set load order
  let loadOrder = await LO_LINE.split(",").reduce(async (accumP, entry) => {
    const accum = await accumP;
    const file = entry;
    if (!modFiles.includes(file)) {
      return Promise.resolve(accum);
...
function runModManager(api, toolId, toolName) {
  ...
  try {
    ...
    if (TOOL_PATH !== undefined) {
      return api
        .runExecutable(TOOL_PATH, [], { suggestDeploy: false })
        .catch((err) =>
...
async function chooseModLoader(api, gameSpec) {
  ...
  return api
    .showDialog(
      ...
      {
        bbcode: t(
          "You must choose between BepInEx and MelonLoader to install mods.{{bl}}" +
      ...
      [{ label: t("BepInEx") }, { label: t("MelonLoader") }],
    )
    .then(async (result) => {
      if (result === undefined) {
        return;
...
async function deconflictModLoaders(api, gameSpec) {
  ...
  return api
    .showDialog(
      ...
      {
        bbcode: t(
          "You have both BepInEx and MelonLoader installed.{{bl}}" +
...
async function psarcExtract(GAME_PATH, api) {
  ...
  try {
    ...
    //if the folders aren't there, the user probably clossed the terminal windows
    return false;
  }
}
...
async function psarcSetup(api) {
  ...
  if (EXTRACTED) {
    ...
    try {
      ...
      fsp.rename(
        path.join(GAME_PATH, PSARCTOOL_PATH, SPCOMPSARC_FILE),
        path.join(GAME_PATH, PSARCTOOL_PATH, BAK_SPCOMPSARC_FILE),
      );
    ...
    try {
      ...
      fsp.rename(
        path.join(GAME_PATH, PSARCTOOL_PATH, BINPSARC_FILE),
        path.join(GAME_PATH, PSARCTOOL_PATH, BAK_BINPSARC_FILE),
      );
...
async function psarcCleanup(api) {
  ...
  CLEANUP_FOLDERS.forEach((folder, idx, arr) => {
    try {
      ...
      fsp.rm(path.join(FOLDERS_PATH, folder), { recursive: true });
  ...
  try {
    ...
    try {
      ...
      fs.statSync(path.join(GAME_PATH, PSARCTOOL_PATH, SPCOMPSARC_FILE));
      vfs.unlinkAsync(path.join(GAME_PATH, PSARCTOOL_PATH, BAK_SPCOMPSARC_FILE));
    } catch {
  ...
  try {
    ...
    try {
      //make sure vanilla file is not in place - this usually means the game was updated
      fs.statSync(path.join(GAME_PATH, PSARCTOOL_PATH, BINPSARC_FILE));
      vfs.unlinkAsync(path.join(GAME_PATH, PSARCTOOL_PATH, BAK_BINPSARC_FILE));
    } catch {
      await fsp.rename(
        path.join(GAME_PATH, PSARCTOOL_PATH, BAK_BINPSARC_FILE),
        path.join(GAME_PATH, PSARCTOOL_PATH, BINPSARC_FILE),
...
async function resolveGameVersion(gamePath) {
  ...
  if (GAME_VERSION === "xbox") {
    ...
    // use DoomTheDarkAges.exe for Steam
    try {
      const exeVersion = require("exe-version");
...
// in context.once
context.api.onAsync("did-deploy", async (profileId, deployment) => {
  //update boot-options.json file on deployment
  const LAST_ACTIVE_PROFILE = selectors.lastActiveProfileForGame(context.api.getState(), GAME_ID);
  if (profileId !== LAST_ACTIVE_PROFILE) return;
...
async function updateJsonFiles(api) {
  ...
  try {
    ...
    const JSON_FOLDER_FILES = await fsp.readdir(path.join(GAME_PATH, JSON_PATH), {
      recursive: true,
    });
    ...
    api.showErrorNotification(
      `Could not update ${JSONFILES_FILE} file with texpack and lodpack file names. Please add entries manually.`,
      err,
      { allowReport: false },
    );
  }
