...
async function getLatestModWorkshopFile(requirement) {
  try {
    let file = null;
    if (requirement.fileType || requirement.filePattern) {
      //narrow multi-file submissions (e.g. .zip and .vmz variants) to this requirement's file
      ...
      if (requirement.fileType) {
        files = files.filter(
          (entry) =>
            String(entry.type || "").toLowerCase() === String(requirement.fileType).toLowerCase(),
        );
      }
      ...
      //the primary file is what the site's own download button serves
    ...
    if (file && !KNOWN_ARCHIVE_EXT.includes(String(file.type || "").toLowerCase())) {
      log(
        "warn",
        `${requirement.userFacingName}: ModWorkshop file ${fileName(file)} does not use a known archive extension - ` +
    ...
    log(
      "warn",
      `Could not get latest ${requirement.userFacingName} file from ModWorkshop API: ${err}`,
    );
    return null;
  }
}
...
async function getLatestModWorkshopVersion(requirement, file) {
  ...
  try {
    ...
    log(
      "warn",
      `Could not get latest ${requirement.userFacingName} version from ModWorkshop API: ${err}`,
    );
    return null;
  }
}
...
async function downloadModWorkshopRequirement(api, gameSpec, requirement, check = true) {
  ...
  try {
    ...
    const latestVersion = pinned
      ...
      : await getLatestModWorkshopVersion(requirement, latestFile);
    const dlInfo = {
    ...
    const batched = [
      ...
      actions.setModAttribute(gameSpec.game.id, modId, "modId", undefined),
      actions.setModAttribute(gameSpec.game.id, modId, "fileId", undefined),
    ];
...
async function checkForModWorkshopUpdateRequirement(api, gameSpec, requirement) {
  ...
  const latestArchive = fileName(latestFile)
    .toLowerCase()
