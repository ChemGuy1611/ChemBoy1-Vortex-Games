...
C:\Game_Tools\0 GitHub Repos\
  ...
  Vortex-Backend\           <- extensions-manifest.json lives here
...
| `VORTEX_MANIFEST_PATH` | `C:\Game_Tools\0 GitHub Repos\Vortex-Backend\out\extensions-manifest.json` | `patch_extensions.py`, `nexus_games_report.py`; also the default in `vortex_utils.load_vortex_manifest()` |
...
## 6. Vortex-Backend Repo

`extensions-manifest.json` at `out/extensions-manifest.json` is the canonical source for which Nexus mod ID corresponds to each Vortex extension. It is used by `patch_extensions.py` (extension URL patching) and `nexus_games_report.py` (supported-status column).

The file is generated by the Vortex-Backend build. Keep the repo up to date (`git pull`) before running scripts that consume it.
...
python -c "import os; p=r'C:\Game_Tools\0 GitHub Repos\Vortex-Backend\out\extensions-manifest.json'; print('manifest OK' if os.path.exists(p) else 'MISSING: ' + p)"

# Check env vars
python -c "import os; [print(k, '=', 'SET' if os.environ.get(k) else 'MISSING') for k in ['NEXUS_API_KEY','STEAM_API_KEY','STEAM_USER_ID','STEAMGRIDDB_API_KEY','VORTEX_MANIFEST_PATH']]"
```
