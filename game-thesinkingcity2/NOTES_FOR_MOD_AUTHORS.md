# Notes for Mod Authors - The Sinking City 2

Packaging rules for The Sinking City 2 mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `PlayGround` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `PlayGround\Content\Paks\LogicMods` |
| Pak Mods | a `.pak` file | `PlayGround\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `PlayGround\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `PlayGround\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `PlayGround\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `PlayGround`, `Engine` or `Content` | the game folder itself (no subfolder) |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | - |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `PlayGround\Binaries\Win64` |
...
Installs to: `PlayGround\Content\Paks\LogicMods`
...
Installs to: `PlayGround\Content\Paks\~mods`
...
Installs to: `PlayGround\Binaries\Win64`
...
Installs to: `PlayGround\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `PlayGround`, `Engine`, `Content`, `Binaries`, `Mods`, `Paks` or `Movies`.
