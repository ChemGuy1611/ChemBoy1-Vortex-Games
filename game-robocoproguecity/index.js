...
const CONFIG_PATH_UNFINISHED = path.join(
  LOCALAPPDATA,
  "RoboCopUnfinishedBusiness",
  "Saved",
  "Config",
  "Windows",
);
const SAVE_PATH_UNFINISHED = path.join(
  LOCALAPPDATA,
  "RoboCopUnfinishedBusiness",
  "Saved",
  "SaveGames",
...
const spec = {
  ...
  discovery: {
    ids: [STEAMAPP_ID, EPICAPP_ID, GOGAPP_ID, XBOXAPP_ID],
    names: [],
  },
};
...
function getExecutable(discoveryPath) {
  ...
  if (isCorrectExec(EXEC_XBOX)) {
    ...
    SHIPPING_EXE = path.join(
      ...
      "Binaries",
      EXEC_FOLDER_XBOX,
      `${EPIC_CODE_NAME}-${EXEC_FOLDER_XBOX}-Shipping.exe`,
    );
  ...
  if (isCorrectExec(EXEC_DEFAULT)) {
    ...
    SHIPPING_EXE = path.join(
      ...
      "Binaries",
      EXEC_FOLDER_DEFAULT,
      `${EPIC_CODE_NAME}-${EXEC_FOLDER_DEFAULT}-Shipping.exe`,
    );
    ...
    actions: [
      {
        ...
        action: (dismiss) => {
          api.showDialog(
            ...
            {
              text:
                `You tried installing a Config file mod, but the game, staging folder, and ${CONFIG_LOC_UNFINISHED} folder are not all on the same drive.\n` +
    ...
    actions: [
      {
        ...
        action: (dismiss) => {
          api.showDialog(
            ...
            {
              text:
                `You tried installing a Save file mod, but the game, staging folder, and ${SAVE_LOC_UNFINISHED} folder are not all on the same drive.\n` +
...
function UNREALEXTENSION_UNFINISHED(context) {
  ...
  context.registerInstaller(
    UE5_SORTABLE_ID_UNFINISHED,
    29,
    testForUnrealMod,
    (files, __destinationPath, gameId) => installUnrealModUnfinished(context.api, files, gameId),
  );
    ...
    actions: [
      {
        ...
        action: (dismiss) => {
          api.showDialog(
            ...
            {
              text:
                `Because ${name} includes the IO-Store Unreal Engine feature, Vortex must use hardlinks to install mods for the game.\n` +
...
async function setupUnfinished(discovery, api, gameSpec) {
  ...
  CHECK_DATA_UNFINISHED = checkPartitions(
    LOCALAPPDATA,
    GAME_PATH_UNFINISHED,
    STAGING_FOLDER_UNFINISHED,
  );
  if (!CHECK_DATA_UNFINISHED) {
    partitionCheckNotify(
      api,
      CHECK_DATA_UNFINISHED,
      GAME_ID_UNFINISHED,
      GAME_NAME_UNFINISHED,
      CONFIG_LOC_UNFINISHED,
      CONFIG_PATH_UNFINISHED,
      SAVE_PATH_UNFINISHED,
    );
  }
  // ASYCRONOUS CODE ///////////////////////////////////
  if (CHECK_DATA_UNFINISHED) {
    //if game, staging folder, and config and save folders are on the same drive
    await vfs.ensureDirWritableAsync(CONFIG_PATH_UNFINISHED);
    await vfs.ensureDirWritableAsync(SAVE_PATH_UNFINISHED);
  } //*/
  //await downloadUe4ss(api, gameSpec, UE4SS_ID_UNFINISHED);
  await vfs.ensureDirWritableAsync(path.join(GAME_PATH_UNFINISHED, SCRIPTS_PATH_UNFINISHED));
  await vfs.ensureDirWritableAsync(path.join(GAME_PATH_UNFINISHED, LOGICMODS_PATH));
  return vfs.ensureDirWritableAsync(path.join(GAME_PATH_UNFINISHED, UE5_PATH));
}
...
function applyGame(context, gameSpec) {
  ...
  context.registerInstaller(
    ...
    (files) => installSave(context.api, files),
  );
...
function applyGameUnfinished(context, gameSpec) {
  ...
  //register mod installers
  context.registerInstaller(
    UE4SSCOMBO_ID_UNFINISHED,
    25,
    testUe4ssComboUnfinished,
    installUe4ssComboUnfinished,
  );
  context.registerInstaller(
    LOGICMODS_ID_UNFINISHED,
    27,
    testLogicUnfinished,
    installLogicUnfinished,
  );
  //29 is pak installer above
  context.registerInstaller(UE4SS_ID_UNFINISHED, 31, testUe4ssUnfinished, installUe4ssUnfinished); //
  context.registerInstaller(
    SCRIPTS_ID_UNFINISHED,
    33,
    testScriptsUnfinished,
    installScriptsUnfinished,
  ); //
  ...
  context.registerInstaller(
    CONTENT_ID_UNFINISHED,
    39,
    testContentUnfinished,
    installContentUnfinished,
  ); //
  context.registerInstaller(CONFIG_ID_UNFINISHED, 41, testConfigUnfinished, (files) =>
    installConfigUnfinished(context.api, files),
  ); //
  context.registerInstaller(
    SAVE_ID_UNFINISHED,
    43,
    (files, gameId) => testSaveUnfinished(context.api, files, gameId),
    (files) => installSaveUnfinished(context.api, files),
  ); //
  context.registerInstaller(
    BINARIES_ID_UNFINISHED,
    45,
    testBinariesUnfinished,
    installBinariesUnfinished,
  ); //
...
function main(context) {
  ...
  if (UNREALDATA.loadOrder === true) {
    ...
    context.registerLoadOrderPage({
      ...
      createInfoPanel: () =>
        context.api.translate(
          `Drag and drop the mods on the left to change the order in which they load. ${specUnfinished.game.name} loads mods in alphanumerical order, so Vortex prefixes ` +
            'the folder names with "AAA, AAB, AAC, ..." to ensure they load in the order you set here. ' +
            "The number in the left column represents the overwrite order. The changes from mods with higher numbers will take priority over other mods which make similar edits.",
        ),
    });
