# Notes for Mod Authors - VEIN

Packaging rules for VEIN mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `Vein` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `Vein\Content\Paks\LogicMods` |
| Pak Mods | a `.pak` file | `Vein\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `Vein\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `Vein\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `Vein\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `Vein`, `Engine` or `Content` | the game folder itself (no subfolder) |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | - |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `Vein\Binaries\Win64` |
...
Installs to: `Vein\Content\Paks\LogicMods`
...
Installs to: `Vein\Content\Paks\~mods`
...
Installs to: `Vein\Binaries\Win64`
...
Installs to: `Vein\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `Vein`, `Engine`, `Content`, `Binaries` or `Mods`.
