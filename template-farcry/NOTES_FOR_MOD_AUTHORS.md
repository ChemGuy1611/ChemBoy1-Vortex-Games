# Notes for Mod Authors - template-farcry (template)

Packaging rules for template-farcry (template) mods, so Vortex installs them to the right place.
...
| XXX Binaries | a file with the `.dll` extension | `bin` |
| XXX Mimoda3 | a file or folder named `info.xml` | `FCModInstaller\ModifiedFilesFCXXX` |
| Mod Installer Mods | a `.a2` file | `FCModInstaller\ModifiedFilesFCXXX` |
| XXX Xml | a file or folder named `gamerprofile.xml` and a file with the `.xml` extension | `DOCUMENTS\My Games\Far Cry XXX\USERID_FOLDER` |
...
Installs to: `FCModInstaller\ModifiedFilesFCXXX`
...
Installs to: `DOCUMENTS\My Games\Far Cry XXX\USERID_FOLDER`
