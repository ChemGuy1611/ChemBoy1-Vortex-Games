...
async function updateBootOptions(api) {
  ...
  try {
    //write to boot-options.json file tex and lod order
    //.texpack files
    ...
    const TEX_FILES = PATCH_FOLDER_FILES.filter(
      (file) => path.extname(file).toLowerCase() === PACK_EXT,
    );
    ...
    const TEX_FILE_NAMES = TEX_FILES.map((file) =>
      file.replace(path.extname(file), "").replace("\\", "/"),
    );
    const TEX_FILE_NAMES_CONV = TEX_FILE_NAMES.map((file) => `../../patch/${file}`); //*/
    BOOT_OPTIONS_JSON[BOOT_TEX_KEY] = TEX_FILE_NAMES_CONV;
    try {
      await fsp.stat(path.join(GAME_PATH, READ_WAD_PATH));
      try {
        //write to read .texpack in "wad" subfolders
        //const WAD_FOLDER_FILES = await fs.readdirAsync(path.join(GAME_PATH, READ_WAD_PATH));
        const WAD_FOLDER_FILES = await fsp.readdir(path.join(GAME_PATH, READ_WAD_PATH), {
          recursive: true,
        });
        const WAD_TEX_FILES = WAD_FOLDER_FILES.filter(
          (file) => path.extname(file).toLowerCase() === PACK_EXT,
        );
        ...
        const WAD_TEX_FILE_NAMES = WAD_TEX_FILES.map((file) =>
          file.replace(path.extname(file), "").replace("\\", "/"),
        );
      ...
      log(
        "warn",
        `Could not read "${PACK_PATH_WAD}" folder for ${PACK_EXT} files. Folder proably does not exist.`,
      );
    }
    //.lodpack files
    const LOD_FILES = PATCH_FOLDER_FILES.filter(
      (file) => path.extname(file).toLowerCase() === LOD_EXT,
    );
    ...
    try {
      ...
      try {
        ...
        const WAD_LOD_FILE_NAMES = WAD_LOD_FILES.map((file) =>
          file.replace(path.extname(file), "").replace("\\", "/"),
        );
      ...
      log(
        "warn",
        `Could not read "${PACK_PATH_WAD}" folder for ${LOD_EXT} files. Folder proably does not exist.`,
      );
    }
    await fsp.writeFile(
...
async function resetBootOptions(api) {
  // Reset boot-options.json file (on purge)
  GAME_PATH = getDiscoveryPath(api);
  try {
    //reset boot-options.json file
    ...
    api.showErrorNotification(
      `Could not reset ${BOOT_OPTIONS_FILEPATH} file. Please remove entries manually.`,
      err,
      { allowReport: false },
    );
  }
...
async function setup(discovery, api, gameSpec) {
  ...
  try {
    ...
    api.showErrorNotification(
      `Could not read file "${BOOT_OPTIONS_FILEPATH}". Please verify your game files.`,
      err,
      { allowReport: false },
    );
  }
