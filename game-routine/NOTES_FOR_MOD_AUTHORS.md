# Notes for Mod Authors - ROUTINE

Packaging rules for ROUTINE mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `Routine` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `Routine\Content\Paks\LogicMods` |
| Pak Mods | a `.pak` file | `Routine\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `Routine\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `Routine\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `Routine\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `Routine`, `Engine` or `Content` | the game folder itself (no subfolder) |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | - |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `Routine\Binaries\Win64` |
...
Installs to: `Routine\Content\Paks\LogicMods`
...
Installs to: `Routine\Content\Paks\~mods`
...
Installs to: `Routine\Binaries\Win64`
...
Installs to: `Routine\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `Routine`, `Engine`, `Content`, `Binaries` or `Mods`.
