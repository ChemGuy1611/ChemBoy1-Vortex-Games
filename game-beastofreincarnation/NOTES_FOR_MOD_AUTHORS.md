# Notes for Mod Authors - Beast of Reincarnation

Packaging rules for Beast of Reincarnation mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `BeastOfReincarnation` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `BeastOfReincarnation\Content\Paks\LogicMods` |
| Pak Mods | a `.pak` file | `BeastOfReincarnation\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `BeastOfReincarnation\Binaries\Win64` |
| Signature Bypass | `dsound.dll` and `UniversalSigBypasser.asi` | `BeastOfReincarnation\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `BeastOfReincarnation\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `BeastOfReincarnation\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `BeastOfReincarnation`, `Engine` or `Content` | the game folder itself (no subfolder) |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | - |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `BeastOfReincarnation\Binaries\Win64` |
...
Installs to: `BeastOfReincarnation`
...
Installs to: `BeastOfReincarnation\Content\Paks\LogicMods`
...
Installs to: `BeastOfReincarnation\Content\Paks\~mods`
...
Installs to: `BeastOfReincarnation\Binaries\Win64`
...
- Recognised only when BOTH `dsound.dll` and `UniversalSigBypasser.asi` are present in the archive.
...
Installs to: `BeastOfReincarnation\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `BeastOfReincarnation`, `Engine`, `Content`, `Binaries`, `Mods`, `Paks` or `Movies`.
