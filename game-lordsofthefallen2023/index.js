  ...
  try {
    ...
    try {
      ...
      api.showErrorNotification(
        `Failed to create ${EAC_CONFIG_FILE} to bypass EAC. Must do it manually.`,
        err,
        { allowReport: false },
      );
    }
  ...
  try {
    ...
    if (!test) {
      try {
        ...
        //if the value didn't exist, above is undefined and throws error. Add new value to object
        content.data = {
          ...content.data,
          [EAC_CONFIG_KEY]: {
            [EAC_CONFIG_VALUE]: EAC_CONFIG_SET,
    ...
    api.showErrorNotification(
      `Failed to write ${EAC_CONFIG_FILE} to bypass EAC. Must do it manually.`,
      err,
      { allowReport: false },
    );
  }
...
function installConfig(api, files) {
  ...
  if (IS_CONFIG === false) {
    api.showErrorNotification(
      ...
      `You tried installing a Config mod, but the game, staging folder, and ${CONFIG_LOC} folders are not all on the same drive. Please move the game and/or staging folder to the same drive as the ${CONFIG_LOC} folders (typically C Drive) to install these types of mods with Vortex.`,
      { allowReport: false },
    );
  }
  return Promise.resolve({ instructions });
...
function installSave(api, files) {
  ...
  if (IS_SAVE === false) {
    api.showErrorNotification(
      ...
      `You tried installing a Save mod, but the game, staging folder, and ${SAVE_LOC} folder are not all on the same drive. Please move the game and/or staging folder to the same drive as the ${SAVE_LOC} folder (typically C Drive) to install these types of mods with Vortex.`,
      { allowReport: false },
    );
  }
  return Promise.resolve({ instructions });
          ...
          api.showDialog(
            ...
            MESSAGE,
            {
              text:
                ...
                `Save Path (No installer available yet): ${path.join(SAVE_PATH)}\n` +
                `\n` +
...
//Notification if Config, Save, and Creations folders are not on the same partition
function antiCheatNotify(api) {
  const NOTIF_ID = `${GAME_ID}-anticheatnotify`;
  const MESSAGE = "EAC Anti-Cheat Must Be Disabled";
  api.sendNotification({
    id: NOTIF_ID,
    type: "warning",
    message: MESSAGE,
    ...
    actions: [
      {
        ...
        action: (dismiss) => {
          api.showDialog(
            ...
            {
              text:
                `${GAME_NAME} implements the Easy Anti-Cheat (EAC) anti-cheat engine to detect modified game files.\n` +
                `For Game Pass users, simply launching the shipping executable will bypass EAC and allow you to use mods.\n` +
                `Vortex detected you are using the Steam or Epic version of the game, so you will need to disable EAC manually in order to use mods.\n` +
                `Click the button below to open the PCGamingWiki page with instructions on how to disable EAC.\n` +
                `Note that the extension is already set up to launch the shipping executable directly when launching from Vortex.\n` +
                `\n`,
            },
            [
              //*
