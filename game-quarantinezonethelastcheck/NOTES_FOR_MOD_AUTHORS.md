# Notes for Mod Authors - Quarantine Zone: The Last Check

Packaging rules for Quarantine Zone: The Last Check mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `QZSim` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `QZSim\Content\Paks\LogicMods` |
| Pak Mods | a `.pak` file | `QZSim\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `QZSim\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `QZSim\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `QZSim\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `QZSim`, `Engine` or `Content` | the game folder itself (no subfolder) |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | - |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `QZSim\Binaries\Win64` |
...
Installs to: `QZSim\Content\Paks\LogicMods`
...
Installs to: `QZSim\Content\Paks\~mods`
...
Installs to: `QZSim\Binaries\Win64`
...
Installs to: `QZSim\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `QZSim`, `Engine`, `Content`, `Binaries` or `Mods`.
