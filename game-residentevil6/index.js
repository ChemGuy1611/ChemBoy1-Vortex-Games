/*///////////////////////////////////////////
Name: Resident Evil 6 Vortex Extension
...
const GAME_ID = "residentevil6";
const STEAMAPP_ID = "221040"; // https://steamdb.info/app/221040/
...
const GAME_NAME = "Resident Evil 6";
const GAME_NAME_SHORT = "RE 6";
const BINARIES_PATH = '.';
const EXEC = "BH6.exe";
const PCGAMINGWIKI_URL = "https://www.pcgamingwiki.com/wiki/Resident_Evil_6";
const EXTENSION_URL = "https://www.nexusmods.com/site/mods/2029"; //Nexus link to this extension. Used for links
...
const APPDATA_FOLDER = path.join('capcom', 'RESIDENT EVIL 6');
...
const SAVE_FOLDER = path.join(SAVEMOD_LOCATION, APPDATA_FOLDER);
let USERID_FOLDER = "";
if (hasUserIdFolder) {
  try {
...
let CONFIG_PATH = path.join(CONFIGMOD_LOCATION, APPDATA_FOLDER);
const CONFIG_PATH_XBOX = CONFIG_PATH; //XBOX Version
const CONFIG_FILES = ["config.ini", "mapping.ini"];
...
function applyGame(context, gameSpec) {
  ...
  //register actions
  context.registerAction('mod-icons', 300, 'open-ext', {}, `Open ${CONFIG_FILES[0]}`, () => {
  ...
  context.registerAction('mod-icons', 300, 'open-ext', {}, `Open ${CONFIG_FILES[1]}`, () => {
    util.opn(path.join(CONFIG_PATH, CONFIG_FILES[1])).catch(() => null);
    }, () => {
      const state = context.api.getState();
      const gameId = selectors.activeGameId(state);
      return gameId === GAME_ID;
