...
const LO_FILE_HEADER = '<?xml version="1.0" encoding="utf-8"?>\r\n'
...
function getLoaderRoot(files) {
  const inPatchFiles = files.find(file =>
...
function installLoader(files) {
  ...
  if (!filtered.some(file => path.basename(file) === LOADER_ASSEMBLY_PATCHED)) {
    log('warn', `${LOADER_ASSEMBLY_PATCHED} is missing from ${rootPath} - the game assembly will not be patched`);
  }
...
function installFallback(api, files, destinationPath) {
  if (!updating_mod) { //a mod update that lands here should not raise the "fallback reached" notice
...
async function ensureNexusRequirementMod(api) {
  try {
    ...
    const existing = Object.values(mods).find(mod =>
      String(mod.attributes?.modId) === String(LOADER_PAGE_NO));
    if (existing !== undefined) { //already tracked - the placeholder, or a real Nexus install
      return;
    }
    ...
    } catch { //offline or not logged in - fall back to the pinned ids
    }
    ...
    const mod = {
      ...
      attributes: {
        ...
        shortDescription: 'Placeholder so Vortex sees the Lobotomy Mod Manager requirement as met. The loader itself is installed from GitHub, and this mod deploys no files.',
    ...
    await new Promise((resolve, reject) => {
      api.events.emit('create-mod', GAME_ID, mod, (err) => (err !== null ? reject(err) : resolve()));
    });
    api.store.dispatch(actions.setModEnabled(profile.id, modId, true));
  } catch (err) { //never block or shout over the loader install
...
async function downloadNexusInstaller(api) {
  ...
  } catch (err) { //courtesy download - it must never block or shout over the loader install
...
async function deserializeLoadOrder(context) {
  ...
  const findMod = (folder) => Object.values(mods).find(mod =>
    (util.getSafe(mods[mod.id]?.attributes, [LO_ATTRIBUTE], '') === folder));
  ...
  const getModName = (folder) => {
    ...
    const name = modMatch.attributes?.customFileName
  ...
  listed.forEach(entry => {
    if (!modFolders.includes(entry.folder)) { //drop entries whose folder is gone
      return;
    }
...
async function serializeLoadOrder(context, loadOrder) {
  ...
  return fs.writeFileAsync(
    path.join(modFolderPath, LO_FILE),
    buildLoadOrderFile(loadOrder),
    { encoding: 'utf8' },
  );
}
...
function applyGame(context, gameSpec) {
  ...
  //register actions
  if (LOAD_ORDER_ENABLED) {
    context.registerAction('mod-icons', 300, 'open-ext', {}, `Open ${LO_FILE} File`, () => {
      const modFolderPath = getModFolderPath(context.api);
      if (modFolderPath !== undefined) {
  ...
  if (nexusCreditDownload) {
    context.registerAction('mod-icons', 300, 'open-ext', {}, `Open ${LOADER_NAME} Nexus Page`, () => {
      util.opn(`https://www.nexusmods.com/${LOADER_DOMAIN}/mods/${LOADER_PAGE_NO}`).catch(() => null);
    }, () => {
      const state = context.api.getState();
      const gameId = selectors.activeGameId(state);
      return gameId === GAME_ID;
