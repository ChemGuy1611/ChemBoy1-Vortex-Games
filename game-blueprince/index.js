...
const GAME_ID = "blueprince";
const STEAMAPP_ID = "1569580";
const STEAMAPP_ID_DEMO = null;
const EPICAPP_ID = null;
const GOGAPP_ID = null;
const XBOXAPP_ID = "RawFury.BluePrince";
const XBOXEXECNAME = "Game";
const DISCOVERY_IDS_ACTIVE = [STEAMAPP_ID, XBOXAPP_ID]; // UPDATE THIS WITH ALL VALID IDs
const GAME_NAME = "Blue Prince";
const GAME_NAME_SHORT = "Blue Prince";
const GAME_STRING = "BLUE PRINCE"; //string for exe and data folder (seem to always match)
const EXEC = `${GAME_STRING}.exe`;
const DATA_FOLDER = `${GAME_STRING}_Data`;
const DEV_REGSTRING = "Dogubomb"; //developer name
const GAME_REGSTRING = "BLUE PRINCE"; //game name
const XBOX_SAVE_STRING = "9s0pnehqffj7t"; //string after "ID_"
...
const EXTENSION_URL = "https://www.nexusmods.com/site/mods/1519"; //Nexus link to this extension. Used for links
const PCGAMINGWIKI_URL = "https://www.pcgamingwiki.com/wiki/Blue_Prince";
    ...
    files.map(async (file) => {
      if (PLUGIN_EXTS.includes(path.extname(file).toLowerCase())) {
        try {
          ...
          api.showErrorNotification(
            `Failed to read mod file "${file}" to determine if for BepInEx or MelonLoader`,
            err,
          );
        }
      }
...
async function chooseModLoader(api, gameSpec) {
  ...
  return api
    .showDialog(
      ...
      [{ label: t("MelonLoader (Recommended)") }, { label: t("BepInEx") }],
    )
    .then(async (result) => {
      if (result === undefined) {
        return;
...
async function setup(discovery, api, gameSpec) {
  ...
  if (bepinexInstalled && allowBepCfgMan) {
    await downloadBepCfgManNotify(api, gameSpec); //notification to download BepInExConfigManager
  } //*/
...
function main(context) {
  ...
  context.once(() => {
    ...
    context.api.onAsync("did-deploy", async (profileId, deployment) => {
      ...
      if (bepinexInstalled && allowBepCfgMan) {
        await downloadBepCfgMan(context.api, spec); //download BepInExConfigManager
      } //*/
      if (melonInstalled && allowMelPrefMan) {
