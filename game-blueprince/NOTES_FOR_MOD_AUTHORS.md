# Notes for Mod Authors - Blue Prince

Packaging rules for Blue Prince mods, so Vortex installs them to the right place.
...
| Root / Game Folder Mods | a `BLUE PRINCE_Data` folder | the game folder itself (no subfolder) |
...
| Asset Replacement Mods | a `.assets` file | `BLUE PRINCE_Data` |
...
- Recognised by a folder named `BLUE PRINCE_Data` or `BLUE PRINCE_Data` in the archive.
