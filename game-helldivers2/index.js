...
const PATCH_CONFLICT_ICON =
  "M1 10.25H5.5V7L9.5 12L5.5 17V13.75H1V10.25ZM23 10.25H18.5V7L14.5 12L18.5 17V13.75H23V10.25ZM10.85 3.4H13.15V13.1H10.85V3.4ZM10.85 15.1H13.15V17.7H10.85V15.1Z";
...
const setPatchOverride = (profileId, hash, order) => ({
  type: SET_PATCH_OVERRIDE,
  payload: { profileId, hash, order },
});
const clearPatchOverride = (profileId, hash) => ({
  type: CLEAR_PATCH_OVERRIDE,
  payload: { profileId, hash },
});
  ...
  if (ARCHIVE_CATALOG === null) {
    ...
    try {
      const raw = JSON.parse(
        fs.readFileSync(path.join(__dirname, ARCHIVE_CATALOG_FILE), { encoding: "utf8" }),
      );
...
async function readManifest(files, destinationPath) {
  ...
  const entry = files.find(
    (file) => path.dirname(file) === "." && path.basename(file).toLowerCase() === MANIFEST_FILE,
  );
...
function collectArchiveDirs(files) {
  ...
  for (const file of files) {
    const segments = normalizeRelPath(path.dirname(file))
      .split("/")
...
function parseInclude(raw, archiveDirs, label) {
  ...
  for (const value of list) {
    ...
    if (!archiveDirs.has(dir)) {
      log(
        "warn",
        `[${GAME_ID}] ${MANIFEST_FILE}: ${label} includes "${value}", which the archive does not contain`,
      );
      continue;
    }
  ...
  return list.reduce((accum, entry, idx) => {
    ...
    const subOptions = rawSubs.reduce((subs, sub, subIdx) => {
      ...
      if (include.length === 0) {
        log(
          "warn",
          `[${GAME_ID}] ${MANIFEST_FILE}: sub-option "${name} / ${subName}" installs nothing, skipping it`,
        );
        return subs;
      }
      subs.push({
...
function parseManifestTree(data, dataPath, archiveDirs, imageFiles) {
  ...
  if (version > 2) {
    log(
      "warn",
      `[${GAME_ID}] ${MANIFEST_FILE} declares version ${version}, which is newer than this extension knows; reading it as version 2`,
    );
  }
  ...
  if (options.length === 0 && Array.isArray(data.Options) && data.Options.length > 0) {
    log(
      "warn",
      `[${GAME_ID}] ${MANIFEST_FILE} lists options but none of them are usable, using the folder layout instead`,
    );
    return undefined;
  }
  ...
  for (const option of options) {
    if (
      ...
      log(
        "warn",
        `[${GAME_ID}] ${MANIFEST_FILE}: option "${option.name}" references category "${option.categoryId}", which is not declared`,
      );
      option.categoryId = undefined;
    }
  }
...
async function installPatchMulti(files, destinationPath) {
  const patchFiles = files.filter(
    (file) => !file.endsWith(path.sep) && parsePatchFile(path.basename(file)) !== undefined,
  );
  ...
  if (manifest !== undefined) {
    ...
    if (manifest.data !== undefined) {
      tree = parseManifestTree(
        manifest.data,
        destinationPath,
        collectArchiveDirs(files),
        manifestFiles,
      );
    }
  }
  ...
  const selection = needsQuery(tree) ? await requestOptionSelection(tree) : defaultSelection(tree);
  return {
    ...
    actions: [
      {
        ...
        action: (dismiss) => {
          api.showDialog(
            "info",
            "Patch targets a missing archive",
            {
              text:
                `"${name}" contains patch files for the following archives, which are not present ` +
...
async function buildPatchPlan(api, profileId) {
  ...
  for (const modId of ordered) {
      ...
      for (const hash of claimed.map((h) => String(h).toLowerCase())) {
        if (!present.has(hash)) {
          warnings.push({
            ...
            modId,
            message:
              `"${getModName(mod)}" is recorded as patching archive ${hash} but has no such ` +
    ...
    const modContributions = Array.from(grouped.values()).sort(
      (lhs, rhs) => lhs.localIdx - rhs.localIdx || lhs.hash.localeCompare(rhs.hash),
    );
  ...
  for (const hash of Object.keys(byArchive)) {
    ...
    if (Array.isArray(override) && override.length > 0) {
      ...
      list.sort((lhs, rhs) => {
        const lhsRank = overrideRank.has(lhs.modId)
          ? overrideRank.get(lhs.modId)
          : Number.MAX_SAFE_INTEGER;
        const rhsRank = overrideRank.has(rhs.modId)
          ? overrideRank.get(rhs.modId)
          : Number.MAX_SAFE_INTEGER;
    ...
    list.forEach((contribution, idx) => {
      contribution.assigned = idx;
    });
    ...
    if (!baseArchives.has(hash) && baseArchives.size > 0) {
      warnings.push({
        ...
        modId: undefined,
        message:
          `Archive ${hash} (${archiveDisplayName(hash)}) is not present in the game folder, ` +
  ...
  for (const hash of Object.keys(byArchive)) {
    const assigned = byArchive[hash]
      .map((contribution) => contribution.assigned)
      .sort((lhs, rhs) => lhs - rhs);
    for (let i = 0; i < assigned.length; ++i) {
      if (assigned[i] !== i) {
        throw new Error(
          `Helldivers 2: patch numbering for archive ${hash} is not contiguous ` +
  ...
  for (const hash of Object.keys(byArchive)) {
    for (const contribution of byArchive[hash]) {
      assignments[`${contribution.modId}|${contribution.hash}|${contribution.localIdx}`] =
  ...
  CONFLICT_MODS = new Set(
    conflicts.reduce(
      (accum, hash) => accum.concat(byArchive[hash].map((contribution) => contribution.modId)),
      [],
    ),
  );
...
function resolveModId(api, filePath) {
  ...
  for (const modId of Object.keys(mods)) {
    ...
    const modRoot =
      path.normalize(path.join(stagingPath, installationPath)).toLowerCase() + path.sep;
...
const mergeOperation = async (filePath, mergePath, context) => {
  ...
  const assigned =
    modId !== undefined ? plan.assignments[`${modId}|${parsed.hash}|${parsed.index}`] : undefined;
  ...
  try {
    ...
    api.showErrorNotification(
      ...
      new Error(
        `Deployment stopped while merging "${modName}". Helldivers 2 needs an archive's ` +
...
async function verifyMergedOutput(api, plan) {
  ...
  api.showErrorNotification(
    ...
    new Error(
      "Helldivers 2 needs each archive's patch files to be numbered without gaps. The " +
        "following archives are missing files, so their mods will not work correctly:\n\n" +
        broken
          .map(
            (entry) =>
              `    ${entry.hash} (${archiveDisplayName(entry.hash)}) - missing ` +
              `patch_${entry.missing.join(", patch_")}`,
          )
          .join("\n") +
...
async function convertSoundPatchMods(api) {
  ...
  util.batchDispatch(
    api.store,
    soundMods.map((modId) => actions.setModType(GAME_ID, modId, PATCH_ID)),
  );
  invalidatePlan();
  log(
    "info",
    `[${GAME_ID}] migrated ${soundMods.length} sound patch mods to the merged patch mod type`,
  );
    ...
    actions: [
      {
        ...
        action: (dismiss) => {
          api.showDialog(
            "info",
            "Sound mods now use the load order",
            {
              text:
                "Sound mods used to be installed without being renamed, so two sound mods for the " +
...
function applyGame(context, gameSpec) {
  ...
  context.registerInstaller(PATCH_ID, 25, testPatch, (files, destinationPath) =>
    installPatchMulti(files, destinationPath),
  );
    ...
    actions: [
      {
        title: "Deploy",
        action: () =>
          context.api.events.emit("deploy-mods", (err) => {
...
async function deserializeLoadOrder(context) {
  ...
  for (const entry of stored) {
    //stored order first, dropping mods that are gone
  ...
  for (const modId of Array.from(patchMods).sort()) {
    //then anything newly installed
    if (seen.has(modId)) continue;
...
function LoadOrderInstructions() {
  ...
  const matched =
    statusFilter.size > 0
      ? loadOrder.filter((entry) => matchesStatus(entry, statusFilter, isEnabled, isEntryLocked))
          .length
      : total;
...
function LoadOrderItemRenderer(props) {
  ...
  const firstUnlocked = loadOrder.findIndex((entry) => !isEntryLocked(entry));
  ...
  const onApplyIndex = React.useCallback(
    (idx) => {
      if (currentIdx === idx || isEntryLocked(loEntry)) return;
      ...
      const dest = loadOrder.filter(
        (entry, i) => !isEntryLocked(entry) && entry.id !== loEntry.id && i < bound,
      ).length;
      const unlocked = loadOrder.filter(
        (entry) => !isEntryLocked(entry) && entry.id !== loEntry.id,
      );
      unlocked.splice(dest, 0, loEntry);
      let next = 0;
      const newLO = loadOrder.map((entry) => (isEntryLocked(entry) ? entry : unlocked[next++]));
  ...
  const allIds = React.useMemo(
    () =>
      loadOrder
        ...
        .map((entry) => entry.id),
    [loadOrder, statusFilter, profile],
  );
  ...
  const onLock = React.useCallback(() => {
    const newLO = loadOrder.map((entry) =>
      entry.id === loEntry.id ? { ...entry, locked: !entryLocked } : entry,
    );
    dispatch(actions.setFBLoadOrder(profile.id, newLO));
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(Icon, {
        name: entryLocked ? "locked" : "unlocked",
        style: { color: entryLocked ? "#e2c04c" : "inherit" },
      }),
    ),
    React.createElement(
      "div",
    ...
    React.createElement(
      "p",
      { className: "load-order-name", style: { flex: 1, marginBottom: 0 } },
      loEntry.name,
    ),
    archives.length > 0
      ? React.createElement(
...
function FbloContextMenu({
    ...
    return React.createElement(
      ...
      menuItem(`Lock Selected (${n})`, () =>
        applyToTargets(
          (lo) =>
            lo.map((entry) =>
              targets.find((target) => target.id === entry.id) ? { ...entry, locked: true } : entry,
            ),
          true,
        ),
      ),
      menuItem(`Unlock Selected (${n})`, () =>
        applyToTargets(
          (lo) =>
            lo.map((entry) =>
              targets.find((target) => target.id === entry.id)
                ? { ...entry, locked: false }
                : entry,
            ),
          true,
      ...
      menuItem(`Move to Top (${n})`, () =>
        applyToTargets((lo) => {
          ...
          const selected = lo.filter(
            (entry) => targets.find((target) => target.id === entry.id) && !isEntryLocked(entry),
          );
          const rest = lo.filter(
            (entry) => !isEntryLocked(entry) && !targets.find((target) => target.id === entry.id),
          );
          const reordered = [...selected, ...rest];
          let next = 0;
          return lo.map((entry) => (isEntryLocked(entry) ? entry : reordered[next++]));
        }, true),
      ),
  ...
  return React.createElement(
    "div",
    { ref: clampRef, style: menuStyle },
    menuItem(isEnabled(item) ? "Disable" : "Enable", () =>
      setVortexEnabled([item], !isEnabled(item)),
    ),
    menuItem(entryLocked ? "Unlock Position" : "Lock Position", () =>
      applyToTargets(
        (lo) =>
          lo.map((entry) => (entry.id === item.id ? { ...entry, locked: !entryLocked } : entry)),
        true,
      ),
    ),
    React.createElement("div", { style: sepStyle }),
    menuItem("Move to Top", () =>
      applyToTargets((lo) => {
        ...
        //Locked entries hold their absolute index - only the unlocked entries reorder into the slots between them
        const moved = lo.filter((entry) => !isEntryLocked(entry) && entry.id === item.id);
...
function PatchOptionsDialog() {
  ...
  React.useEffect(() => {
    ...
    return () => {
      OPTION_LISTENERS.delete(setRequest);
    };
  }, []);
  ...
  const unanswered = tree.options.filter(
    (option) =>
  ...
  const renderSubOption = (option, sub) =>
    React.createElement(
      "div",
  ...
  const grouped = [
    {
      category: undefined,
      options: tree.options.filter((option) => option.categoryId === undefined),
    },
  ]
    .concat(
  ...
  return React.createElement(
    ...
    React.createElement(
      Modal.Body,
      null,
      tree.description !== undefined ? React.createElement("p", null, tree.description) : null,
      React.createElement(
        "div",
        { className: "hd2-option-actions" },
        React.createElement(
          "p",
          null,
          tree.exclusive
      ...
      React.createElement(
        "div",
        { className: "hd2-option-layout" },
        React.createElement(
          "div",
          { className: "hd2-option-list" },
          grouped.map((group) =>
            React.createElement(
              "div",
              { key: group.category?.id ?? "hd2-ungrouped" },
              group.category !== undefined
                ? React.createElement("h5", null, group.category.name)
                : null,
        ...
        React.createElement(
          ...
          focusedImage !== undefined
            ...
            : null,
          focused !== undefined ? React.createElement("h5", null, focused.name) : null,
          focused !== undefined && focused.description !== undefined
            ? React.createElement("p", null, focused.description)
            : null,
        ),
      ),
      unanswered.length > 0
  ...
  if (plan === undefined) {
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          "p",
          { style: { padding: 12, fontWeight: "bold", color: "yellow" } },
              ...
              ...plan.conflicts.map((hash) => {
                ...
                return React.createElement(
                  ...
                  React.createElement(
                    ...
                    React.createElement(Icon, {
                      name: isOpen ? "showhide-down" : "showhide-right",
                      style: { marginRight: 6 },
                    }),
                    React.createElement(
                      "span",
                      { style: { fontWeight: "bold" } },
                      archiveDisplayName(hash),
                    ),
                    React.createElement(
                      "span",
                      { style: { marginLeft: 8, opacity: 0.7, fontFamily: "monospace" } },
                      hash,
                    ),
                    React.createElement(
                      "span",
                    ...
                    ? React.createElement(
                        ...
                        ...contributions.map((contribution, idx) =>
                          React.createElement(
                            ...
                            React.createElement(
                              "span",
                              { style: { fontFamily: "monospace", minWidth: 90, opacity: 0.8 } },
                              `patch_${contribution.assigned}`,
                            ),
                            React.createElement(
                              "span",
                              { style: { flex: 1 } },
                              contribution.modName,
                            ),
                            React.createElement(
                              Button,
                              {
...
function main(context) {
  ...
  context.registerMerge(
    //register merger for patch mods (graphics and sound alike)
  ...
  context.once(() => {
    ...
    api.events.on("gamemode-activated", (gameId) => {
      ...
      convertSoundPatchMods(api).catch((err) =>
        log("warn", `[${GAME_ID}] could not convert sound patch mods`, err),
      );
    });
    ...
    api.onAsync("will-deploy", async (profileId) => {
      const lastActiveHelldiverProfile = selectors.lastActiveProfileForGame(
        api.getState(),
        GAME_ID,
      );
      ...
      try {
        ...
        api.showErrorNotification("Could not work out the patch order", err, {
          allowReport: false,
        });
        throw err;
      }
    ...
    api.events.on("did-install-mod", (gameId, archiveId, modId) => {
      ...
      invalidatePlan();
      warnAboutInertArchives(api, modId).catch((err) =>
        log("warn", `[${GAME_ID}] could not check the mod's archives`, err),
      );
    });
