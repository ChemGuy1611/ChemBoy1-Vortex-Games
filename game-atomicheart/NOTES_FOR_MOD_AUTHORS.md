# Notes for Mod Authors - Atomic Heart

Packaging rules for Atomic Heart mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `AtomicHeart` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `AtomicHeart\Content\Paks\LogicMods` |
| Pak Mods | a `.pak` file | `AtomicHeart\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `AtomicHeart\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `AtomicHeart\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `AtomicHeart\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `AtomicHeart`, `Engine` or `Content` | the game folder itself (no subfolder) |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | - |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `AtomicHeart\Binaries\Win64` |
...
Installs to: `AtomicHeart\Content\Paks\LogicMods`
...
Installs to: `AtomicHeart\Content\Paks\~mods`
...
Installs to: `AtomicHeart\Binaries\Win64`
...
Installs to: `AtomicHeart\Binaries\Win64\ue4ss\Mods`
