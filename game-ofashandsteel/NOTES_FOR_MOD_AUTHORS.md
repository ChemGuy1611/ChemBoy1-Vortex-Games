# Notes for Mod Authors - Of Ash and Steel

Packaging rules for Of Ash and Steel mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `ofAshAndSteelGame` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `ofAshAndSteelGame\Content\Paks\LogicMods` |
| Pak Mods | a `.pak` file | `ofAshAndSteelGame\Content\Paks` |
| UE4SS Itself | a `dwmapi.dll` file | `ofAshAndSteelGame\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `ofAshAndSteelGame\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `ofAshAndSteelGame\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `ofAshAndSteelGame`, `Engine` or `Content` | the game folder itself (no subfolder) |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | - |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `ofAshAndSteelGame\Binaries\Win64` |
...
Installs to: `ofAshAndSteelGame\Content\Paks\LogicMods`
...
Installs to: `ofAshAndSteelGame\Content\Paks`
...
Installs to: `ofAshAndSteelGame\Binaries\Win64`
...
Installs to: `ofAshAndSteelGame\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `ofAshAndSteelGame`, `Engine`, `Content`, `Binaries` or `Mods`.
