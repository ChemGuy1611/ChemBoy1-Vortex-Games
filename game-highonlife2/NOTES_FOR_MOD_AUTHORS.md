# Notes for Mod Authors - High on Life 2

Packaging rules for High on Life 2 mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `HighOnLife2` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `HighOnLife2\Content\Paks\LogicMods` |
| Pak Mods | a `.pak` file | `HighOnLife2\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `HighOnLife2\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `HighOnLife2\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `HighOnLife2\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `HighOnLife2`, `Engine` or `Content` | the game folder itself (no subfolder) |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | - |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `HighOnLife2\Binaries\Win64` |
...
Installs to: `HighOnLife2\Content\Paks\LogicMods`
...
Installs to: `HighOnLife2\Content\Paks\~mods`
...
Installs to: `HighOnLife2\Binaries\Win64`
...
Installs to: `HighOnLife2\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `HighOnLife2`, `Engine`, `Content`, `Binaries` or `Mods`.
