# Notes for Mod Authors - Crisol: Theater of Idols

Packaging rules for Crisol: Theater of Idols mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `CRToiPrototype` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `CRToiPrototype\Content\Paks\LogicMods` |
| Pak Mods | a `.pak` file | `CRToiPrototype\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `CRToiPrototype\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `CRToiPrototype\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `CRToiPrototype\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `CRToiPrototype`, `Engine` or `Content` | the game folder itself (no subfolder) |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | - |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `CRToiPrototype\Binaries\Win64` |
...
Installs to: `CRToiPrototype\Content\Paks\LogicMods`
...
Installs to: `CRToiPrototype\Content\Paks\~mods`
...
Installs to: `CRToiPrototype\Binaries\Win64`
...
Installs to: `CRToiPrototype\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `CRToiPrototype`, `Engine`, `Content`, `Binaries` or `Mods`.
