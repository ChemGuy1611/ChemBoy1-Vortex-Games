...
const GAME_ID = "dragonballsparkingzero"; //same as Nexus domain
const STEAMAPP_ID = "1790600"; //from steamdb.info
...
const GAME_NAME_SHORT = "DB Sparking! Zero"; //Try for 8-10 characters
const EPIC_CODE_NAME = "SparkingZERO"; //Folder in root
const EXEC = "SparkingZERO.exe"; //This is true ~80% of the time. Change if different
...
const EXTENSION_URL = "https://www.nexusmods.com/site/mods/1055"; //Nexus link to this extension. Used for links
...
const UE5_SORTABLE_ID = `${GAME_ID}-ue5-sortable-modtype`; //already game-specific pre-migration, kept per Step C2 no-rename rule
const LEGACY_UE5_SORTABLE_ID = "ue5-sortable-modtype"; //bare pre-namespacing id this game's 0.5.1 release also carried - kept so already-installed mods stay visible
...
const LOGICMODS_PATH = path.join(EPIC_CODE_NAME, "Content", "Paks", "LogicMods"); //this game's LOGICMODS_PATH already ends in the LogicMods folder itself, unlike most template adopters
...
//Signature Bypass - this game ships its own SigBypass mod, not the universal site/1416 tool
const SIGBYPASS_ID = `${GAME_ID}-sigbypass`;
const SIGBYPASS_NAME = "Signature Bypass";
const SIGBYPASS_DLL = "dsound.dll";
const SIGBYPASS_LUA = "dbsparkingzeroutocbypass.asi"; //not actually lua - this game's companion file is an .asi
const SIGBYPASS_PAGE_NO = 18;
const SIGBYPASS_FILE_NO = 2175;
const SIGBYPASS_DOMAIN = GAME_ID;
...
//SZModLoader (this game's own third-party mod loader, no template equivalent)
...
const MODLOADER_DOMAIN = GAME_ID;

//LFSE (SZModLoader sibling tool)
const LFSE_ID = `${GAME_ID}-lfse`;
const LFSE_PATH = path.join(EPIC_CODE_NAME, "Mods");
const LFSE_FOLDER = "LFSE";

//Individual SZModLoader mods (single .uplugin file)
const MODLOADERMOD_ID = `${GAME_ID}-modloadermod`;
const MODLOADERMOD_NAME = "SZModLoader Mod";
const MODLOADERMOD_PATH = path.join(EPIC_CODE_NAME, "Mods");
const MODLOADERMOD_EXT = ".uplugin";

//SZModLoader JSON mods + the JsonFiles.json manifest SZModLoader reads on launch
const JSON_ID = `${GAME_ID}-json`;
const JSON_NAME = "SZModLoader JSON";
...
if (ue4ssLoadOrder) {
  ...
  MODTYPE_FOLDERS.push(LOGICMODS_PATH); //LOGICMODS_PATH already ends in the LogicMods folder for this game
}
if (hasModKit) {
  MODTYPE_FOLDERS.push(MODKITMOD_PATH);
}
MODTYPE_FOLDERS.push(MODLOADER_PATH, JSON_PATH); //SZModLoader Mods folder (also LFSE/ModLoaderMod target) + JSON subfolder
...
//SZModLoader + LFSE + individual .uplugin mods + JSON mods - always present for this game
spec.modTypes.push(
  {
    id: MODLOADER_ID,
    name: MODLOADER_NAME,
...
//Test for SZModLoader itself (the mod loader runtime, no template equivalent)
...
//Test for LFSE (SZModLoader sibling tool, no template equivalent)
...
//Test for individual SZModLoader mods (packaged as a single .uplugin file)
function testModLoaderMod(files, gameId) {
  const isMod =
    files.find((file) => path.extname(file).toLowerCase() === MODLOADERMOD_EXT) !== undefined;
  let supported = gameId === spec.game.id && isMod;
...
//Install individual SZModLoader mods
function installModLoaderMod(files) {
  ...
  const instructions = filtered.map((file) => {
    return {
      type: "copy",
      source: file,
      destination: path.join(path.basename(modFile, MODLOADERMOD_EXT), file.substr(idx)),
    };
  });
  instructions.push(setModTypeInstruction);
  return Promise.resolve({ instructions });
...
//Test for SZModLoader JSON mods
...
//Check if SZModLoader is installed
...
//Function to auto-download SZModLoader from Nexus (this game's own page, no template equivalent)
async function downloadModLoader(api, gameSpec, check = true) {
  ...
  if (!isInstalled || !check) {
    ...
    const GAME_DOMAIN = MODLOADER_DOMAIN;
    api.sendNotification({
      //notification indicating install process
      id: NOTIF_ID,
      message: `Installing ${MOD_NAME}`,
...
async function resetJsonFiles(api) {
  ...
  try {
    ...
    api.showErrorNotification(
      `Could not reset ${JSONFILES_FILE} file. Please remove entries manually.`,
      err,
      {
        allowReport: false,
      },
...
async function deserializeLogicMods(api) {
  ...
  GAME_PATH = getDiscoveryPath(api);
  const logicModsFolder = path.join(GAME_PATH, LOGICMODS_PATH); //LOGICMODS_PATH already ends in the LogicMods folder for this game
...
async function migrateLegacyUe5SortableType100(api, oldVersion) {
...
async function setup(discovery, api, gameSpec) {
  ...
  //saves live inside this game's own install folder, not Local AppData - SAVE_PATH_DEFAULT/
  //SAVEMOD_LOCATION are unused for this game, recompute the real path now that GAME_PATH is known
  SAVE_PATH = getUserIdFolder(path.join(GAME_PATH, "SaveGame"));
  ...
  await downloadModLoader(api, gameSpec); //SZModLoader, no template equivalent
  if (ue4ssLoadOrder) {
...
function applyGame(context, gameSpec) {
  ...
  //stays registered as a deploy-safety net for any mod not yet retagged by
  //migrateLegacyUe5SortableType() (registerMigration/gamemode-activated runs it silently)
  context.registerModType(
  ...
  //!audit-skip: installer-priority - same priority as MODKITMOD_ID above, but hasModKit is a fixed
  //false const for this game (no ModKit here), so that branch never registers; exactly one of the
  //two installers at 25 is ever live
  ...
  context.registerInstaller(LFSE_ID, 44, testLFSE, installLFSE);
  context.registerInstaller(MODLOADERMOD_ID, 45, testModLoaderMod, installModLoaderMod);
  context.registerInstaller(JSON_ID, 46, testJson, installJson);
  ...
  context.registerAction(
    ...
    () => {
      ...
      util.opn(path.join(discovery.path, MODLOADER_PATH)).catch(() => null);
    },
    () => {
      const state = context.api.getState();
      const gameId = selectors.activeGameId(state);
  ...
  context.registerAction(
    ...
    () => {
      const state = context.api.getState();
      const discovery = selectors.discoveryByGame(state, GAME_ID);
      util.opn(path.join(discovery.path, JSON_PATH)).catch(() => null);
    },
    () => {
      const state = context.api.getState();
      const gameId = selectors.activeGameId(state);
  ...
  context.registerAction(
    ...
    () => {
      const state = context.api.getState();
      const discovery = selectors.discoveryByGame(state, GAME_ID);
      util.opn(path.join(discovery.path, JSON_PATH, JSONFILES_FILE)).catch(() => null);
    },
    () => {
      const state = context.api.getState();
      const gameId = selectors.activeGameId(state);
...
function main(context) {
  ...
  context.registerMigration((old) => migrateLegacyUe5SortableType100(context.api, old));
  if (UNREALDATA.loadOrder === true) {
  ...
  context.once(() => {
    ...
    //Permanent safety net for any mod that slipped past the registerMigration version gate -
...
async function didPurge(api, profileId) {
  ...
  await resetJsonFiles(api); //SZModLoader JsonFiles.json manifest, no template equivalent
  return Promise.resolve();
}
  ...
  if (isMulti) {
    ...
    return React.createElement(
      ...
      menuItem(`Open LogicMods Folder (${n})`, () => {
        util.opn(path.join(GAME_PATH, LOGICMODS_PATH)).catch(() => null); //LOGICMODS_PATH already ends in the LogicMods folder for this game
        onClose();
      }),
      React.createElement("div", { style: sepStyle }),
      menuItem(`Disable Selected (${n})`, () => {
