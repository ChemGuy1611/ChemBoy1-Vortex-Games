# Notes for Mod Authors - SurrounDead

Packaging rules for SurrounDead mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `SurrounDead` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `SurrounDead\Content\Paks\LogicMods` |
| Pak Mods | a `.pak` file | `SurrounDead\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `SurrounDead\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `SurrounDead\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `SurrounDead\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `SurrounDead`, `Engine` or `Content` | the game folder itself (no subfolder) |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | - |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `SurrounDead\Binaries\Win64` |
...
Installs to: `SurrounDead\Content\Paks\LogicMods`
...
Installs to: `SurrounDead\Content\Paks\~mods`
...
Installs to: `SurrounDead\Binaries\Win64`
...
Installs to: `SurrounDead\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `SurrounDead`, `Engine`, `Content`, `Binaries` or `Mods`.
