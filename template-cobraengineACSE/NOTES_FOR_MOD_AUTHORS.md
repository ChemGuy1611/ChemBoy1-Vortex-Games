# Notes for Mod Authors - template-cobraengineACSE (template)

Packaging rules for template-cobraengineACSE (template) mods, so Vortex installs them to the right place.
...
- Recognised by a folder named `Win64`, `Blueprints`, `TerrainSkirts`, `ProvidedCustomTextures`, `Parks` or `Movies` in the archive.
...
Installs to: `USER_HOME\Saved Games\Frontier Developments\XXX\USERID_FOLDER\Saves`
