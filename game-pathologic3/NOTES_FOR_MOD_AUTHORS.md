# Notes for Mod Authors - Pathologic 3

Packaging rules for Pathologic 3 mods, so Vortex installs them to the right place.
...
| Root / Game Folder Mods | a `Pathologic3_Data` folder or a `dxgi.dll` file | the game folder itself (no subfolder) |
...
Recognised when the archive contains a file or folder named `P2ModLoader.exe`.

## Mod

Recognised when the archive contains a file or folder named `modinfo.ltx` and a file with one of these extensions: `.xml`.
...
- Recognised by a folder named `Pathologic3_Data` or `MonoBleedingEdge` in the archive.
- Recognised by a file named `dxgi.dll`, `version.dll`, `nvngx.dll`, `libxess.dll` or `amd_fidelityfx_dx12.dll`.
