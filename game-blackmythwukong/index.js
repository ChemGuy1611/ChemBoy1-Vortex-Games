...
const GAME_ID = "blackmythwukong"; //same as Nexus domain
const STEAMAPP_ID = "2358720"; //from steamdb.info
const STEAMAPP_ID_DEMO = "XXX"; //VERIFY if the EPIC_CODE_NAME and EXEC_DEMO match Steam full game
const EPICAPP_ID = "f53c5471fd0e47619e72b6d21a527abe"; //from egdata.app
...
const GAME_NAME_SHORT = "BM Wukong"; //Try for 8-10 characters
const EPIC_CODE_NAME = "b1"; //Folder in root
...
//UE specific
const ENGINE_VERSION = "5.0.0.0"; //Unreal Engine version. usually '4.27.2.0' or '5.X.X.0'. Written to UE4SS-settings.ini if writeEngineVersion is enabled. From PCGamingWiki build= (not community-verified); inert since writeEngineVersion is off
...
const UE4SS_PAGE_NO = 0; //19 - unset due to lack of updates - set these if there is a customized UE4SS Nexus page
const UE4SS_FILE_NO = 400;
...
const SAVE_LOC = "Game Folder"; //string for notification text. This game's saves sit inside the install folder, not Local AppData
const CONFIGMOD_LOCATION = LOCALAPPDATA; //almost always matches. Some are in game folder or Documents.
const SAVEMOD_LOCATION = CONFIGMOD_LOCATION; //not the real save location (see setup() override) - kept equal to CONFIGMOD_LOCATION so configSaveMatch stays true and the partition check runs against a real path instead of the game-folder-relative one
...
const LEGACY_UE5_SORTABLE_ID = "ue5-sortable-modtype"; //bare pre-namespacing id this game's 0.3.2 release also carried - kept so already-installed mods stay visible
...
const LOGICMODS_PATH = path.join(EPIC_CODE_NAME, "Content", "Paks", "LogicMods"); //already ends in the folder itself for this game (kept from the 0.3.2 extension) - the 4 sites that would otherwise double-append LOGICMODS_FOLDER drop the second path.join arg
...
const SIGBYPASS_LUA = "sig.lua"; //this game's own SigBypass mod, not the universal tool
const SIGBYPASS_PAGE_NO = 133; //this game's own Nexus page, not the universal site/1416 page
const SIGBYPASS_FILE_NO = 565;
...
if (ue4ssLoadOrder) {
  ...
  MODTYPE_FOLDERS.push(LOGICMODS_PATH); //LOGICMODS_PATH already ends in the folder for this game
}
if (hasModKit) {
...
async function deserializeLogicMods(api) {
  ...
  GAME_PATH = getDiscoveryPath(api);
  const logicModsFolder = path.join(GAME_PATH, LOGICMODS_PATH); //LOGICMODS_PATH already ends in the folder for this game
...
async function setup(discovery, api, gameSpec) {
  ...
  SAVE_PATH = getUserIdFolder(path.join(GAME_PATH, EPIC_CODE_NAME, "Saved", "SaveGames"));
...
function applyGame(context, gameSpec) {
  ...
  //Legacy pre-namespacing pak modType - kept registered only so a straggler still deploys
  //correctly in the brief window before migrateLegacyUe5SortableType() retags it
  context.registerModType(
  ...
  if (isMulti) {
    ...
    return React.createElement(
      ...
      menuItem(`Open LogicMods Folder (${n})`, () => {
        util.opn(path.join(GAME_PATH, LOGICMODS_PATH)).catch(() => null); //LOGICMODS_PATH already ends in the folder for this game
        onClose();
      }),
      React.createElement("div", { style: sepStyle }),
      menuItem(`Disable Selected (${n})`, () => {
