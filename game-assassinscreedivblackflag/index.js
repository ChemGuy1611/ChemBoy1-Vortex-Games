...
const UPLAYAPP_ID = "273";
const STEAMAPP_ID = "242050";
const GAME_ID = "assassinscreedivblackflag";
        ...
        },
        {
          ...
          action: (dismiss) => {
            api.showDialog(
              ...
              {
                text:
                  `For some legacy texture mods, you must use ${MOD_NAME} to inject textures into memory at runtime.\n` +
...
async function downloadResoRep32Bit(discovery, api, gameSpec) {
  ...
  try {
    ...
    const nxmUrl = `nxm://${GAME_DOMAIN}/mods/${PAGE_ID}/files/${FILE_ID}`;
    const dlId = await util.toPromise((cb) =>
