...
const GAME_ID = "theouterworlds"; //same as Nexus domain
const STEAMAPP_ID = "578650"; //Classic - from steamdb.info
const STEAMAPP_ID_SPACERS = "1920490"; //Spacer's Choice Edition - from steamdb.info
const STEAMAPP_ID_DEMO = null; //no demo
const EPICAPP_ID = "Rosemallow"; //Classic - from egdata.app
const EPICAPP_ID_SPACERS = "cb3bf7ba89574a66ae3b795e039d4dbc"; //Spacer's Choice Edition - from egdata.app
const GOGAPP_ID = "1242541569"; //Classic - from gogdb.org
const GOGAPP_ID_SPACERS = "1986509485"; //Spacer's Choice Edition - from gogdb.org
const XBOXAPP_ID = "PrivateDivision.TheOuterWorldsWindows10"; //Classic - from appxmanifest.xml
const XBOXAPP_ID_SPACERS = "PrivateDivision.64850D0BF098F"; //Spacer's Choice Edition - from appxmanifest.xml
const XBOXEXECNAME = "App"; //Classic - from appxmanifest.xml
const XBOXEXECNAME_SPACERS = "AppTheOuterWorldsShipping"; //Spacer's Choice Edition - from appxmanifest.xml
const XBOX_PUB_ID = "hv3d7yfbgr2rp"; //get from Save folder. '8wekyb3d8bbwe' if published by Microsoft
const DISCOVERY_IDS_ACTIVE = [
  STEAMAPP_ID,
  STEAMAPP_ID_SPACERS,
  EPICAPP_ID,
  EPICAPP_ID_SPACERS,
  ...
  XBOXAPP_ID,
  XBOXAPP_ID_SPACERS,
]; // UPDATE THIS WITH ALL VALID IDs

const GAME_NAME = "The Outer Worlds";
const GAME_NAME_SHORT = "The Outer Worlds"; //Try for 8-10 characters
const EPIC_CODE_NAME = "Indiana"; //Folder in root
const EXEC = "TheOuterWorlds.exe"; //Classic edition
const EXEC_SPACERS = "TheOuterWorldsSpacersChoiceEdition.exe"; //Spacer's Choice Edition - two full editions, not just per-store exe naming; see getExecutable()
...
//multiExe forced true (not derived from the EXEC_EPIC/EXEC_GOG/EXEC_DEMO formula below): the real
//variance here is EXEC vs EXEC_SPACERS (edition), which that formula can't see since those three
//constants only ever mirror EXEC.
let multiExe = true; //toggle for multiple executables (Epic/GOG/Demo don't match Steam)
...
const IO_STORE = false; //true if the Paks folder contains .ucas and .utoc files - old file's fileExt was ".pak" only
...
const CONFIG_FOLDERNAME = "WindowsNoEditor"; //UE 4 games are often 'WindowsNoEditor' - "Windows", "WindowsClient", "WindowsNoEditor"
...
const CONFIG_PATH_XBOX = path.join(CONFIGMOD_LOCATION, DATA_FOLDER, "Saved", "Config", "WinGDK"); //XBOX Version - Spacer's Choice Edition (matches shipped old file's CONFIG_PATH_XBOX_NEW)
//Classic Edition Xbox config is virtualized under a Packages folder instead - shipped old file's
//CONFIG_PATH_XBOX, confirmed working in production, so preserved verbatim rather than adopting the
//Spacer's Choice/template pattern above for this edition.
const CONFIG_PATH_XBOX_CLASSIC = path.join(
  LOCALAPPDATA,
  "Packages",
...
//Save support did not exist in the old file for either edition - both paths below are new (template
//default shape), never live-tested. Flag as unverified at the live-test gate.
const SAVE_PATH_XBOX = path.join(
  ...
  "SystemAppData",
  "wgs",
); //XBOX Version - Classic Edition
const SAVE_PATH_XBOX_SPACERS = path.join(
  LOCALAPPDATA,
  "Packages",
  `${XBOXAPP_ID_SPACERS}_${XBOX_SAVE_STRING}`,
  "SystemAppData",
  "wgs",
); //XBOX Version - Spacer's Choice Edition
...
let USERID_FOLDER = "";
let STORE_FOLDER = "";
let GAME_EDITION = "classic"; //"classic" or "spacers" - which of the two shipped editions was detected on disk, set by getExecutable()
let EPIC_LAUNCH_ID = EPICAPP_ID; //edition-dependent Epic app id, set by getExecutable(), read by requiresLauncher()
let XBOX_LAUNCH_ID = XBOXAPP_ID; //edition-dependent Xbox app id, set by getExecutable(), read by requiresLauncher()
let XBOX_LAUNCH_NAME = XBOXEXECNAME; //edition-dependent Xbox appExecName, set by getExecutable(), read by requiresLauncher()
...
//XBOX_LAUNCH_ID/XBOX_LAUNCH_NAME/EPIC_LAUNCH_ID are edition-dependent - set by getExecutable(), which
//Vortex always calls before requiresLauncher() during discovery/launch.
async function requiresLauncher(gamePath, store) {
  ...
  if (store === "xbox" && DISCOVERY_IDS_ACTIVE.includes(XBOX_LAUNCH_ID)) {
    return Promise.resolve({
      launcher: "xbox",
      addInfo: {
  ...
  //*
  if (store === "epic" && DISCOVERY_IDS_ACTIVE.includes(EPIC_LAUNCH_ID)) {
    return Promise.resolve({
      launcher: "epic",
      addInfo: {
...
//Get correct executable for game version - two full editions ship under one Nexus page (Classic and
//Spacer's Choice), each with their own store ids and exe name, cutting across every store. Xbox always
//launches through EXEC_XBOX regardless of edition, so edition there is told apart by checking whether
//the Classic root exe is ALSO present alongside it - shipped old file's proven detection, preserved
//verbatim. EXEC_EPIC/EXEC_GOG/EXEC_DEMO are not checked separately since they always equal EXEC for
//this game (no per-store exe variance on top of the per-edition one).
function getExecutable(discoveryPath) {
  if (statCheckSync(discoveryPath, EXEC_XBOX)) {
    ...
    if (statCheckSync(discoveryPath, EXEC)) {
      //Classic Edition
      GAME_EDITION = "classic";
      CONFIG_PATH = CONFIG_PATH_XBOX_CLASSIC;
      ...
      GAME_EDITION = "spacers";
      CONFIG_PATH = CONFIG_PATH_XBOX;
      EPIC_LAUNCH_ID = EPICAPP_ID_SPACERS;
      XBOX_LAUNCH_ID = XBOXAPP_ID_SPACERS;
      XBOX_LAUNCH_NAME = XBOXEXECNAME_SPACERS;
      SAVE_PATH = getUserIdFolder(SAVE_PATH_XBOX_SPACERS);
    }
    return EXEC_XBOX;
  }
  if (statCheckSync(discoveryPath, EXEC_SPACERS)) {
    //Spacer's Choice Edition
    GAME_VERSION = "steam";
    GAME_EDITION = "spacers";
  ...
  //Classic Edition - also the default fallback
  GAME_VERSION = "steam";
  GAME_EDITION = "classic";
  BINARIES_PATH = path.join(EPIC_CODE_NAME, "Binaries", EXEC_FOLDER_DEFAULT);
  SCRIPTS_PATH = path.join(BINARIES_PATH, UE4SS_MOD_PATH);
...
//Get correct game version - async. EXEC_EPIC/EXEC_GOG/EXEC_DEMO not checked separately - see getExecutable().
async function setGameVersionAsync(gamePath) {
  ...
  if (await statCheckAsync(gamePath, EXEC_SPACERS)) {
    GAME_VERSION = "steam";
    return GAME_VERSION;
  } //*/
}

//Get correct game version - synchronous. EXEC_EPIC/EXEC_GOG/EXEC_DEMO not checked separately - see getExecutable().
function setGameVersionSync(gamePath) {
  ...
  if (statCheckSync(gamePath, EXEC_SPACERS)) {
    GAME_VERSION = "steam";
    return GAME_VERSION;
  } //*/
}
...
async function resolveGameVersion(gamePath, exePath) {
  ...
  //No per-platform "-Shipping.exe" naming is confirmed for this game (old file never used one) - read
  //the root launcher exe directly instead, matching the shipped old file's proven approach.
  const READ_FILE = path.join(gamePath, GAME_EDITION === "spacers" ? EXEC_SPACERS : EXEC);
  let version = "0.0.0";
  if (GAME_VERSION === "xbox") {
...
function applyGame(context, gameSpec) {
  ...
  const game = {
    ...
    supportedTools: [
      ...
      {
        //Spacer's Choice Edition - old file shipped this live/uncommented alongside the Classic tool
        //above (unlike trepang2's single-edition model), since either exe can be present on disk.
        id: `${GAME_ID}-customlaunchspacers`,
        name: `Custom Launch`,
        logo: `exec.png`,
        executable: () => EXEC_SPACERS,
        requiredFiles: [EXEC_SPACERS],
        detach: true,
        relative: true,
        exclusive: true,
        shell: true,
