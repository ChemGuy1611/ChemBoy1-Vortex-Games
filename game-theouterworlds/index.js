...
function getExecutable(discoveryPath) {
  if (statCheckSync(discoveryPath, EXEC_XBOX)) {
    ...
    if (statCheckSync(discoveryPath, EXEC_CLASSIC)) {
      //Classic
  ...
  if (statCheckSync(discoveryPath, EXEC_NEW)) {
    //Spacer's Choice
...
async function resolveGameVersion(gamePath, exePath) {
  ...
  if (GAME_VERSION === "classic") {
    //use exe (note that this only returns the UE engine version right now)
...
async function setup(discovery, api, gameSpec) {
  ...
  await vfs.ensureDirWritableAsync(path.join(discovery.path, PAK_PATH));
