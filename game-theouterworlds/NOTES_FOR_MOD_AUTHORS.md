# Notes for Mod Authors - The Outer Worlds

Packaging rules for The Outer Worlds mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `Indiana` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `Indiana\Content\Paks\LogicMods` |
| Pak Mods | a `.pak` file | `Indiana\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `Indiana\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `Indiana\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `Indiana\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `Indiana`, `Engine` or `Content` | the game folder itself (no subfolder) |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | - |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `Indiana\Binaries\Win64` |
...
Installs to: `Indiana\Content\Paks\LogicMods`
...
Installs to: `Indiana\Content\Paks\~mods`
...
Installs to: `Indiana\Binaries\Win64`
...
Installs to: `Indiana\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `Indiana`, `Engine`, `Content`, `Binaries`, `Mods`, `Paks` or `Movies`.
