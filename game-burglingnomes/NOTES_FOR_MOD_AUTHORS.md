# Notes for Mod Authors - Burglin' Gnomes

Packaging rules for Burglin' Gnomes mods, so Vortex installs them to the right place.
...
| Root / Game Folder Mods | a `Gnomium_Data` folder | the game folder itself (no subfolder) |
...
| Asset Replacement Mods | a `.assets` file | `Gnomium_Data` |
...
- Recognised by a folder named `Gnomium_Data` or `Gnomium_Data` in the archive.
