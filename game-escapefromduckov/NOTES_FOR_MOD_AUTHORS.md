# Notes for Mod Authors - Escape from Duckov

Packaging rules for Escape from Duckov mods, so Vortex installs them to the right place.
...
| Root / Game Folder Mods | a `Duckov_Data` folder | the game folder itself (no subfolder) |
...
| Asset Replacement Mods | a `.assets` file | `Duckov_Data` |
...
- Recognised by a folder named `Duckov_Data` or `Duckov_Data` in the archive.
