    ...
    actions: [
      {
        ...
        action: (dismiss) => {
          api.showDialog(
            ...
            {
              text:
                "The extension has been updated to completely remove Fluffy Mod Manager.  Please re-install your full mod list.\n" +
...
async function setup(discovery, api, gameSpec) {
  ...
  try {
    ...
    try {
      ...
      log(
        "error",
        `Could not copy ${OO2_DLL} to ${PSARCTOOL_PATH} and ${PSARCTOOL_EXT_PATH}: ${err}`,
      );
    }
  }
