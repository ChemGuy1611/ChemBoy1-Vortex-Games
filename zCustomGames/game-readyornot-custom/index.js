/*
Name: Ready Or Not Vortex Extension
Structure: UE5
Author: ChemBoy1
Version: 0.1.4
Date: 08/01/2024
...
const EPIC_CODE_NAME = "ReadyOrNot";
...
function testConfig(files, gameId) {
  ...
  const isConfig = files.some(
    (file) => path.basename(file).toLowerCase() === (CONFIG_FILE1 || CONFIG_FILE2),
  );
...
function testVoFolder(files, gameId) {
  const isMod = files.some((file) => path.basename(file) === VO_FOLDER);
  let supported = gameId === spec.game.id && isMod;
...
function installVoFolder(files) {
  const modFile = files.find(
    (file) => path.basename(file).toLowerCase() === VO_FOLDER.toLowerCase(),
  );
  const idx = modFile.indexOf(`${path.basename(modFile)}${path.sep}`);
  const rootPath = path.dirname(modFile);
...
//Installer test for Fluffy Mod Manager files
function testTocFolder(files, gameId) {
  const isMod = files.some((file) => path.basename(file) === TOC_FOLDER);
  let supported = gameId === spec.game.id && isMod;
...
function installTocFolder(files) {
  const modFile = files.find(
    (file) => path.basename(file).toLowerCase() === TOC_FOLDER.toLowerCase(),
  );
  const idx = modFile.indexOf(`${path.basename(modFile)}${path.sep}`);
  const rootPath = path.dirname(modFile);
...
function testVo(files, gameId) {
  // Make sure we're able to support this mod
  const isMod =
    files.find((file) => VO_EXT.includes(path.extname(file).toLowerCase())) !== undefined;
  let supported = gameId === spec.game.id && isMod;
...
function installVo(files) {
  const modFile = files.find((file) => VO_EXT.includes(path.extname(file).toLowerCase()));
  const idx = modFile.indexOf(path.basename(modFile));
  const rootPath = path.dirname(modFile);
...
function installUnrealMod(api, files, gameId) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    const installFiles =
      modFiles.length > 3 ? yield chooseFilesToInstall(api, modFiles, fileExt) : modFiles;
...
function applyGame(context, gameSpec) {
  ...
  context.registerInstaller(`${GAME_ID}-config`, 40, testConfig, installConfig);
  context.registerInstaller(`${GAME_ID}-save`, 45, testSave, installSave);
  context.registerInstaller(`${GAME_ID}-vofolder`, 50, testVoFolder, installVoFolder);
  context.registerInstaller(`${GAME_ID}-tocfolder`, 55, testTocFolder, installTocFolder);
  context.registerInstaller(`${GAME_ID}-vofiles`, 60, testVo, installVo);
}
