...
function testI3d(files, gameId) {
  const isMod =
    files.find((file) => I3D_EXTS.includes(path.extname(file).toLowerCase())) !== undefined;
...
async function resolveGameVersion(gamePath) {
  ...
  if (GAME_VERSION === "xbox") {
    ...
    try {
      ...
      version = exeVersion.getProductVersion(path.join(gamePath, EXEC_X64));
      return Promise.resolve(version);
    } catch (err) {
...
async function setup(discovery, api, gameSpec) {
  ...
  await vfs.ensureDirWritableAsync(path.join(discovery.path, I3D_PATH));
  await vfs.ensureDirWritableAsync(path.join(discovery.path, PDLC_PATH));
