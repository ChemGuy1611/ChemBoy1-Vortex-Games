# Notes for Mod Authors - StarRupture

Packaging rules for StarRupture mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `StarRupture` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `StarRupture\Content\Paks\LogicMods` |
| Pak Mods | a `.pak` file | `StarRupture\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `StarRupture\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `StarRupture\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `StarRupture\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `StarRupture`, `Engine` or `Content` | the game folder itself (no subfolder) |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | - |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `StarRupture\Binaries\Win64` |
...
Installs to: `StarRupture\Content\Paks\LogicMods`
...
Installs to: `StarRupture\Content\Paks\~mods`
...
Installs to: `StarRupture\Binaries\Win64`
...
Installs to: `StarRupture\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `StarRupture`, `Engine`, `Content`, `Binaries` or `Mods`.
