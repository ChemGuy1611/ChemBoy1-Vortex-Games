# Notes for Mod Authors - REANIMAL

Packaging rules for REANIMAL mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `Everholm` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `Everholm\Content\Paks\LogicMods` |
| Pak Mods | a `.pak` file | `Everholm\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `Everholm\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `Everholm\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `Everholm\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `Everholm`, `Engine` or `Content` | the game folder itself (no subfolder) |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | - |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `Everholm\Binaries\Win64` |
...
Installs to: `Everholm\Content\Paks\LogicMods`
...
Installs to: `Everholm\Content\Paks\~mods`
...
Installs to: `Everholm\Binaries\Win64`
...
Installs to: `Everholm\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `Everholm`, `Engine`, `Content`, `Binaries` or `Mods`.
