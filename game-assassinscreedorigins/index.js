...
const TEXTURE_ID = "assassinscreedorigins-textures";
const ATK_ID = "assassinscreedorigins-ATK";
const ATK_EXEC = 'anviltoolkit.exe';
const FORGER_ID = "assassinscreedorigins-forger";
const FORGER_EXEC = 'forger.exe';
const PATCH_ID = "assassinscreedorigins-forgerpatch";
...
const spec = {
  "game": {
    ...
    "executable": EXEC,
    "logo": "assassinscreedorigins.jpg",
    "mergeMods": true,
    "modPath": ".",
  ...
  "modTypes": [
    {
      "id": "assassinscreedorigins-binaries",
    ...
    {
      "id": "assassinscreedorigins-texturesgamefolder",
...
async function downloadForger(discovery, api, gameSpec) {
  ...
  if (!modLoaderInstalled) {
    //notification indicating install process
    const NOTIF_ID = 'assassinscreedorigins-forger-installing';
    api.sendNotification({
      id: NOTIF_ID,
...
function resorepNotify(api) {
  api.sendNotification({
    id: 'resorep-notification-assassinscreedorigins',
    type: 'warning',
    ...
    actions: [
      {
        ...
        action: (dismiss) => {
          api.showDialog('question', 'Action required', {
            text: 'Some of the most popular mods for AC Origins require a software called Resorep.\n'
...
function applyGame(context, gameSpec) {
  ...
  //context.registerInstaller('assassinscreedorigins-dds', 25, testSupportedContent, installContent);
  context.registerInstaller('assassinscreedorigins-forger', 25, forgerTestSupportedContent, forgerInstallContent);
