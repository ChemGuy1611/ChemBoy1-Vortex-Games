# Notes for Mod Authors - Lost Soul Aside

Packaging rules for Lost Soul Aside mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `Projectlsa` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `Projectlsa\Content\Paks\LogicMods\LogicMods` |
| Pak Mods | a `.pak` file | `Projectlsa\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `Projectlsa\Binaries\Win64` |
| Signature Bypass | `dsound.dll` and `sig.lua` | `Projectlsa\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `Projectlsa\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `Projectlsa\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `Projectlsa` | the game folder itself (no subfolder) |
| Contentfolder | a file or folder named `Content` | - |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | `CONFIGMOD_LOCATION\Saved Games\LostSoulAside\Saved\Config\WindowsNoEditor` |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `Projectlsa\Binaries\Win64` |
...
Installs to: `Projectlsa\Content\Paks\LogicMods\LogicMods`
...
Installs to: `Projectlsa\Content\Paks\~mods`
...
Installs to: `Projectlsa\Binaries\Win64`
...
- Recognised only when BOTH `dsound.dll` and `sig.lua` are present in the archive.
...
Installs to: `Projectlsa\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `Projectlsa`.
...
- Installed to `CONFIGMOD_LOCATION\Saved Games\LostSoulAside\Saved\Config\WindowsNoEditor`.

Installs to: `CONFIGMOD_LOCATION\Saved Games\LostSoulAside\Saved\Config\WindowsNoEditor`
