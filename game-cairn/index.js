...
const TS_COMMUNITY = "cairn"; //https://thunderstore.io/c/cairn/
...
const EXTENSION_URL = "https://www.nexusmods.com/site/mods/1674"; //Nexus link to this extension. Used for links
...
const VERSION_FILE = path.join("StreamingAssets", "Version.info"); // LIKELY to change - usually .txt or .info file, i.e. Version.info. app.info typically does NOT contain version number
let VERSION_FILE_PATH = path.join(DATA_FOLDER, VERSION_FILE);
const hasVersionFile = true; //set to true if there is a Version.info file that contains the game version number
...
const CONFIG_FOLDERNAME = "PLAYER"; //not consumed directly - CONFIG_PATH is hand-built above for this game's real nested layout
const SAVE_FOLDERNAME = "STORY"; //not consumed directly - SAVE_PATH_DEFAULT is hand-built above; kept non-"XXX" so the Open Save Folder action stays enabled
...
let recommendedLoader = ""; // bep/mel - If loaderChoice false, this determines downloaded loader. Otherwise shows as "(Recommended)" in selector. '' = no recommendation (matches this game's pre-port neutral dialog).
...
//This game keeps both under "Saved Games", not the template's default LocalLow - CONFIG_FOLDERNAME/
//SAVE_FOLDERNAME don't fit either (real paths need two extra segments, not one), so those stay
//unused template placeholders and the real segments are hardcoded below instead.
const CONFIG_FOLDER = path.join(USER_HOME, "Saved Games", DEV_REGSTRING, GAME_REGSTRING);
let USERID_FOLDER = "";
if (hasUserIdFolder) {
  try {
...
const CONFIG_PATH = path.join(CONFIG_FOLDER, USERID_FOLDER, "PERSISTENT", "PLAYER");
