# Notes for Mod Authors - Cairn

Packaging rules for Cairn mods, so Vortex installs them to the right place.
...
| Root / Game Folder Mods | a `Cairn_Data` folder | the game folder itself (no subfolder) |
...
| Asset Replacement Mods | a `.assets` file | `Cairn_Data` |
...
- Recognised by a folder named `Cairn_Data` or `Cairn_Data` in the archive.
