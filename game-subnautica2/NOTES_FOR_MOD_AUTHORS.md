# Notes for Mod Authors - Subnautica 2

Packaging rules for Subnautica 2 mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `Subnautica2` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `Subnautica2\Content\Paks\LogicMods` |
| Pak Mods | a `.pak` file | `Subnautica2\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `Subnautica2\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `Subnautica2\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `Subnautica2\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `Subnautica2`, `Engine` or `Content` | the game folder itself (no subfolder) |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | - |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `Subnautica2\Binaries\Win64` |
...
Installs to: `Subnautica2\Content\Paks\LogicMods`
...
Installs to: `Subnautica2\Content\Paks\~mods`
...
Installs to: `Subnautica2\Binaries\Win64`
...
Installs to: `Subnautica2\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `Subnautica2`, `Engine`, `Content`, `Binaries`, `Mods`, `Paks`, `FMOD`, `Macros`, `Movies`, `PerfTours`, `Smoketest`, `Splash` or `VideoTours`.
