...
function setUe4ssLoEnabled(gameId, value) {
  return { type: SET_UE4SS_LO_ENABLED, payload: { gameId, value } };
}
...
const SERVERPAKS_PATH_ALT = path.join(
  EPIC_CODE_NAME,
  "Builds",
  "WindowsServer",
  EPIC_CODE_NAME,
...
if (!ue4ssLoadOrder) {
  ...
  spec.modTypes = spec.modTypes.filter(
    (type) => ![UE4SS_ID, SCRIPTS_ID, DLL_ID, LOGICMODS_ID].includes(type.id),
  );
}
...
if (!ue4ssLoadOrder) {
  //LogicMods are blueprint paks loaded by UE4SS's BPModLoaderMod, so they need UE4SS
  specServer.modTypes = specServer.modTypes.filter(
    (type) => ![UE4SS_ID, SCRIPTS_ID, DLL_ID, LOGICMODS_ID].includes(type.id),
  );
}
...
function testUe4ssCombo(files, gameId) {
  ...
  let supported =
    (gameId === spec.game.id || gameId === specServer.game.id) && isContent && isBinaries;
  ...
  if (
    ...
    try {
      ...
      try {
        ...
        log(
          "info",
          `Successfully created enabled.txt for UE4SS combo mod folder: ${path.basename(modFolderPath)}`,
        );
      } catch {
        log(
          "error",
          `Could not create enabled.txt for UE4SS combo mod folder: ${path.basename(modFolderPath)}`,
        );
      }
    }
  }
...
async function chooseInstallDestination(api, files) {
  ...
  return api
    .showDialog(
      "question",
      t("Choose Destination for Pak Mod Files"),
      {
        text: t(
          "Choose the destination for the pak files.\n" +
            `\n` +
...
async function serializeLoadOrderServer(context, loadOrder) {
  if (mod_update_all_profile[GAME_ID_SERVER]) {
    notifyLoadOrderPaused(context.api, GAME_ID_SERVER);
    return;
  }
  const props = generatePropsServer(context, undefined);
  if (props === undefined) {
...
async function serializeUe4ss(api, loadOrder) {
  ...
  if (mod_update_all_profile[activeGameId]) {
    notifyLoadOrderPaused(api, activeGameId);
    return;
  }
  GAME_PATH = getDiscoveryPath(api, activeGameId);
  const profile = selectors.activeProfile(state);
...
function applyGame(context, gameSpec) {
  ...
  context.registerInstaller(UE4SSCOMBO_ID, 26, testUe4ssCombo, (files, workingDir, gameId) =>
    installUe4ssCombo(context.api, files, workingDir, gameId),
  ); //not gated on ue4ssLoadOrder - also handles mods with both Binaries and Content folders that are not for UE4SS
  if (ue4ssLoadOrder) {
    context.registerInstaller(LOGICMODS_ID, 27, testLogic, installLogic);
  }
  context.registerInstaller(UE5_SORTABLE_ID, 30, testPak, (files) =>
    installPak(context.api, files),
  ); //Pak installer
  if (ue4ssLoadOrder) {
    context.registerInstaller(UE4SS_ID, 31, testUe4ss, installUe4ss);
    context.registerInstaller(SCRIPTS_ID, 35, testScripts, (files, fileName, gameId) =>
      installScripts(context.api, files, fileName, gameId),
    );
    context.registerInstaller(DLL_ID, 37, testDll, (files, fileName, gameId) =>
      installDll(context.api, files, fileName, gameId),
    );
  }
  ...
  context.registerInstaller(BINARIES_ID, 49, testBinaries, (files, fileName, gameId) =>
    installBinaries(context.api, files, fileName, gameId),
  );
...
function applyGameServer(context, gameSpec) {
  ...
  //register mod installers
  context.registerInstaller(UE5_SORTABLE_ID_SERVER, 29, testPakServer, (files) =>
    installPakServer(context.api, files),
  ); //Pak installer - DEDICATED SERVER
...
function main(context) {
  ...
  if (ue4ssLoadOrder) {
    ...
    context.registerSettings(
      ...
      GameSettings,
      () => ({}),
      () => selectors.activeGameId(context.api.getState()) === GAME_ID_SERVER,
      150,
    );
  ...
  context.once(() => {
    ...
    api.onAsync("check-mods-version", (gameId, mods, forced) => {
      ...
      if (
        (gameId !== GAME_ID && gameId !== GAME_ID_SERVER) ||
        UE4SS_PAGE_NO !== 0 ||
        !ue4ssLoadOrder
      ) {
        return Promise.resolve();
    ...
    api.events.on("mod-update", (gameId, modId, fileId) => {
      if ([GAME_ID, GAME_ID_SERVER].includes(gameId)) {
        updateModIds[gameId].set(String(modId), {
          firstSeen: Date.now(),
    ...
    api.events.on("will-install-mod", (gameId, archiveId, modId) => {
      if (![GAME_ID, GAME_ID_SERVER].includes(gameId)) return;
      updating_mod[gameId] = Array.from(updateModIds[gameId].keys()).some(
...
async function didDeploy(api, profileId) {
  ...
  if (updateModIds[gameId].size > 0) {
    ...
    for (const [nexusId, { firstSeen, targetFileId }] of Array.from(updateModIds[gameId])) {
      ...
      if (landed) {
        ...
        log(
          "warn",
          `[${gameId}] Mod update tracking for Nexus mod ${nexusId} timed out without landing; releasing load order guard for it.`,
        );
        updateModIds[gameId].delete(nexusId);
      }
    }
  ...
  if (guardWasArmed && !mod_update_all_profile[gameId]) {
    try {
      const refreshedLO =
        gameId === GAME_ID_SERVER
          ? await deserializeLoadOrderServer({ api })
      ...
      log("warn", `[${gameId}] post-update load order refresh failed`, err);
    }
  }
  ...
  if (writeEngineVersion && isUe4ssInstalled(api, gameSpec)) {
    try {
      ...
      try {
        ...
        log(
          "info",
          `[${gameId}] Could not read engine version from shipping exe, falling back to ENGINE_VERSION constant: ${err.message}`,
        );
      }
      ...
      log(
        "info",
        `[${gameId}] Failed to read UE4SS Settings INI file and write Engine Version: ${err.message}`,
      );
    }
  }
...
function LoadOrderItemRenderer(props) {
  ...
  const serializeForActiveGame = React.useCallback(
    (newLO) => {
...
function GameSettings() {
  ...
  const onToggle = React.useCallback(
    (checked) => {
      ...
      reconcileEnabledTxt(api, activeGameId, !checked).catch((err) =>
