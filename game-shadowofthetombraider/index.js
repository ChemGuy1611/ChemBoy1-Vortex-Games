...
const GAME_NAME = "Shadow of the Tomb Raider";
const GAME_NAME_SHORT = "SOTTR";
const EXEC = "SOTTR.exe";
...
const MANAGER_ID = "shadowofthetombraider-manager";
const MANAGER_EXEC = "sottrmodmanager.exe";

const SK_ID = "shadowofthetombraider-sk";
const SK_NAME = "Special K Plugin";
...
const tools = [
  ...
  },
  {
    ...
    requiredFiles: [MANAGERUNIFIED_EXEC],
    relative: true,
    exclusive: true,
    detach: true,
    //shell: true,
...
//Check if mod injector is installed
function isSKInstalled(api, spec) {
  const state = api.getState();
  const mods = state.persistent.mods[spec.game.id] || {};
  return Object.keys(mods).some((id) => mods[id]?.type === SK_ID);
}
...
async function downloadSK(api, gameSpec) {
  let skInstalled = isSKInstalled(api, gameSpec);

  if (!skInstalled) {
    //notification indicating install process
    api.sendNotification({
      id: "shadowofthetombraider-sk-installing",
      message: "Installing Special K Plugin",
      type: "activity",
      noDismiss: true,
      allowSuppress: false,
    });
    ...
    try {
      ...
      const batched = [
        ...
        actions.setModType(gameSpec.game.id, modId, SK_ID), // Set the mod type
      ];
      util.batchDispatch(api.store, batched); // Will dispatch both actions.
      //Show the user the download page if the download, install process fails
    } catch (err) {
...
function setupNotify(api) {
  const NOTIF_ID = `setup-notification-shadowofthetombraider`;
  const MESSAGE = `Mod Installation and Setup Instructions`;
  api.sendNotification({
    id: NOTIF_ID,
    type: "warning",
    message: MESSAGE,
    ...
    actions: [
      {
        ...
        action: (dismiss) => {
          api.showDialog(
            ...
            {
              text:
                "The SOTTR Mod Manager tool downloaded by this extension requires setup.\n" +
                'Please launch the tool and enable your mod folders inside the folder: "[RootGameFolder]\\Mods".\n' +
                "It is recommended that you use SOTTR Mod Manager to enable/disable mods after install.\n" +
                "\n" +
                'For Special K Texture Mods, you need to select the mod type "Special K Texture Mod" after downloading.\n' +
                "The Special K Plugin is disabled after download by default. Please only enable this if you are using Special K mods.\n" +
                "If your game crashes on launch with the Special K Plugin enabled, just disable the mod and it should launch again.\n",
            },
            [
      ...
      },
      {
        ...
        action: (dismiss) => {
          api.showDialog(
            ...
            {
              text:
                `For some mods, you must use ${MOD_NAME} to deploy the mod to the game files after installing with Vortex.\n` +
