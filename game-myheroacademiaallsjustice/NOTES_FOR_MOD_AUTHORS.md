# Notes for Mod Authors - MY HERO ACADEMIA: All's Justice

Packaging rules for MY HERO ACADEMIA: All's Justice mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `HeroGame` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `HeroGame\Content\Paks\LogicMods` |
| Pak Mods | a `.pak` file | `HeroGame\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `HeroGame\Binaries\Win64` |
| Signature Bypass | `dsound.dll` and `UniversalSigBypasser.asi` | `HeroGame\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `HeroGame\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `HeroGame\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `HeroGame`, `Engine` or `Content` | the game folder itself (no subfolder) |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | - |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `HeroGame\Binaries\Win64` |
...
Installs to: `HeroGame\Content\Paks\LogicMods`
...
Installs to: `HeroGame\Binaries\Win64\ue4ss\Mods`
