...
const MSCLOADER_FILES_ARRAY = [
  //delete these files on purge to remove MSCLoader
  "winhttp.dll",
...
const INSTALLER_DELETE_FOLDERS = ["Plugins", "BepInEx"];
...
function installMscLoader(files, workingDir) {
  ...
  try {
    ...
    log(
      "error",
      `Failed to copy custom ${MSCLOADER_NAME} Installer files to ${workingDir} during install: ${err}`,
    );
  }
...
async function installPlugin(api, gameSpec, files, workingDir) {
    ...
    const wrongLoader = await api.showDialog(
      ...
      "Wrong Mod Loader",
      {
        bbcode: api.translate(
          `Vortex has detected that the ${MOD_NAME} archive has ${MSCLOADER_NAME} plugins, but you have installed BepInEx or MelonLoader.[br][/br][br][/br]` +
  ...
  if ((isBepinex || isMelon) && mscInstalled) {
    const wrongLoader = await api.showDialog(
      ...
      {
        bbcode: api.translate(
          `Vortex has detected that the ${MOD_NAME} archive has BepInEx/MelonLoader plugins, but you have installed MSCLoader.[br][/br][br][/br]` +
            `The installation will be cancelled to avoid issues.[br][/br][br][/br]` +
  ...
  if (isMsc) {
    ...
    const folder = files.find((file) =>
      MSCLOADER_MOD_FOLDERS.includes(path.basename(file).toLowerCase()),
    );
    if (folder !== undefined) {
...
async function chooseModLoader(api, gameSpec) {
  ...
  return api
    .showDialog(
      ...
      [{ label: t(MSC_LABEL) }, { label: t("BepInEx") }, { label: t("MelonLoader") }],
    )
    .then(async (result) => {
      if (result === undefined) {
        return;
...
async function removeMscFiles(api, gameSpec) {
  //run on purge too
  GAME_PATH = getDiscoveryPath(api);
...
async function setup(discovery, api, gameSpec) {
  ...
  if (
    ...
    (bepinexInstalled && mscInstalled) ||
    (melonInstalled && mscInstalled)
  ) {
...
function applyGame(context, gameSpec) {
  ...
  context.registerAction(
    ...
    "open-ext",
    {},
    'Delete "BepInEx"+"Plugins" Folders (Pre-MSCLInstaller)',
    async () => {
  ...
  context.registerAction(
    ...
    "open-ext",
    {},
    'Restore "BepInEx"+"Plugins" Folders (Post-MSCLInstaller)',
    async () => {
      GAME_PATH = getDiscoveryPath(context.api);
...
function main(context) {
  ...
  context.once(() => {
    ...
    context.api.onAsync("did-deploy", (profileId, deployment) => {
      ...
      if (mscInstalled) {
        //check for condition if MSC Installer mod is installed, but not installed to the game folder
