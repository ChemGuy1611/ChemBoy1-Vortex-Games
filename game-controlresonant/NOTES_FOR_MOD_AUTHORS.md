# Notes for Mod Authors - CONTROL Resonant

Packaging rules for CONTROL Resonant mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Root / Game Folder Mods | a `data` folder | the game folder itself (no subfolder) |
...
- Recognised by a folder named `data` or `data_pack2` in the archive.
