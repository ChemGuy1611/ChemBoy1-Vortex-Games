# Notes for Mod Authors - Dispatch

Packaging rules for Dispatch mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `Dispatch` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `Dispatch\Content\Paks\LogicMods` |
| Pak Mods | a `.pak` file | `Dispatch\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `Dispatch\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `Dispatch\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `Dispatch\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `Dispatch`, `Engine` or `Content` | the game folder itself (no subfolder) |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | - |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `Dispatch\Binaries\Win64` |
...
Installs to: `Dispatch\Content\Paks\LogicMods`
...
Installs to: `Dispatch\Content\Paks\~mods`
...
Installs to: `Dispatch\Binaries\Win64`
...
Installs to: `Dispatch\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `Dispatch`, `Engine`, `Content`, `Binaries` or `Mods`.
