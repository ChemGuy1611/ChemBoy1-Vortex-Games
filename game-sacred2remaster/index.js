...
function testRoot(files, gameId) {
  const isRoot = files.some((file) =>
    ROOT_FOLDERS_LOWER.includes(path.basename(file).toLowerCase()),
  );
  const isRootSub = files.some((file) =>
    ROOTSUB_FOLDERS_LOWER.includes(path.basename(file).toLowerCase()),
  );
  const isPakSub = files.some((file) =>
    PAKSUB_FOLDERS_LOWER.includes(path.basename(file).toLowerCase()),
  );
  const isTexSub = files.some((file) =>
    TEXTURESUB_FOLDERS_LOWER.includes(path.basename(file).toLowerCase()),
  );
  const isUiSub = files.some((file) =>
    UISUB_FOLDERS_LOWER.includes(path.basename(file).toLowerCase()),
  );
  let supported =
    gameId === spec.game.id && (isRoot || isRootSub || isPakSub || isTexSub || isUiSub);
...
function applyGame(context, gameSpec) {
  ...
  context.registerInstaller(`${GAME_ID}-fallback`, 40, testFallback, (files, destinationPath) =>
