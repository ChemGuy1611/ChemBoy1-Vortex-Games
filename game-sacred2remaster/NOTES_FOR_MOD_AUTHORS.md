# Notes for Mod Authors - Sacred 2 Remaster

Packaging rules for Sacred 2 Remaster mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Root / Game Folder Mods | a `scripts` folder | the game folder itself (no subfolder) |
...
- Recognised by a folder named `scripts`, `video`, `pak`, `locale`, `Textures` or `ui` in the archive.
