...
const BEPCFGMAN_REQUIREMENTS = [
  {
    ...
    //autoInstall is deliberately omitted - this extension installs ConfigurationManager unattended,
    //gated by the downloadCfgMan toggle, matching its existing behavior.
...
function testSkin(files, gameId) {
  ...
  const isExcluded = files.some(
    (file) =>
      path.extname(file).toLowerCase() === ".exe" || path.extname(file).toLowerCase() === ".dll",
