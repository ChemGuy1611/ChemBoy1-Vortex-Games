...
const REF_FILE_NO = 8000;

const FLUFFY_ID = `re4-fluffymodmanager`;
...
const spec = {
  ...
  discovery: {
    ids: [STEAMAPP_ID, STEAMAPP_ID_DEMO],
    names: [],
  },
};
...
function testUpscaler(files, gameId) {
  const isMod = files.some(
    (file) => path.basename(file).toLowerCase() === UPSCALER_FILE.toLowerCase(),
  );
    ...
    actions: [
      ...
      },
      {
        ...
        action: (dismiss) => {
          api.showDialog(
            "question",
            "Run Fluffy Mod Manager to Enable Mods",
            {
              text:
                `You must use ${MOD_NAME} to enable mods after installing with Vortex.\n` +
...
async function setup(discovery, api, gameSpec) {
  ...
  await vfs.ensureDirWritableAsync(path.join(GAME_PATH, UPSCALER_PATH));
  await vfs.ensureDirWritableAsync(path.join(GAME_PATH, PRESET_PATH));
  return vfs.ensureDirWritableAsync(path.join(GAME_PATH, MOD_PATH_USED));
}
