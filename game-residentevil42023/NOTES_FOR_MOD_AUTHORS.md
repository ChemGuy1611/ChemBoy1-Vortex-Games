# Notes for Mod Authors - Resident Evil 4 (2023) + Chainsaw Demo

Packaging rules for Resident Evil 4 (2023) + Chainsaw Demo mods, so Vortex installs them to the right place.
...
- Recognised by any file named `dxgi.dll`, `nvngx_dlss.dll` or `amd_fidelityfx_dx12.dll`.
...
Handled by the `testUpscaler` installer. Inspect the extension source for the exact archive layout it expects.

Installs to: `reframework\plugins`
