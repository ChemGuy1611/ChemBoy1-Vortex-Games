# Notes for Mod Authors - MISERY

Packaging rules for MISERY mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `MISERY` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `MISERY\Content\Paks\LogicMods` |
| Pak Mods | a `.pak` file | `MISERY\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `MISERY\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `MISERY\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `MISERY\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `MISERY`, `Engine` or `Content` | the game folder itself (no subfolder) |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | - |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `MISERY\Binaries\Win64` |
...
Installs to: `MISERY\Content\Paks\LogicMods`
...
Installs to: `MISERY\Content\Paks\~mods`
...
Installs to: `MISERY\Binaries\Win64`
...
Installs to: `MISERY\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `MISERY`, `Engine`, `Content`, `Binaries` or `Mods`.
