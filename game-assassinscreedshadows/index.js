...
const UPLAYAPP_ID = "8006"; //Ubisoft Connect App ID — from SOFTWARE\WOW6432Node\Ubisoft\Launcher\Installs\
const STEAMAPP_ID = "3159330"; //https://steamdb.info/app/XXX/
...
const hasReforger = true; //true if game uses ReForger (Xbox package, found through the registry)
...
const hasPatchTextures = true; //true if game takes loose .dds textures as Forger patches — mutually exclusive with hasResorep
const hasSound = false; //true if game takes .pck sound bank replacements
const hasFixes = false; //true if game has a community "fixes" DLL package — commented out, no mod uses it yet
...
const DLC_FOLDERS = ["dlc_10", "dlc_26", "dlc_28", "dlc_29"];
...
const LEGACY_MODTYPES = [
  {
    id: `${GAME_ID}-forgefiledlc10`,
    name: "Forge Replacement (dlc_10) (legacy)",
    targetPath: path.join("{gamePath}", "dlc_10"),
  },
  {
    id: `${GAME_ID}-forgefiledlc26`,
    name: "Forge Replacement (dlc_26) (legacy)",
    targetPath: path.join("{gamePath}", "dlc_26"),
  },
  {
    id: `${GAME_ID}-forgefiledlc28`,
    name: "Forge Replacement (dlc_28) (legacy)",
    targetPath: path.join("{gamePath}", "dlc_28"),
  },
  {
    id: `${GAME_ID}-forgefiledlc29`,
    name: "Forge Replacement (dlc_29) (legacy)",
    targetPath: path.join("{gamePath}", "dlc_29"),
  },
];
...
function setupNotify(api) {
  ...
  const MESSAGE = "ReForger Setup Required";
  api.sendNotification({
    id: NOTIF_ID,
    type: "warning",
    message: MESSAGE,
        ...
        action: (dismiss) => {
          api.showDialog(
            ...
            MESSAGE,
            {
              text:
                `\n` +
                `Some of the most popular mods for ${GAME_NAME_SHORT} require a tool called ReForger.\n` +
                `This tool has been automatically downloaded and run for you by the extension.\n` +
                `\n` +
                `You need to run ReForger on the notification or the Tools tab to apply Forger-format patches to the game files.\n`,
            },
            [
