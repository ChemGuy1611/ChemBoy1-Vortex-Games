# Notes for Mod Authors - Batman: Arkham Asylum

Packaging rules for Batman: Arkham Asylum mods, so Vortex installs them to the right place.
...
- Recognised by a folder named `BmGame`, `Engine`, `Binaries`, `Config`, `CookedPC`, `Splash`, `Localization` or `Movies` in the archive.
