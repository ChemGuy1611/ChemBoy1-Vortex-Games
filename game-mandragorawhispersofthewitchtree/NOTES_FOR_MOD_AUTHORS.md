# Notes for Mod Authors - Mandragora: Whispers of the Witch Tree

Packaging rules for Mandragora: Whispers of the Witch Tree mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `man` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `man\Content\Paks\LogicMods\LogicMods` |
| Pak Mods | a `.pak` file | `man\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `man\Binaries\Win64` |
| Signature Bypass | `d3d9.dll` and `bypasssignature.lua` | `man\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `man\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `man\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `man` | the game folder itself (no subfolder) |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | `LOCALAPPDATA\MandragoraWotWT\Saved\Config\WindowsNoEditor` |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `man\Binaries\Win64` |
...
Installs to: `man\Content\Paks\LogicMods\LogicMods`
...
Installs to: `man\Content\Paks\~mods`
...
Installs to: `man\Binaries\Win64`
...
- Recognised only when BOTH `d3d9.dll` and `bypasssignature.lua` are present in the archive.
...
Installs to: `man\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `man`.
...
- Installed to `LOCALAPPDATA\MandragoraWotWT\Saved\Config\WindowsNoEditor`.

Installs to: `LOCALAPPDATA\MandragoraWotWT\Saved\Config\WindowsNoEditor`
