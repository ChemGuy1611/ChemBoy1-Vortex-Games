...
async function resolveFcModdingDownloadUrl(requirement) {
  try {
    ...
    log(
      "warn",
      `Could not resolve the ${requirement.userFacingName} download URL on fcmodding.com: ${err}`,
    );
    return null;
  }
}
...
async function getFcModdingPageVersion(requirement) {
  try {
    ...
    log(
      "warn",
      `Could not read the ${requirement.userFacingName} version from ${pageUrl(requirement)}: ${err}`,
    );
    return null;
  }
}
