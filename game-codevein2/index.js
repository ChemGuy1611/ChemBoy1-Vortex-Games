...
const GAME_ID = "codevein2"; //same as Nexus domain
const STEAMAPP_ID = "2362060"; //from steamdb.info
const STEAMAPP_ID_DEMO = "3733960"; //VERIFY if the EPIC_CODE_NAME and EXEC_DEMO match Steam full game
...
const GAME_NAME = "CODE VEIN II";
const GAME_NAME_SHORT = "CODE VEIN II"; //Try for 8-10 characters
const EPIC_CODE_NAME = "CodeVein2"; //Folder in root
...
const PCGAMINGWIKI_URL = "https://www.pcgamingwiki.com/wiki/Code_Vein_II";
const EXTENSION_URL = "https://www.nexusmods.com/site/mods/1646"; //Nexus link to this extension. Used for links
...
function installRhff(files) {
  const modFile = files.find(
    (file) => path.basename(file).toLowerCase() === RHFF_DLL.toLowerCase(),
  );
  const idx = modFile.indexOf(path.basename(modFile));
  const rootPath = path.dirname(modFile);
  const rootPrefix = rootPath === "." ? "" : rootPath + path.sep;
