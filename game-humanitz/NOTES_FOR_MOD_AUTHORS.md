# Notes for Mod Authors - HumanitZ

Packaging rules for HumanitZ mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `Humanitz` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `Humanitz\Content\Paks\LogicMods` |
| Pak Mods | a `.pak` file | `Humanitz\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `Humanitz\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `Humanitz\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `Humanitz\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `Humanitz`, `Engine` or `Content` | the game folder itself (no subfolder) |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | - |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `Humanitz\Binaries\Win64` |
...
Installs to: `Humanitz\Content\Paks\LogicMods`
...
Installs to: `Humanitz\Content\Paks\~mods`
...
Installs to: `Humanitz\Binaries\Win64`
...
Installs to: `Humanitz\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `Humanitz`, `Engine`, `Content`, `Binaries` or `Mods`.
