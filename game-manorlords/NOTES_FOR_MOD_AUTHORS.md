# Notes for Mod Authors - Manor Lords

Packaging rules for Manor Lords mods, so Vortex installs them to the right place.
...
Recognised when the archive contains a file or folder named `ae_bp` and a file with the `.pak` extension.
