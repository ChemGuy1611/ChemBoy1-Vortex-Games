...
const GAME_ID = "manorlords"; //same as Nexus domain
const STEAMAPP_ID = "1363080"; //from steamdb.info
const STEAMAPP_ID_DEMO = null; //no demo
const EPICAPP_ID = "8a5e1dedaa0e4f4e8b2ac2c00e849111"; //from egdata.app
const GOGAPP_ID = "1361243432"; // from gogdb.org
const XBOXAPP_ID = "HoodedHorse.ManorLords"; //from appxmanifest.xml
const XBOXEXECNAME = "AppManorLordsShipping"; //from appxmanifest.xml
const XBOX_PUB_ID = "znaey1dw2bdpr"; //get from Save folder. Same publisher string as game-menace (both Hooded Horse) - VERIFY at live test
...
const GAME_NAME_SHORT = "Manor Lords"; //Try for 8-10 characters
const EPIC_CODE_NAME = "ManorLords"; //Folder in root
...
const UE4SS_PAGE_NO = 229; //set these if there is a customized UE4SS Nexus page
...
//repackage mods where LogicMods pak is in UE4SS mods folder (game-specific - no template equivalent)
...
//Test for MLUE4SS combo mod files (game-specific - no template equivalent)
...
function installMlUe4ss(files) {
  ...
  const nonPakFiles = filtered.filter(
    (file) => !(path.extname(file).toLowerCase() === MLUE4SS_EXT),
  );
  const instructions = nonPakFiles
    .map((file) => ({ type: "copy", source: file, destination: path.join(SCRIPTS_PATH, file) }))
    .concat(
      pakFiles.map((file) => ({
        type: "copy",
        source: file,
        destination: path.join(LOGICMODS_PATH, LOGICMODS_FOLDER, path.basename(file)),
      })),
    );
  instructions.push(setModTypeInstruction);
  return Promise.resolve({ instructions });
...
//pak installer's priority (UE5_SORTABLE_ID = 29): MLUE4SS (20, always registered), Mod Kit (25,
//only when hasModKit), UE4SS Combo (26, always registered) and LogicMods (27, only when
//logicModsLoadOrder). An installer at or below priority 29 never gets a turn once a pak-type file
//is present, since testPak's own check is just "has a pak-type file" with no exclusivity - so
//lower-priority markers can't have "actually" claimed the archive and are not checked here. Used
//by retagFomodPakMod() below to work out, after the fact, what a FOMOD-installed mod would have
//been classified as had FOMOD not intercepted it first. Pass paths relative to the staging folder
//when calling this on a staged tree: an absolute path drags the staging root's own segments in,
//and a staging folder living under the game's root folder name would then satisfy that check for
//every mod.
function beatsPakInstaller(files) {
  const segsLower = pathSegments(files).map((seg) => seg.toLowerCase());
  const hasMlUe4ssFolder = segsLower.includes(MLUE4SS_FOLDER.toLowerCase());
  const hasMlUe4ssExt = files.some((file) => path.extname(file).toLowerCase() === MLUE4SS_EXT);
  if (hasMlUe4ssFolder && hasMlUe4ssExt) return true; //MLUE4SS (20)
  if (hasModKit) {
...
function applyGame(context, gameSpec) {
  ...
  const game = {
    ...
    supportedTools: [
      ...
      {
        //preserved from pre-FBLO extension - dual launch buttons for default vs Xbox exe
  ...
  //MLUE4SS_ID is a pre-existing game-specific installer with no template equivalent, preserved at its
  //own priority 20 (below the pak installer's 29, so it still runs first for its LogicMods-in-UE4SS-
  //folder repackage case). Everything else is at template spacing.
