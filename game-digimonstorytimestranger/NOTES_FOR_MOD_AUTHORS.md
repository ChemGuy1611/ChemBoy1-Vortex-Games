# Notes for Mod Authors - Digimon Story Time Stranger

Packaging rules for Digimon Story Time Stranger mods, so Vortex installs them to the right place.
...
| Reloadedmodloader | a file or folder named `modconfig.json` and a file or folder named `XXX.dll` | `Reloaded\Mods\` |
...
Recognised when the archive contains a file or folder named `modconfig.json` and a file or folder named `XXX.dll`.
