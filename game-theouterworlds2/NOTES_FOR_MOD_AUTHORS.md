# Notes for Mod Authors - The Outer Worlds 2

Packaging rules for The Outer Worlds 2 mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `Arkansas` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `Arkansas\Content\Paks\LogicMods\LogicMods` |
| Pak Mods | a `.pak` file | `Arkansas\Content\Paks\~mods` |
...
| Root / Game Folder Mods | a top-level folder such as `Arkansas` | the game folder itself (no subfolder) |
| Contentfolder | a file or folder named `Content` | `Arkansas` |
...
Installs to: `Arkansas\Content\Paks\LogicMods\LogicMods`
...
Installs to: `Arkansas\Content\Paks\~mods`
...
- Recognised by a top-level folder matching any of: `Arkansas`.
