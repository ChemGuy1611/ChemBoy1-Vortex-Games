# Notes for Mod Authors - The Long Dark

Packaging rules for The Long Dark mods, so Vortex installs them to the right place.
...
| Root / Game Folder Mods | a `tld_Data` folder | the game folder itself (no subfolder) |
...
| Asset Replacement Mods | a `.assets` file | `tld_Data` |
...
- Recognised by a folder named `tld_Data` or `tld_Data` in the archive.
