# Notes for Mod Authors - BioShock Infinite

Packaging rules for BioShock Infinite mods, so Vortex installs them to the right place.
...
| Root / Game Folder Mods | a `XGame` folder | the game folder itself (no subfolder) |
| Cooked Content Mods | a `.tfc` file | `XGame\CookedPCConsole_FR` |
| Movie / Cutscene Replacements | a `.bik` file | `XGame\Movies` |
| Binaries / Injector Mods | a `BioShockInfinite.exe` file or a `.dll` file | `Binaries\Win32` |
...
- Recognised by a folder named `XGame`, `DLC`, `Engine`, `Binaries`, `Config`, `Content`, `CookedPCConsole_FR`, `Localization` or `Movies` in the archive.
...
- Recognised by any file with the `.tfc`, `.xxx` or `.ucs` extensions.

Installs to: `XGame\CookedPCConsole_FR`
...
- Recognised by any file named `BioShockInfinite.exe`.
