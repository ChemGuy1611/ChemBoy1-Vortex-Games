...
| `utility_functions`                 | Inserts standard utility functions (`isDir`, `statCheckSync`, `statCheckAsync`, `getAllFiles`, `getDiscoveryPath`, `purge`, `deploy`) before `function modTypePriority` for any extension missing them. The injected bodies are fs-convention aware: they are written in the migrated idiom (native `fs`, `fsp` for promises) and `_fs_idiom()` rewrites the async calls back to `fs.statAsync` / `fs.readdirAsync` when the target file has not been migrated off the vortex-api `fs` wrapper yet, or to `fs.promises.` when it is migrated but has no `fsp` binding. `fs.statSync` needs no adaptation - both surfaces export it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
