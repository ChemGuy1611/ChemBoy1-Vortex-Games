...
| `is_multi_game_extension(src)`                                                                      | Return `True` if `src` calls `context.registerGame` more than once (comments stripped), i.e. bundles 2+ distinct games/variants behind one Nexus page                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `has_unity_bepinex_parity(src, folder)`                                                             | `has_template_shape_parity` against `template-unitybepinex`, with `UNITY_PARITY_KNOWN_EXCEPTIONS` and `isXna` exempted both ways                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `has_unity_hybrid_parity(src, folder)`                                                              | Same as `has_unity_bepinex_parity` but against `template-unitymelonloaderbepinex-hybrid`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `has_anvil_template_parity(src, folder)`                                                            | `has_template_shape_parity` against `template-anvilengine`, for extensions `detect_engine` reports as `Anvil`. No toggle exemptions                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `has_farcry_template_parity(src, folder)`                                                           | `has_template_shape_parity` against `template-farcry`, for extensions `detect_engine` reports as `Dunia`. No toggle exemptions                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `ANVIL_PARITY_KNOWN_EXCEPTIONS`                                                                     | Same, for Anvil games. Empty today - every Anvil divergence from `template-anvilengine` is a pending port                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `FARCRY_PARITY_KNOWN_EXCEPTIONS`                                                                    | Same, for Far Cry games. Empty today - every Far Cry divergence from `template-farcry` is a pending port                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
- Path CONSTRUCTION (which base folder, how many subfolder segments, whether config/save share one folder) differs structurally across the 16 templates — `template-basic`'s `CONFIGMOD_LOCATION`/`APPDATA_FOLDER`/`CONFIG_FOLDERNAME` split doesn't exist on most other templates, and a few (`rpgmaker`) link `SAVE_FOLDERNAME = CONFIG_FOLDERNAME`.
...
| `resources/lists/games-unity-bepinex-parity.txt`    | Unity+BepInEx games with every function/toggle `template-unitybepinex` HEAD has (extras don't count against it; see `audit_parity.py` below)                                                                      |
...
| `resources/lists/games-multi-game.txt`              | Extensions that call `context.registerGame` more than once, i.e. bundle several distinct games/variants behind one Nexus page                                                                                     |
...
Mechanical layer for the template-parity audit: UE4-5 load-order parity plus Unity BepInEx/hybrid, Anvil and Far Cry template parity. Regenerates the parity-related `resources/lists/` flag lists via `categorize_games.py`, then reports on games not yet at parity and a couple of invariants a stale or partial port can silently break.
...
**Unity BepInEx / hybrid:** for each family, reports `at_parity`/`total` and, for every game not at parity, prints its missing function/toggle counts against its own template's HEAD — the same measurement the `unity-loader-downloader-migration` plan tracked by hand, wave over wave. Extra functions/toggles a game has beyond its template are reported as informational only, never as a parity failure — a game's own unique code (e.g. `mousepiforhire`'s BepLoader patch, `romestead`'s ModSettingsMenu) is expected, not drift. `isXna` is exempted from the toggle diff in both directions (it selects an engine variant, not an optional subsystem). `game-menace` and `game-mywintercar` are carved out permanently by explicit user decision — see `UNITY_PARITY_KNOWN_EXCEPTIONS` in `vortex_utils.py`.

**Anvil / Far Cry:** same measurement again, against `template-anvilengine` and `template-farcry`. Neither family has a toggle exemption — every boolean in those two templates gates an optional subsystem, so a missing one is always a pending port rather than a deliberate variant. Both started at zero: no Anvil extension carried a single boolean feature toggle before the template gained its EDIT ZONE, and the Far Cry games carry one of the template's five. `ghostreconbreakpoint`'s buildtable installers are deliberately outside the template and show up as extras, which is expected.
...
All four are regenerated at the end of every run, for the games that released successfully: `EXTENSION_EXPLAINED.md` via `generate_explained.js`, the two `NOTES_FOR_MOD_AUTHORS` files via `generate_notes.js`, and `DESCRIPTION.bbcode` — the Nexus mod page description — via `generate_notes.js --description`. The first three are overwritten outright; for the description, only the install-notes list is rewritten and the rest of the page stays as the author wrote it.
...
Inserted code matches the quote style the file already uses for `require()`. The repo is oxfmt-formatted to double quotes, so `const fs = require("fs");` is what lands in an extension; a file oxfmt does not touch (`resources/snippets.js`) keeps whatever it already uses. Without this the inserted lines were rewritten by the next `npm run format`.

Call-site detection allows whitespace around the dot, so a fluent line break — `return fs\n  .statAsync(...)` — is matched and rewritten like any other call. (An earlier version's `fs\.` regex missed this form: the call was left as `fs.statAsync` while `fs` was rebound to native, i.e. a runtime `TypeError`. `node --check` passes such a file; only `npx eslint` catches it, and only when the file also drops its native binding.)

Runs a **cleanup pass** on a file that already carries the migration markers (`fs: vfs` in the destructure, or a native `const fs = require('fs')`) but still has migratable `fs.*` calls — an earlier run missed a site. It rewrites the residual call sites and leaves the already-rebound bindings alone, rather than skipping the file.
