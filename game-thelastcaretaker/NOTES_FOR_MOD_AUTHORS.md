# Notes for Mod Authors - The Last Caretaker

Packaging rules for The Last Caretaker mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `Voyage` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `Voyage\Content\Paks\LogicMods` |
| Pak Mods | a `.pak` file | `Voyage\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `Voyage\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `Voyage\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `Voyage\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `Voyage`, `Engine` or `Content` | the game folder itself (no subfolder) |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | - |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `Voyage\Binaries\Win64` |
...
Installs to: `Voyage\Content\Paks\LogicMods`
...
Installs to: `Voyage\Content\Paks\~mods`
...
Installs to: `Voyage\Binaries\Win64`
...
Installs to: `Voyage\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `Voyage`, `Engine`, `Content`, `Binaries` or `Mods`.
