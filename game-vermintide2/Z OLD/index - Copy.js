  ...
  "modTypes": [
    {
      ...
      "name": "binaries",
      "priority": "high",
      "targetPath": "C:\\Program Files (x86)\\Steam\\steamapps\\common\\Warhammer Vermintide 2\\binaries"
    },
    {
      "id": "vermintide2-binariesdx12",
      "name": "binaries_dx12",
      "priority": "high",
      "targetPath": "C:\\Program Files (x86)\\Steam\\steamapps\\common\\Warhammer Vermintide 2\\binaries_dx12"
    }
  ],
  "discovery": {
    "ids": [
...
*/
function setup(api) {
  const reshade = () => api.sendNotification({
          ...
          action: () => {
            api.showDialog('question', 'Action required', {
              text: 'Vermintide 2 requires Reshades for most mods available on Nexus Mods.\n'
                  ...
                  + 'Install Fluffy MM as a mod in the default location.'
            }, [
              { label: 'Continue', action: (dismiss) => dismiss() },
              { label: 'Download Reshade', action: (dismiss) => {
