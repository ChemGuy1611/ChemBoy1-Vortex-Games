# Notes for Mod Authors - PC Building Simulator

Packaging rules for PC Building Simulator mods, so Vortex installs them to the right place.
...
Recognised when the archive contains a file or folder named `PCBS_Data`.
...
- Recognised by any file named `Assembly-CSharp-firstpass.dll`.

Installs to: `PCBS_Data\Managed`
