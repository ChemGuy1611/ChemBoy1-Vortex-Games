# Notes for Mod Authors - Lies of P

Packaging rules for Lies of P mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `LiesofP` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `LiesofP\Content\Paks\LogicMods` |
| Pak Mods | a `.pak` file | `LiesofP\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `LiesofP\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `LiesofP\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `LiesofP\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `LiesofP`, `Engine` or `Content` | the game folder itself (no subfolder) |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | - |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `LiesofP\Binaries\Win64` |
...
Installs to: `LiesofP\Content\Paks\LogicMods`
...
Installs to: `LiesofP\Content\Paks\~mods`
...
Installs to: `LiesofP\Binaries\Win64`
...
Installs to: `LiesofP\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `LiesofP`, `Engine`, `Content`, `Binaries`, `Mods`, `Paks` or `Movies`.
...
- Recognised by any of these filenames in the archive: `engine.ini`, `game.ini`, `gameusersettings.ini`, `input.ini`, `scalability.ini`, `hardware.ini`, `deviceprofiles.ini`, `compat.ini`, `runtimeoptions.ini`, `gameplaytags.ini`, `enhancedinput.ini`, `consolevariables.ini`, `scalabilitygroups.ini`, `defaultengine.ini` or `defaultinput.ini`.
