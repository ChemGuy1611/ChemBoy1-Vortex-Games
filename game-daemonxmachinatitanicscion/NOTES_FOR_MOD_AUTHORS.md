# Notes for Mod Authors - Daemon X Machina: Titanic Scion

Packaging rules for Daemon X Machina: Titanic Scion mods, so Vortex installs them to the right place.
...
| Config File Mods | a config file such as `engine.ini` or `game.ini` | `CONFIGMOD_LOCATION\DXMTS\Saved\Config\Windows` |
...
- Installed to `CONFIGMOD_LOCATION\DXMTS\Saved\Config\Windows`.

Installs to: `CONFIGMOD_LOCATION\DXMTS\Saved\Config\Windows`
