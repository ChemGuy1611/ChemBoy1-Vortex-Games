...
const EXTENSION_URL = "https://www.nexusmods.com/site/mods/1429"; //Nexus link to this extension. Used for links
const PCGAMINGWIKI_URL = "https://www.pcgamingwiki.com/wiki/Daemon_X_Machina%3A_Titanic_Scion";
...
function getShippingExe(gamePath) {
  ...
  if (isCorrectExec(EXEC_DEFAULT)) {
    SHIPPING_EXE = path.join(
      ...
      "Binaries",
      EXEC_FOLDER_XBOX,
      `${EPIC_CODE_NAME}-${EXEC_FOLDER_DEFAULT}${SHIPEXE_STRING_DEFAULT}-Shipping.exe`,
    );
    return SHIPPING_EXE;
  }
  if (isCorrectExec(EXEC_EPIC)) {
    SHIPPING_EXE = path.join(
      ...
      "Binaries",
      EXEC_FOLDER_DEFAULT,
      `${EPIC_CODE_NAME}-${EXEC_FOLDER_DEFAULT}${SHIPEXE_STRING_EGS}-Shipping.exe`,
    );
    return SHIPPING_EXE;
  }
}
