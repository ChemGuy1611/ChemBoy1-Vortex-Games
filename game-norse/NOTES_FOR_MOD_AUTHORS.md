# Notes for Mod Authors - NORSE: Oath of Blood

Packaging rules for NORSE: Oath of Blood mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `Norse` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `Norse\Content\Paks\LogicMods` |
| Pak Mods | a `.pak` file | `Norse\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `Norse\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `Norse\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `Norse\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `Norse`, `Engine` or `Content` | the game folder itself (no subfolder) |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | - |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `Norse\Binaries\Win64` |
...
Installs to: `Norse\Content\Paks\LogicMods`
...
Installs to: `Norse\Content\Paks\~mods`
...
Installs to: `Norse\Binaries\Win64`
...
Installs to: `Norse\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `Norse`, `Engine`, `Content`, `Binaries` or `Mods`.
