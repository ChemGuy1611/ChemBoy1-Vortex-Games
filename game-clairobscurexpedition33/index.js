...
const GAME_ID = "clairobscurexpedition33"; //same as Nexus domain
const STEAMAPP_ID = "1903340"; //from steamdb.info
const STEAMAPP_ID_DEMO = "XXX"; //VERIFY if the EPIC_CODE_NAME and EXEC_DEMO match Steam full game
const EPICAPP_ID = "f18fc860e6b4419e89147983bf769723"; //from egdata.app
const GOGAPP_ID = "2125022825"; // from gogdb.org
const XBOXAPP_ID = "KeplerInteractive.Expedition33"; //from appxmanifest.xml
const XBOXEXECNAME = "AppExpedition33Shipping"; //from appxmanifest.xml
const XBOX_PUB_ID = "ymj30pw7xe604"; //get from Save folder. '8wekyb3d8bbwe' if published by Microsoft
...
const GAME_NAME_SHORT = "Clair Obscur E33"; //Try for 8-10 characters
const EPIC_CODE_NAME = "Sandfall"; //Folder in root
const EXEC = "Expedition33_Steam.exe"; //This is true ~80% of the time. Change if different
const EXEC_EPIC = "Expedition33_EGS.exe"; //change these 3 if different
...
//UE specific
const ENGINE_VERSION = "5.4.0"; //Unreal Engine version. usually '4.27.2.0' or '5.X.X.0'. Written to UE4SS-settings.ini if writeEngineVersion is enabled
...
const SHIPEXE_STRING_GOG = "GOG"; //getExecutable's GOG branch below is hand-adjusted for this game - see the inline note there
...
const UE5_SORTABLE_ID = `${GAME_ID}-ue5-sortable-modtype`; //already game-specific pre-migration - kept as-is per Step C2 (renaming would orphan existing installs)
...
//pre-migration deviation: a dedicated Content Folder modtype/installer, kept since template has no
//equivalent and existing installs are already tagged with this type - see testContent/installContent
...
//pre-migration deviation: already ends in the LogicMods folder itself (not Paks root like template) -
//kept as-is since existing installs deploy here; the 4 sites that would otherwise double-append
//LOGICMODS_FOLDER are adjusted below (MODTYPE_FOLDERS push, deserializeLogicMods, LogicMods context menus)
...
if (ue4ssLoadOrder) {
  ...
  //LOGICMODS_PATH already ends in the LogicMods folder for this game - see its declaration
...
function getExecutable(discoveryPath) {
  ...
  if (statCheckSync(discoveryPath, EXEC_GOG)) {
    GAME_VERSION = "gog";
    BINARIES_PATH = path.join(EPIC_CODE_NAME, "Binaries", EXEC_FOLDER_DEFAULT);
    //game-local override: real file is SandFallGOG-Win64-Shipping.exe (string right after the
    //project name, not after the folder like the other branches) - confirmed against the pre-migration file
    SHIPPING_EXE = path.join(
      BINARIES_PATH,
...
//Installer test for a bare Content folder (pre-migration deviation - see CONTENT_ID declaration)
...
//Installer install a bare Content folder
...
function applyGame(context, gameSpec) {
  ...
  //ahead of ROOT (39) since testRoot's ROOTSUB_FOLDERS match now also catches a bare "Content"
  //folder - CONTENT_ID must get first look so existing installs keep resolving to it
