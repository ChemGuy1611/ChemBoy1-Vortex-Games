...
Date: 2026-09-21
Notes:
- Mods are installed to the MODKit Mods folder in User Documents by default, so game folder mod types
  are only offered when Documents, the game folder and the Vortex staging folder share a drive.
- Adds inZOI-only mod types for the Documents Creations/AIGenerated/Canvas folders and the Mod Enabler.
////////////////////////////////////////////////*/
...
const GAME_ID = "inzoi"; //same as Nexus domain
const STEAMAPP_ID = "2456740"; //from steamdb.info
const STEAMAPP_ID_DEMO = "XXX"; //VERIFY if the EPIC_CODE_NAME and EXEC_DEMO match Steam full game
const EPICAPP_ID = null; //from egdata.app - no Epic release
const GOGAPP_ID = null; // from gogdb.org - no GOG release
const XBOXAPP_ID = null; //from appxmanifest.xml - no Xbox release
...
const GAME_NAME_SHORT = "inZOI"; //Try for 8-10 characters
const EPIC_CODE_NAME = "BlueClient"; //Folder in root
const EXEC = "inZOI.exe"; //This is true ~80% of the time. Change if different
...
const hasModKit = true; //toggle for UE ModKit mod support
...
//UE specific
const ENGINE_VERSION = "5.4.0.0"; //Unreal Engine version. usually '4.27.2.0' or '5.X.X.0'. Written to UE4SS-settings.ini if writeEngineVersion is enabled
...
const UE4SS_SUBFOLDERS = [
  "MapGenBP",
  "MemberVarLayoutTemplates",
  "UE4SS_Signatures",
  "VTableLayoutTemplates",
]; //subfolders of UE4SS folder
const SAVE_EXTS = [".sav"];
const SAVE_COMPAT_VERSIONS = ["steam"]; //game versions with installable save mods (never Xbox) - Steam is the only release
...
const SAVE_LOC = "Documents"; //string for notification text. Config and Save mods are almost always in the same place
const CONFIGMOD_LOCATION = LOCALAPPDATA; //almost always matches. Some are in game folder or Documents.
const SAVEMOD_LOCATION = DOCUMENTS; //saves live in Documents\inZOI\SaveGames, config stays in Local AppData
...
//inZOI keeps saves, ModKit mods and the in-game creation folders under Documents\inZOI
const DOCS_FOLDER = "inZOI";
const DOCS_LOC = "User Documents";
const DOCS_PATH = path.join(DOCUMENTS, DOCS_FOLDER);
const CONFIG_PATH_DEFAULT = path.join(
  CONFIGMOD_LOCATION,
  DATA_FOLDER,
  "Saved",
  "Config",
...
const SAVE_PATH_DEFAULT = path.join(DOCS_PATH, "SaveGames"); //saves sit directly under Documents\inZOI, not the usual Saved/SaveGames
...
let CHECK_DOCS = false; //game folder mod types need Documents, the game and the staging folder on one drive (default mod path is in Documents)
...
const UE5_SORTABLE_ID = `${GAME_ID}-ue5-sortable-modtype`; //kept at its pre-migration value (already game-specific) - renaming would orphan every installed pak mod
const UE5_SORTABLE_NAME = "UE Sortable Pak Mod";
const LEGACY_UE5_SORTABLE_ID = "ue5-sortable-modtype"; //bare shared mod type from before the game-specific id - stragglers are notified to reinstall
...
const MODKITMOD_ID = `${GAME_ID}-ue5modkitpak`; //kept at its pre-migration value so installed ModKit mods stay tagged
const MODKITMOD_NAME = "UE5 MODKit Pak Mod";
const MODKITMOD_FILE = "mod_manifest.json";
const MODKITMOD_EXT = ".uplugin";
const MODKITMOD_PATH = path.join(DOCS_PATH, "Mods"); //absolute - ModKit mods load from Documents, not the game folder
...
//Mod Enabler (legacy dsound.dll loader, superseded by the ModKit but still used by older mods)
...
const MOD_PATH_DEFAULT = MODKITMOD_PATH; //ModKit mods are the default install target for this game
...
//MODKITMOD_PATH is absolute (Documents), so it is not added to MODTYPE_FOLDERS - setup() ensures it directly
...
const spec = {
  game: {
    ...
    modPathIsRelative: false, //MOD_PATH_DEFAULT is an absolute Documents path
...
if (hasModKit) {
  spec.modTypes.push({
    ...
    name: MODKITMOD_NAME,
    priority: "high",
    targetPath: MODKITMOD_PATH,
  });
}

//inZOI-only mod types for the folders the game's own creation tools write to
spec.modTypes.push(
  {
    id: CREATIONS_ID,
    name: CREATIONS_NAME,
...
function pathPattern(api, game, pattern) {
  try {
    ...
    //this happens if the executable comes back as "undefined", usually caused by another app locking down the folder
...
async function installModKitMod(api, files, fileName) {
  ...
  try {
    //read mod.json file to get folder name (game will crash if this doesn't match)
    let contents = await fsp.readFile(path.join(fileName, rootPath, MODKITMOD_FILE), "utf8");
    contents = util.deBOM(contents);
    const JSON_OBJECT = JSON.parse(contents);
    const JSON_MOD_NAME = JSON_OBJECT["ProjectName"]; //inZOI's ModKit writes "ProjectName"
    if (JSON_MOD_NAME === undefined) {
      throw new Error(`"ProjectName" key not found in ${MODKITMOD_FILE} for mod ${MOD_NAME}`);
    }
    MOD_FOLDER = JSON_MOD_NAME;
  } catch (err) {
    //the installed folder must match ProjectName or the game crashes, so tell the user to check it
    log(
      "error",
      `Could not read ${MODKITMOD_FILE} file for mod ${MOD_NAME} - ${err}: ${err.message}`,
    );
    api.sendNotification({
      id: `${GAME_ID}-projectnamenotfound`,
      type: "error",
      message: `Could not get "ProjectName" from the ${MODKITMOD_FILE} file for mod ${MOD_NAME}.\nYou must manually verify the folder above the mod files is named correctly.`,
      allowSuppress: false,
      actions: [],
    });
  }
...
//Shared FOMOD guard for the inZOI-only installers below
...
function testModEnabler(files, gameId) {
  ...
  const supported = gameId === spec.game.id && isMod && isMod2 && !hasFomodInstaller(files);
...
function installModEnabler(api, files) {
  ...
  //Mod Enabler installs into the game folder, so it needs the game on the Documents/staging partition
  GAME_PATH = getDiscoveryPath(api);
  const CHECK = checkPartitions(DOCUMENTS, GAME_PATH);
  if (!CHECK) {
    api.showErrorNotification(
      `Could not install mod as "${MODENABLER_NAME}"`,
      `You tried installing a "${MODENABLER_NAME}" mod, but the game folder, Vortex Mod Staging folder, and ${DOCS_LOC} folder are not all on the same drive. Please move the game and/or Vortex Mod Staging Folder to the same drive as the ${DOCS_LOC} folder (typically C Drive) to install these types of mods with Vortex.`,
      { allowReport: false },
    );
...
//Installer test for archives built around one of the game's Documents folders (Creations, AIGenerated, Canvas)
function testDocsFolder(files, gameId, folderName) {
  const isMod = files.some(
    (file) => path.basename(file).toLowerCase() === folderName.toLowerCase(),
  );
  const supported = gameId === spec.game.id && isMod && !hasFomodInstaller(files);
...
//Install an archive built around one of the game's Documents folders
function installDocsFolder(files, folderName, modTypeId) {
  const modFile = files.find(
    (file) => path.basename(file).toLowerCase() === folderName.toLowerCase(),
  );
  const idx = modFile.indexOf(`${path.basename(modFile)}${path.sep}`);
  const setModTypeInstruction = { type: "setmodtype", value: modTypeId };

  // Remove directories and anything above the matched folder
  const filtered = files.filter((file) => !file.endsWith(path.sep));
  const instructions = filtered.map((file) => {
    return {
      type: "copy",
...
//Installer test for loose creation files that carry no folder of their own
function testDocsFile(files, gameId, matcher) {
  const supported = gameId === spec.game.id && files.some(matcher) && !hasFomodInstaller(files);
...
//Install loose creation files, keeping any folder the archive already provides
function installDocsFile(files, fileName, matcher, modTypeId) {
  const setModTypeInstruction = { type: "setmodtype", value: modTypeId };
  const MOD_NAME = path.basename(fileName);
  let MOD_FOLDER = MOD_NAME.replace(/(\.installing)*(\.zip)*(\.rar)*(\.7z)*( )*/gi, "");
  let modFile = files.find(matcher);
  ...
  if (path.basename(rootPath) !== ".") {
    ...
    modFile = rootPath; //target the folder so other folders in its directory come along
...
//Installer test for My3DPrinter files (model plus its data file)
function testMy3DPrinter(files, gameId) {
  const hasModel = files.some((file) => path.extname(file).toLowerCase() === MY3DPRINTER_EXT1);
  const hasData = files.some((file) => path.extname(file).toLowerCase() === MY3DPRINTER_EXT2);
  const supported = gameId === spec.game.id && hasModel && hasData && !hasFomodInstaller(files);
...
const isMy3DPrinterFile = (file) => MY3DPRINTER_EXTS.includes(path.extname(file).toLowerCase());
const isMyAppearancesFile = (file) => path.basename(file).toLowerCase() === MYAPPEARANCES_FILE;
const isAnimationsFile = (file) => path.basename(file).toLowerCase() === ANIMATIONS_FILE;
const isTexturesFile = (file) => path.basename(file).toLowerCase() === TEXTURES_FILE;
...
async function installUe4ssCombo(files, workingDir) {
  ...
  if (GAME_VERSION === "xbox") {
    try {
      ...
      await fsp.stat(path.join(workingDir, modFile, "Win64"));
      await fsp.rename(
        path.join(workingDir, modFile, "Win64"),
        path.join(workingDir, modFile, "WinGDK"),
      ); //*/
  ...
  const logicPakBasenames = filtered
    .filter(
      (f) =>
        f.toLowerCase().includes(`${LOGICMODS_FOLDER.toLowerCase()}${path.sep}`) &&
...
function testUe4ss(files, gameId) {
  ...
  const isFolder = files.some((file) =>
    UE4SS_SUBFOLDERS_LOWER.includes(path.basename(file).toLowerCase()),
  );
...
async function installScripts(api, files, fileName) {
  const scriptsFolder = files.find(
    (file) => path.basename(file).toLowerCase() === SCRIPTS_FOLDER.toLowerCase(),
  );
...
function installConfig(api, files) {
  ...
  instructions.push(setModTypeInstruction);
  GAME_PATH = getDiscoveryPath(api);
  const IS_CONFIG = checkPartitions(CONFIGMOD_LOCATION, DOCUMENTS);
  if (IS_CONFIG === false) {
...
async function installSave(api, files) {
  ...
  instructions.push(setModTypeInstruction);
  const IS_SAVE = checkPartitions(SAVEMOD_LOCATION, DOCUMENTS);
  if (IS_SAVE === false) {
...
//Check if the Mod Enabler is installed
...
//* Download the Mod Enabler from its Nexus page
async function downloadModEnabler(api, gameSpec, check = true) {
  ...
  if (!isInstalled || !check) {
    ...
    try {
      ...
      //Show the user the download page if the download or install fails
...
function checkPartitions(folder, discoveryPath) {
  if (!preferHardlinks && !IO_STORE) {
    //only do early return if hardlinks have no benefits and aren't required
    return true;
  }
  try {
    // Define paths
...
function partitionCheckNotify(api, CHECK_CONFIG, CHECK_SAVE, CHECK_DOCS) {
    ...
    actions: [
      {
        ...
        action: (dismiss) => {
          api.showDialog(
            ...
            {
              text:
                `Because ${GAME_NAME} includes the IO-Store Unreal Engine feature (or because hardlinks are preferred), Vortex must use hardlinks to install mods for the game.\n` +
                ...
                `  - Game folder mod types: ${CHECK_DOCS ? `ENABLED: ${DOCS_LOC} folder is on the same partition as the game and the Vortex staging folder, so the pak, UE4SS, Root and Mod Enabler modtypes are available` : `DISABLED: ${DOCS_LOC} folder is NOT on the same partition as the game and the Vortex staging folder, so the pak, UE4SS, Root and Mod Enabler modtypes are NOT available`}\n` +
                `\n` +
                `Game Path: ${GAME_PATH}\n` +
...
//The game only loads a ModKit mod when its manifest has bEnable set, so flip it after every deploy
async function setModkitModsEnabled() {
  let paths = [];
  try {
    const raw = await fsp.readdir(MODKITMOD_PATH, { recursive: true });
    for (let entry of raw) {
      if (path.basename(entry).toLowerCase() === MODKITMOD_FILE) {
        paths.push(path.join(MODKITMOD_PATH, entry));
      }
    }
  } catch {
    return; //ModKit Mods folder does not exist yet
  }

  for (let manifest of paths) {
    let content = await fsp.readFile(manifest);
    ...
    try {
      ...
      if (json.bEnable === false || json.bEnable === undefined) {
        ...
        await fsp.writeFile(manifest, content, { encoding: "utf8" });
      }
    } catch (err) {
      log("error", `[${GAME_NAME}] Could not parse/write ${manifest}: ${err} - ${err.message}`);
    }
  }
}
...
async function setup(discovery, api, gameSpec) {
  ...
  //config and save mods land in the user profile, so they are measured against Documents
  //(where the default mod path lives) rather than the game folder
  CHECK_CONFIG = checkPartitions(CONFIGMOD_LOCATION, DOCUMENTS);
  if (configSaveMatch) {
    CHECK_SAVE = CHECK_CONFIG;
  } else {
    CHECK_SAVE = checkPartitions(SAVEMOD_LOCATION, DOCUMENTS);
  }
  CHECK_DOCS = checkPartitions(DOCUMENTS, GAME_PATH);
  if (!CHECK_CONFIG || !CHECK_SAVE || !CHECK_DOCS) {
    partitionCheckNotify(api, CHECK_CONFIG, CHECK_SAVE, CHECK_DOCS);
  }
  ...
  //Documents folders are safe to write-check - they sit under the user profile, not the game
  await vfs.ensureDirWritableAsync(DOCS_PATH);
  ...
  await vfs.ensureDirWritableAsync(MODKITMOD_PATH); //also the default mod path
...
function applyGame(context, gameSpec) {
  ...
  //mods deploy into Documents by default, so only the game folder mod types are gated on the
  //partition check - the Documents ones always sit on the staging folder's drive
  const gameFolderTypeSupported = (gameId) => {
    GAME_PATH = getDiscoveryPath(context.api);
    if (GAME_PATH !== undefined) {
      CHECK_DOCS = checkPartitions(DOCUMENTS, GAME_PATH);
    }
    return gameId === GAME_ID && GAME_PATH !== undefined && CHECK_DOCS === true;
  };

  //register mod types recursively (types that are always the same)
  (gameSpec.modTypes || []).forEach((type, idx) => {
    const isGameFolderType = String(type.targetPath).includes("{gamePath}");
    context.registerModType(
      type.id,
      modTypePriority(type.priority) + idx,
      (gameId) => {
        var _a;
        const isDiscovered = !!((_a =
          context.api.getState().settings.gameMode.discovered[gameId]) === null || _a === void 0
          ? void 0
          : _a.path);
        if (!isDiscovered || gameId !== gameSpec.game.id) {
          return false;
        }
        return isGameFolderType ? gameFolderTypeSupported(gameId) : true;
      },
  ...
  //kept registered only so a straggler still deploys correctly in the brief window before
  ...
  //Mod Enabler drops its loader next to the game executable
  context.registerModType(
    MODENABLER_ID,
    60,
    gameFolderTypeSupported,
  ...
  if (hasModKit === true) {
    context.registerInstaller(MODKITMOD_ID, 25, testModKitMod, (files, fileName) =>
      installModKitMod(context.api, files, fileName),
    );
  }
  ...
  if (ue4ssLoadOrder) {
    ...
    context.registerInstaller(DLL_ID, 37, testDll, (files, fileName) =>
      installDll(context.api, files, fileName),
    );
  }
  context.registerInstaller(ROOT_ID, 39, testRoot, installRoot);
  context.registerInstaller(CONFIG_ID, 41, testConfig, (files) =>
  ...
  //inZOI-only installers: the Documents creation folders and the legacy Mod Enabler
  context.registerInstaller(
    CREATIONS_ID,
    28,
    (files, gameId) => testDocsFolder(files, gameId, CREATIONS_FOLDER),
    (files) => installDocsFolder(files, CREATIONS_FOLDER, CREATIONS_ID),
  );
  context.registerInstaller(
    AIGENERATED_ID,
    30,
    (files, gameId) => testDocsFolder(files, gameId, AIGENERATED_FOLDER),
    (files) => installDocsFolder(files, AIGENERATED_FOLDER, AIGENERATED_ID),
  );
  context.registerInstaller(
    CANVAS_ID,
    32,
    (files, gameId) => testDocsFolder(files, gameId, CANVAS_FOLDER),
    (files) => installDocsFolder(files, CANVAS_FOLDER, CANVAS_ID),
  );
  //ahead of the Script (35) and DLL (37) installers, which would otherwise claim sig.lua/dsound.dll
  context.registerInstaller(MODENABLER_ID, 34, testModEnabler, (files) =>
    installModEnabler(context.api, files),
  );
  context.registerInstaller(MY3DPRINTER_ID, 36, testMy3DPrinter, (files, fileName) =>
    installDocsFile(files, fileName, isMy3DPrinterFile, MY3DPRINTER_ID),
  );
  context.registerInstaller(
    ANIMATIONS_ID,
    38,
    (files, gameId) => testDocsFile(files, gameId, isAnimationsFile),
    (files, fileName) => installDocsFile(files, fileName, isAnimationsFile, ANIMATIONS_ID),
  );
  context.registerInstaller(
    MYAPPEARANCES_ID,
    40,
    (files, gameId) => testDocsFile(files, gameId, isMyAppearancesFile),
    (files, fileName) => installDocsFile(files, fileName, isMyAppearancesFile, MYAPPEARANCES_ID),
  );
  context.registerInstaller(
    TEXTURES_ID,
    42,
    (files, gameId) => testDocsFile(files, gameId, isTexturesFile),
    (files, fileName) => installDocsFile(files, fileName, isTexturesFile, TEXTURES_ID),
  );
  ...
  context.registerAction(
    ...
    () => {
      util.opn(MODKITMOD_PATH).catch(() => null);
    },
    () => {
  ...
  context.registerAction(
    ...
    "open-ext",
    {},
    `Open ${GAME_NAME} Documents Folder`,
    () => {
      util.opn(DOCS_PATH).catch(() => null);
    },
    () => {
      const state = context.api.getState();
      const gameId = selectors.activeGameId(state);
  ...
  context.registerAction(
    ...
    () => {
      downloadModEnabler(context.api, spec, false);
    },
    () => {
      const state = context.api.getState();
      const gameId = selectors.activeGameId(state);
...
async function didDeploy(api, profileId) {
  ...
  if (hasModKit) {
    await setModkitModsEnabled(); //a ModKit mod only loads when its manifest has bEnable set
  }
