# inZOI — Vortex Extension Explained

## Overview

| Property | Value |
| --- | --- |
| Name | inZOI Vortex Extension |
| Engine / Structure | Unreal Engine 4-5 Game |
| Author | ChemBoy1 |

### Notes

- Mods are installed to the MODKit Mods folder in User Documents by default, so game folder mod types
- Adds inZOI-only mod types for the Documents Creations/AIGenerated/Canvas folders and the Mod Enabler.

## Key Identifiers

| Property | Value |
| --- | --- |
| Game ID | `inzoi` |
| Executable | `inZOI.exe` |
| Executable (Xbox) | `gamelaunchhelper.exe` |
| Executable (GOG) | `inZOI.exe` |
| Executable (Demo) | `inZOI.exe` |
| Extension Page | [https://www.nexusmods.com/site/mods/1241](https://www.nexusmods.com/site/mods/1241) |
| PCGamingWiki | [https://www.pcgamingwiki.com/wiki/InZOI](https://www.pcgamingwiki.com/wiki/InZOI) |

## Supported Stores

- **Steam** — `2456740`

## Feature Flags

| Flag | Value | Description |
| --- | --- | --- |
| `hasXbox` | `false` | toggle for Xbox version logic. |
| `multiExe` | `false` | toggle for multiple executables (Epic/GOG/Demo don't match Steam) |
| `setupNotification` | `false` | enable to show the user a notification with special instructions (specify below) |
| `hasModKit` | `true` | toggle for UE ModKit mod support |
| `hasServer` | `false` | toggle for server pak mod logic |
| `preferHardlinks` | `true` | set true to perform partition checks when IO-STORE=false for Config/Save modtypes so that hardlinks available to more users |
| `autoDownloadUe4ss` | `false` | toggle for auto downloading UE4SS (only applies when ue4ssLoadOrder is enabled) |
| `writeEngineVersion` | `false` | toggle to write ENGINE_VERSION into UE4SS-settings.ini (EngineVersionOverride) on deploy, when UE4SS is installed |
| `SIGBYPASS_REQUIRED` | `false` | set true if there are .sig files in the Paks folder |
| `IO_STORE` | `true` | true if the Paks folder contains .ucas and .utoc files |
| `hasUserIdFolder` | `true` | true if there is a folder in the Save path that is a user ID that must be read (i.e. Steam ID) |
| `debug` | `false` | toggle for debug mode |
| `PAKMOD_LOADORDER` | `true` | set to false if you don't want loadOrder. If must be in "Paks" root, disable loadOrder. |
| `FBLO` | `true` | set to false to use legacy load order page |
| `ue4ssLoadOrder` | `true` | master toggle for UE4SS support: UE4SS/Scripts/DLL/LogicMods mod types and installers, UE4SS buttons, load order page, and mods.txt writing |
| `logicModsLoadOrder` | `true` | enable load order page and load_order.txt writing for LogicMods/Blueprint pak mods |
| `collectionsLoadOrder` | `true` | include UE4SS and LogicMods load orders in collections (ANDed with the toggles above) |
| `SYM_LINKS` | `true` | true if symlink deployment is enabled for this game |
| `CHECK_CONFIG` | `false` | boolean to check if game, staging folder, and config and save folders are on the same drive |
| `CHECK_SAVE` | `false` | secondary same as above (if save and config are in different locations) |
| `CHECK_DOCS` | `false` | game folder mod types need Documents, the game and the staging folder on one drive (default mod path is in Documents) |
| `mod_update_all_profile` | `false` | for mod update to keep them in the load order and not uncheck them |
| `updating_mod` | `false` | used to see if it's a mod update or not |

## Mod Types

Mod types define where each category of mod gets deployed:

| Name | ID | Priority | Target Path |
| --- | --- | --- | --- |
| UE4SS Script-LogicMod Combo | `inzoi-ue4sscombo` | high | `{gamePath}` |
| UE4SS LogicMods (Blueprint) | `inzoi-logicmods` | high | `{gamePath}/BlueClient/Content/Paks` |
| Paks (no "~mods") | `inzoi-pakalt` | high | `{gamePath}/BlueClient/Content/Paks` |
| Root Folder | `inzoi-root` | high | `{gamePath}` |
| UE5 MODKit Pak Mod | `inzoi-ue5modkitpak` | high | `DOCUMENTS/inZOI/Mods` |
| Creations (Documents) | `inzoi-creations` | high | `CREATIONS_PATH` |
| UE Sortable Pak Mod | `inzoi-ue5-sortable-modtype` | 25 | `?` |
| UE4SS Script Mod | `inzoi-scripts` | 50 | `?` |
| UE4SS DLL Mod | `inzoi-ue4ssdll` | 52 | `?` |
| Binaries (Engine Injector) | `inzoi-binaries` | 54 | `?` |
| UE4SS | `inzoi-ue4ss` | 56 | `?` |
| Legacy UE - REINSTALL TO SORT | `ue5-sortable-modtype` | 65 | `?` |
| Mod Enabler | `inzoi-modenabler` | 60 | `?` |
| Config (Local AppData) | `inzoi-config` | 62 | `?` |
| Saves (Documents) | `inzoi-save` | 64 | `?` |

## Mod Installers

Installers run in priority order (lower number = tested first). The first installer whose test returns `supported: true` handles the archive.

| Installer ID | Priority |
| --- | --- |
| `inzoi-ue5modkitpak` | 25 |
| `inzoi-ue4sscombo` | 26 |
| `inzoi-logicmods` | 27 |
| `inzoi-ue5-sortable-modtype` | 29 |
| `inzoi-ue4ss` | 31 |
| `inzoi-scripts` | 35 |
| `inzoi-ue4ssdll` | 37 |
| `inzoi-root` | 39 |
| `inzoi-config` | 41 |
| `inzoi-save` | 43 |
| `inzoi-binaries` | 49 |
| `inzoi-creations` | 28 |
| `inzoi-aigenerated` | 30 |
| `inzoi-canvas` | 32 |
| `inzoi-modenabler` | 34 |
| `inzoi-my3dprinter` | 36 |
| `inzoi-animations` | 38 |
| `inzoi-myappearances` | 40 |
| `inzoi-textures` | 42 |

## Toolbar Actions

These buttons appear in the Vortex mod-icons toolbar when this game is active:

- Open MODKit Mods Folder (Documents)
- Open MODKit Folder (Epic)
- Open inZOI Documents Folder
- Download Mod Enabler (Legacy)
- Open Paks Folder
- Open Binaries Folder
- Open UE4SS Mods Folder
- Open LogicMods Folder
- Open Config Folder
- Open Saves Folder
- Download UE4SS
- Open UE4SS Settings INI
- Open UE4SS mods.txt
- Open PCGamingWiki Page
- Open SteamDB Page
- View Changelog
- Submit Bug Report
- Open Downloads Folder

## Auto-Downloaded Dependencies

| Dependency | Version | Details |
| --- | --- | --- |
| UE4SS | — | — |

## Special Features

- **Load Order** — mods are assigned numbered folder names or sorted based on their position in the load order.
- **UE4SS Load Order** — manages UE4SS script/DLL mod load order via a dedicated page; serializes order to `mods.txt` on deploy.
- **Deploy Hook** (`did-deploy`) — runs custom logic (e.g., notifications, metadata patching) every time mods are deployed.
- **Purge Hook** (`did-purge`) — runs custom logic when mods are purged.
- **Auto-Downloader** — can automatically download required tools (mod loader, managers, etc.).
- **FOMOD Awareness** — installers check for and skip `fomod/ModuleConfig.xml` to avoid conflicts with the built-in FOMOD installer.
- **Version Detection** — detects game version (Steam/Xbox/GOG/Demo) and adjusts paths accordingly.
