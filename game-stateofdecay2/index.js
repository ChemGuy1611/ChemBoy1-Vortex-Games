...
function applyGame(context, gameSpec) {
  ...
  //('ue4-sortable-modtype') globally itself, reading UNREALDATA via spec.game.details.unrealEngine above.
  //COOKED_ID and MANAGER_ID (game-specific, no template equivalent) register via the generic modTypes loop
  //above - see the spec.modTypes array.
  ...
  //Priorities renumbered around UEMI's fixed ue4-pak-installer slot (prio 25, external). UE4SSCOMBO_ID
  //must run BEFORE UEMI's generic "any .pak present" test or its archives get intercepted by it - kept
  //tight, consecutive, immediately below 25. Everything else doesn't test for .pak, so stays above 25 at
  //template's original spacing.
  if (hasModKit === true) {
