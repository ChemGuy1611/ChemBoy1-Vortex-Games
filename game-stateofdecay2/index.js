...
const GAME_ID = "stateofdecay2"; //same as Nexus domain
const STEAMAPP_ID = "495420"; //from steamdb.info
const STEAMAPP_ID_DEMO = null; //no demo
const EPICAPP_ID = "Snoek"; //from egdata.app
const GOGAPP_ID = null; // from gogdb.org
const XBOXAPP_ID = "Microsoft.Dayton"; //from appxmanifest.xml
const XBOXEXECNAME = "Shipping"; //from appxmanifest.xml
...
const GAME_NAME_SHORT = "State of Decay 2"; //Try for 8-10 characters
const EPIC_CODE_NAME = "StateOfDecay2"; //Folder in root
...
let PAKMOD_PATH = path.join(LOCALAPPDATA, EPIC_CODE_NAME, "Saved", "Paks"); //preserved from pre-FBLO extension - Paks live in LocalAppData, not under gamePath (no "~mods" folder for this game)
...
const ue4ssLoadOrder = false; //master toggle for UE4SS support: UE4SS/Scripts/DLL/LogicMods mod types and installers, UE4SS buttons, load order page, and mods.txt writing. Shipped dormant - this game had no prior UE4SS support; stack is wired so the toggle can flip later without a second pass.
const logicModsLoadOrder = false; //enable load order page and load_order.txt writing for LogicMods/Blueprint pak mods. Dormant with ue4ssLoadOrder - LogicMods need UE4SS's BPModLoaderMod, which this game doesn't offer while the stack is off.
...
const UNREALDATA = {
  absModsPath: PAKMOD_PATH, //preserved from pre-FBLO extension - Paks live in LocalAppData, not under gamePath
  fileExt: PAKMOD_EXTS,
...
//NOTE: no PAK_ALT_ID here - this game has no in-gamePath Paks folder to offer as a "no ~mods" manual
//override (Paks live entirely in LocalAppData - see UNREALDATA.absModsPath above).
const PAK_PATH = UNREALDATA.absModsPath;
const PAK_EXT = ".pak";

//Cooked mods (game-specific, no template equivalent) - preserved from pre-FBLO extension. This is the
//game's dominant/default mod format (raw .uasset overrides), not Paks.
...
//SoD2 Mod Manager (game-specific, no template equivalent) - external tool + auto-downloaded modtype
//preserved from pre-FBLO extension.
const MANAGER_ID = `${GAME_ID}-modmanager`;
const MANAGER_NAME = "SoD2 Mod Manager";
const MANAGER_EXEC = "modintegrator.exe";

//Community Editor (game-specific tool, no template equivalent) - preserved from pre-FBLO extension.
...
//Default mod install target is Cooked, not Paks - preserved from pre-FBLO extension. Cooked (raw .uasset
//overrides) is this game's dominant mod format; Paks is a secondary UEMI-managed type.
...
//NOTE: no PAK_PATH/PAK_ALT_PATH here - Paks live in LocalAppData (UNREALDATA.absModsPath), not gamePath,
//so they don't belong in this gamePath-relative writability list. See setup() for their own writability check.
let MODTYPE_FOLDERS = [];
if (ue4ssLoadOrder) {
...
const spec = {
  ...
  modTypes: [
    ...
    //NOTE: no PAK_ALT_ID entry - this game has no in-gamePath Paks folder to offer as a manual override.
    {
      id: ROOT_ID,
      name: ROOT_NAME,
      priority: "high",
...
//Cooked mods and SoD2 Mod Manager installer functions (game-specific, no template equivalent, preserved from
//pre-FBLO extension)

//Test for Cooked mods
  ...
  if (!modManagerInstalled) {
    ...
    try {
      ...
      const batched = [
        ...
        actions.setModType(gameSpec.game.id, modId, MANAGER_ID),
      ];
...
//Installer test for SoD2 Mod Manager
...
//Install SoD2 Mod Manager files
...
async function setup(discovery, api, gameSpec) {
  ...
  //Cooked and Paks folders live in LocalAppData, not gamePath - modFoldersEnsureWritable joins against
  //GAME_PATH, so these are ensured directly instead (preserved from pre-FBLO extension).
  await vfs.ensureDirWritableAsync(path.join(LOCALAPPDATA, COOKED_PATH));
  await vfs.ensureDirWritableAsync(UNREALDATA.absModsPath);
...
function applyGame(context, gameSpec) {
  ...
  //Pak modType - UNREALDATA.absModsPath is already an absolute LocalAppData path (see the pre-FBLO
  //extension's own targetPath and UEMI's former getUnrealModsPath early-return for absModsPath), so
  //the targetPath function returns it directly - no {gamePath} pathPattern substitution needed here,
  //unlike the template's gamePath-relative default. COOKED_ID and MANAGER_ID (game-specific, no
  //template equivalent) register via the generic modTypes loop above - see the spec.modTypes array.
  context.registerModType(
    UE5_SORTABLE_ID,
    25,
    (gameId) => {
  ...
  //Cooked and SoD2 Mod Manager installers (game-specific, no template equivalent)
  context.registerInstaller(COOKED_ID, 45, testCooked, installCooked);
  context.registerInstaller(MANAGER_ID, 47, testModManager, installModManager);
  ...
  context.registerAction(
    ...
    () => {
      downloadModManager(context.api, gameSpec).catch(() => null);
    },
    () => {
      const state = context.api.getState();
      const gameId = selectors.activeGameId(state);
...
function main(context) {
  ...
  context.registerMigration((old) => migrateUemiPakType310(context.api, old));
  if (UNREALDATA.loadOrder === true) {
...
//longer required. Retag only - both ids resolve to the same UNREALDATA.absModsPath folder, so
...
async function migrateUemiPakType310(api, oldVersion) {
  if (semver.gte(oldVersion, "3.1.0")) return;
