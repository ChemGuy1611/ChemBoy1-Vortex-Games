# Notes for Mod Authors - Painkiller

Packaging rules for Painkiller mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `Painkiller` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `Painkiller\Content\Paks\LogicMods\LogicMods` |
| Pak Mods | a `.pak` file | `Painkiller\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `Painkiller\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `Painkiller\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `Painkiller\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `Painkiller` | the game folder itself (no subfolder) |
| Contentfolder | a file or folder named `Content` | `Painkiller` |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | `CONFIGMOD_LOCATION\Painkiller\Saved\Config\Windows` |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `Painkiller\Binaries\Win64` |
...
Installs to: `Painkiller\Content\Paks\LogicMods\LogicMods`
...
Installs to: `Painkiller\Content\Paks\~mods`
...
Installs to: `Painkiller\Binaries\Win64`
...
Installs to: `Painkiller\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `Painkiller`.
...
- Installed to `CONFIGMOD_LOCATION\Painkiller\Saved\Config\Windows`.

Installs to: `CONFIGMOD_LOCATION\Painkiller\Saved\Config\Windows`
