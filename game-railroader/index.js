...
function testUmm(files, gameId) {
  const isUmm = files.some(
    (file) => path.basename(file).toLowerCase() === UMM_INST_EXEC.toLowerCase(),
  );
...
async function installUmm(api, files, workingDir) {
  ...
  const hasFile = (relPath) =>
    filtered.some((file) => file.toLowerCase() === archivePath(relPath).toLowerCase());
  const instructions = filtered.map((file) => {
    return {
      ...
      destination: path.join(
        UMM_FOLDER,
        rootPath === "." ? file : file.substring(rootPath.length + 1),
      ),
    };
  });
  ...
  try {
    ...
    gameInfo = undefined;
    api.showErrorNotification(`Could not read the ${UMM_NAME} game list`, err, {
      allowReport: false,
    });
  }
  ...
  const gameHasXml =
    ...
    statCheckSync(discoveryPath, path.join(DATA_FOLDER, "Managed", UMM_XML_DLL));
  if (!gameHasXml) {
...
function setUmmRegistry(api) {
  ...
  try {
    ...
    winapi.RegSetKeyValue(
      UMM_REG_HIVE,
      UMM_REG_KEY,
      "ExePath",
      path.join(discoveryPath, UMM_INST_PATH),
    );
  } catch (err) {
...
function runUmm(api) {
  ...
  try {
    ...
    const TOOL_PATH =
      discoveryPath !== undefined ? path.join(discoveryPath, UMM_INST_PATH) : undefined;
...
async function getRailloader(api) {
  ...
  api.showDialog(
    ...
    {
      text:
        `${RAILLOADER_NAME}'s official download site is currently offline, so Vortex can not fetch it for you.\n` +
...
function testRailloaderApp(files, gameId) {
  const isLoader = files.some((file) =>
    RAILLOADER_FILES.includes(path.basename(file).toLowerCase()),
  );
...
function applyGame(context, gameSpec) {
  ...
  context.registerInstaller(UMM_ID, 25, testUmm, (files, workingDir) =>
    installUmm(context.api, files, workingDir),
  );
