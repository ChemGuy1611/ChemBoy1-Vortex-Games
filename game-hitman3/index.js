...
function installLoader(files) {
  ...
  const isExcluded = (file) =>
    LOADER_EXCLUDE_LOWER.includes(file.substr(idx).split(path.sep)[0].toLowerCase());

  // Remove directories, anything that isn't in the rootPath, and the folders the framework owns.
  const filtered = files.filter(
    (file) => file.startsWith(rootPrefix) && !file.endsWith(path.sep) && !isExcluded(file),
  );
...
function testMod(files, gameId) {
  ...
  const hasManifest = files.some((file) =>
    MOD_FILES_LOWER.includes(path.basename(file).toLowerCase()),
  );
...
async function installMod(files, destinationPath) {
  ...
  const archiveFiles = files.filter((file) =>
    MOD_ARCHIVE_EXTS.includes(path.extname(file).toLowerCase()),
  );
  if (archiveFiles.length > 0) {
    //a mod packed inside another archive is already in the shape the framework wants
  ...
  if (!hasManifest) {
    //loose RPKG files - "Add a Mod" takes those directly, no repacking needed
  ...
  let packPaths = [];
  if (modDirs.length > 0) {
    //already mod folders - pack those and leave any loose readme behind
    packPaths = modDirs.map((dir) => path.join(destinationPath, dir));
  } else {
    //manifest is at the archive root - wrap the whole thing in a folder named after the mod
    let wrapper = modName;
...
async function downloadNexusInstaller(api) {
  ...
  try {
    ...
    //courtesy download - it must never block or shout over the framework install
...
function deployNotify(api) {
  ...
  const MORE_TEXT =
    `Deploying only places each mod in "<GameFolder>\\${MOD_PATH}", packed as a single archive. ` +
