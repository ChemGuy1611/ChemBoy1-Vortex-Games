...
const DATA_FOLDER = path.join("Godot", "app_userdata", "XXX");
...
const LOADER_PIN =
  ENGINE_VERSION === "3" ? { pinVersion: LOADER_VERSION, pinTag: `v${LOADER_VERSION}` } : {};
const REQUIREMENTS = [
  {
    //Godot Mod Loader
...
const LOADER3_DL_URL =
  "https://github.com/GodotModding/godot-mod-loader/releases/download/v6.3.0/godot-mod-loader_v6.3.0_self-setup.zip";
const LOADER4_DL_URL =
  "https://github.com/GodotModding/godot-mod-loader/releases/download/v7.0.1/ModLoader-Self-Setup_7.0.1-WIN.zip";
...
function installLoader(files) {
  ...
  const filtered = files.filter((file) => !isExcludedLoaderFile(file) && !file.endsWith(path.sep));
