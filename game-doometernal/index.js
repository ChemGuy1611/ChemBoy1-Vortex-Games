...
const GOGAPP_ID = "1344230395"; //https://www.gogdb.org/product/1344230395
const XBOXAPP_ID = "BethesdaSoftworks.DOOMEternal-PC";
const XBOXEXECNAME = "Game";
const DISCOVERY_IDS_ACTIVE = [STEAMAPP_ID, EPICAPP_ID, XBOXAPP_ID, GOGAPP_ID]; // UPDATE THIS WITH ALL VALID IDs
...
const GB_REQUIREMENTS = [
  //GameBanana requirements for gamebanana_downloader.js
  {
    //EternalModInjector
    gbItemType: "Tool",
    gbItemId: INJ_TOOL_ID,
    modType: INJECTOR_ID,
    userFacingName: INJECTOR_NAME,
    fallbackVersion: INJ_REV,
    fallbackFileId: INJ_DL_ID,
    fileIdAttribute: INJ_FILE_ATTR,
    pageUrl: INJ_URL,
  },
];
...
// (EternalModInjector download/update logic lives in gamebanana_downloader.js)

//Check for injector updates when Vortex checks mod versions (Check for Updates button)
async function onCheckModVersion(api, gameId, mods, forced) {
  try {
    await checkForGameBananaUpdate(api, spec, GB_REQUIREMENTS);
  } catch (err) {
    ...
    actions: [
      {
        ...
        action: (dismiss) => {
          api.showDialog(
            ...
            {
              text:
                `Vortex does not support Xbox version of DOOM Eternal. You must manually dump the game files according to the instructions below.\n` +
...
async function setup(discovery, api, gameSpec) {
  ...
  await downloadGameBanana(api, gameSpec, GB_REQUIREMENTS);
  await checkForGameBananaUpdate(api, gameSpec, GB_REQUIREMENTS).catch(() => null); //update check should never block setup
  await vfs.ensureDirWritableAsync(path.join(discovery.path, "doomSandBox"));
