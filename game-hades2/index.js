...
const MODUTIL_URL =
  "https://github.com/SGG-Modding/ModUtil/releases/download/2.10.1/SGG_Modding-ModUtil-2.10.1.zip"; //legacy, pre-1.0
...
const spec = {
  ...
  discovery: {
    ids: [STEAMAPP_ID, EPICAPP_ID, XBOXAPP_ID],
    names: [],
  },
};
...
function makeIsSupported(api) {
  return (gameId) => {
    ...
    return (
      gameId === GAME_ID &&
      !!((_a = api.getState().settings.gameMode.discovered[gameId]) === null || _a === void 0
        ? void 0
        : _a.path)
    );
  };
...
function testPlugin(files, gameId) {
  ...
  const isMod =
    files.some((file) => path.basename(file).toLowerCase() === PLUGIN_FILE) &&
...
async function installPlugin(files, destinationPath) {
  ...
  // Remove directories and anything that isn't in the rootPath.
  const filtered = files.filter((file) => file.startsWith(stripPrefix) && !file.endsWith(path.sep));
    ...
    actions: [
      {
        ...
        action: (dismiss) => {
          api.showDialog(
            "question",
            "Mod Importer Must Be Run",
            {
              text:
                "The Mod Importer tool downloaded by this extension must be run every time new mods are installed.\n" +
  ...
  context.once(() => {
    ...
    api.onAsync("check-mods-version", (gameId, mods, forced) => {
      ...
      return checkForThunderstoreUpdate(api, spec, TS_REQUIREMENTS).catch((err) =>
        log("warn", `Failed to check for Hell2Modding requirement updates: ${err}`),
      );
    });
    if (thunderstoreBrowser) {
