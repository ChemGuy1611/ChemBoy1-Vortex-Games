# Notes for Mod Authors - MiSide

Packaging rules for MiSide mods, so Vortex installs them to the right place.
...
| Root / Game Folder Mods | a `MiSideFull_Data` folder | the game folder itself (no subfolder) |
...
| Asset Replacement Mods | a `.assets` file | `MiSideFull_Data` |
...
- Recognised by a folder named `MiSideFull_Data` or `MiSide_Data` in the archive.
