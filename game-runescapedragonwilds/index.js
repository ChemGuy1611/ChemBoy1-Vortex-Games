...
const GAME_NAME_SHORT = "RuneScape DW";
const EXEC = "RSDragonwilds.exe";
let GAME_PATH = "";
...
const EPIC_CODE_NAME = "RSDragonwilds";
...
const SAVE_FOLDER = path.join(LOCALAPPDATA, EPIC_CODE_NAME, "Saved");
let USERID_FOLDER = "";
...
const UE4SS_PAGE_NO = 4;
const UE4SS_FILE_NO = 20;
...
function installCharacter(api, files) {
  ...
  if (IS_CHARACTER === false) {
    api.showErrorNotification(
      `Could not install mod as Save Character`,
      `You tried installing a Save Character mod, but the game, staging folder, and Local AppData folder are not all on the same drive. Please move the game and/or staging folder to the same drive as the Local AppData folder (typically C Drive) to install these types of mods with Vortex.`,
      { allowReport: false },
    );
  }
  return Promise.resolve({ instructions });
...
function applyGame(context, gameSpec) {
  ...
  context.registerInstaller(CHARACTER_ID, 43, testCharacter, (files) =>
