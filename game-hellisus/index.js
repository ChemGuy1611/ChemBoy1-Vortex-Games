...
const GAME_ID = "hellisus"; //same as Nexus domain
const STEAMAPP_ID = "1620730"; //from steamdb.info
const STEAMAPP_ID_DEMO = "3623800"; //VERIFY if the EPIC_CODE_NAME and EXEC_DEMO match Steam full game
const EPICAPP_ID = "14ce1acff0db4cf8bf59533318058c7c"; //from egdata.app
const GOGAPP_ID = null; // from gogdb.org
const XBOXAPP_ID = "BigbenInteractiveSA.HellisUs"; //from appxmanifest.xml
const XBOXEXECNAME = "AppHellisUsShipping"; //from appxmanifest.xml
const XBOX_PUB_ID = "tqjv3vrxr8ppw"; //get from Save folder. '8wekyb3d8bbwe' if published by Microsoft
const DISCOVERY_IDS_ACTIVE = [STEAMAPP_ID, EPICAPP_ID, STEAMAPP_ID_DEMO, XBOXAPP_ID]; // UPDATE THIS WITH ALL VALID IDs

const GAME_NAME = "Hell is Us";
const GAME_NAME_SHORT = "Hell is Us"; //Try for 8-10 characters
const EPIC_CODE_NAME = "HellIsUs"; //Folder in root
...
const PCGAMINGWIKI_URL = "https://www.pcgamingwiki.com/wiki/Hell_is_Us";
const STEAMDB_URL = `https://steamdb.info/app/${STEAMAPP_ID}/`;
const EXTENSION_URL = "https://www.nexusmods.com/site/mods/1417"; //Nexus link to this extension. Used for links
...
//UE specific
const ENGINE_VERSION = "5.5.4.0"; //Unreal Engine version. usually '4.27.2.0' or '5.X.X.0'. Written to UE4SS-settings.ini if writeEngineVersion is enabled
...
const PAK_ALT_ID = `${GAME_ID}-pak`; //kept from the pre-parity extension so existing installs keep their mod type
...
const CONTENT_PATH = EPIC_CODE_NAME;
...
const LOGICMODS_PATH = path.join(EPIC_CODE_NAME, "Content", "Paks", "LogicMods"); //already ends in the LogicMods folder for this game
...
const SIGBYPASS_LUA = "universal.lua";
const SIGBYPASS_PAGE_NO = 22;
const SIGBYPASS_FILE_NO = 64;
...
function installLogic(files) {
  //indexed off the pak itself, not the LogicMods folder: the mod type path already ends in
  //LogicMods for this game, so the staged layout must stay flat (matches the 0.1.0 extension)
...
//Installer test for Content folder files
...
//Installer install Content folder files
...
//pak installer's priority (UE5_SORTABLE_ID = 29): UE4SS Combo (26, always registered) and
//LogicMods (27, only when logicModsLoadOrder) - this game has no Mod Kit installer. An installer
//at or below priority 29 never gets a turn once a pak-type file is present, since testPak's own
//check is just "has a pak-type file" with no exclusivity - so lower-priority markers can't have
...
function applyGame(context, gameSpec) {
  ...
  context.registerInstaller(CONTENT_ID, 38, testContent, installContent); //ahead of the Root installer, whose ROOTSUB branch would otherwise claim "Content" folder mods
