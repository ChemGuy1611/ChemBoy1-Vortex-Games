...
const EXEC = "re3.exe";
const EXEC_DEMO = "re3demo.exe";
const REF_STRING = "REFramework"; //file name for nightly latest URL - constant in recent versions. Not named per game any longer.
const GAME_NAME = "Resident Evil 3 (2020)";
const GAME_NAME_SHORT = "RE 3";
const PCGAMINGWIKI_URL = "https://www.pcgamingwiki.com/wiki/Resident_Evil_3_%282020%29";
const STEAMDB_URL = `https://steamdb.info/app/${STEAMAPP_ID}/`;
const EXTENSION_URL = "https://www.nexusmods.com/site/mods/433"; //Nexus link to this extension. Used for links

const FLUFFY_FOLDER = "RE3R";
...
const ROOT_EXTS = [".exe"];
const REF_PAGE_NO = 882;
const REF_FILE_NO = 3101;
const REF_DOMAIN = GAME_ID;
const CONFIG_PATH = ".";
const CONFIG_FILE = "re3_config.ini";
...
async function getSavePath() {
  ...
  let STEAM_PATH = "";
  try {
    const instPath = winapi.RegGetValue(STEAM_HIVE, STEAM_KEY, STEAM_VALUE);
    if (!instPath) {
      STEAM_PATH = "";
    } else {
  ...
  if (!isInstalled) {
    ...
    try {
      ...
      try {
        ...
        const fileTime = (input) => Number.parseInt(input.uploaded_time, 10);
        const file = modFiles
          .filter((file) => file.category_id === 1 && !file.file_name.toLowerCase().includes("rt"))
          .sort((lhs, rhs) => fileTime(lhs) - fileTime(rhs))
          .reverse()[0];
        if (file === undefined) {
          throw new util.ProcessCanceled(`No ${MOD_NAME} main file found`);
      ...
      },
      {
        ...
        action: (dismiss) => {
          api.showDialog(
            ...
            {
              text:
                `You must use ${MOD_NAME} to enable most mods after installing with Vortex.\n` +
...
function applyGame(context, gameSpec) {
  ...
  if (!reZip) {
    context.registerInstaller(FLUFFYMOD_ID, 48, testFluffyMod, installFluffyMod);
  } else {
