# Notes for Mod Authors - Styx BOG

Packaging rules for Styx BOG mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `Styx3` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `Styx3\Content\Paks\LogicMods` |
| Pak Mods | a `.pak` file | `Styx3\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `Styx3\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `Styx3\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `Styx3\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `Styx3`, `Engine` or `Content` | the game folder itself (no subfolder) |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | - |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `Styx3\Binaries\Win64` |
...
Installs to: `Styx3\Content\Paks\LogicMods`
...
Installs to: `Styx3\Content\Paks\~mods`
...
Installs to: `Styx3\Binaries\Win64`
...
Installs to: `Styx3\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `Styx3`, `Engine`, `Content`, `Binaries` or `Mods`.
