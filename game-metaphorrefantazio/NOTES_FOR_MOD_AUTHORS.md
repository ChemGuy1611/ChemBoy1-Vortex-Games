# Notes for Mod Authors - Metaphor: ReFantazio

Packaging rules for Metaphor: ReFantazio mods, so Vortex installs them to the right place.
...
Recognised when the archive contains a file or folder named `modconfig.json` and a file or folder named `mrfpc.modloader.dll`.

Installs to: `Reloaded\Mods\MRFPC_Mod_Loader`
