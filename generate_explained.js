...
const FLAG_DESCRIPTIONS = {
  ...
  ue4ssLoadOrder:
    "enables UE4SS script/DLL mod load order management and mods.txt serialization on deploy",
  // Store / platform
  ...
  enableSaveInstaller:
    "enables the save file installer (only recommended if saves are in the game folder)",
  ...
  // Save / config
  hasUserIdFolder:
    "a user ID folder (Steam ID, username) exists in the save path and must be detected at runtime",
...
function getFlagDescription(name, comment) {
  ...
  if (name.endsWith("_IS_ELEVATED"))
    return "the tool requires elevated/admin privileges to install";
...
function extractDependencies(src, table) {
  ...
  // UE4SS
  if (
    src.includes("downloadUe4ss") ||
    src.includes("UE4SS_URL") ||
    src.includes("autoDownloadUe4ss")
  ) {
    deps.push({
      name: "UE4SS",
      version: null,
...
function extractConfigSavePaths(table) {
  ...
  // Config paths
  const configPath =
    table.get("CONFIG_PATH") || table.get("CONFIG_FOLDER") || table.get("CONFIG_PATH_DEFAULT");
  ...
  // Save paths
  const savePath =
    table.get("SAVE_PATH") || table.get("SAVE_PATH_DEFAULT") || table.get("SAVE_FOLDER");
...
function detectSpecialFeatures(src, flags, stores) {
  ...
  // Load Order
  const hasLoadOrder = flags.some(
    (f) =>
      (f.name === "loadOrder" || f.name === "PAKMOD_LOADORDER" || f.name === "FBLO") &&
      f.value === "true",
  );
  if (hasLoadOrder) {
    features.push(
  ...
  // Auto-Downloader
  const hasDownloader =
    src.includes("require('./downloader')") ||
    src.includes("downloadBrowser") ||
    src.includes("downloadManager") ||
    src.includes("downloadLoader") ||
    src.includes("auto-download") ||
    src.includes("vortex-download") ||
    src.includes("downloadACSE") ||
    src.includes("downloadModLoader") ||
    src.includes("downloadBepInEx") ||
    src.includes("downloadFluffy") ||
    src.includes("downloadREFramework") ||
    src.includes("downloadUe4ss") ||
  ...
  if (src.includes("context.requireExtension")) {
    ...
    if (reqMatch) {
      ...
      if (optional.length > 0) {
        features.push(
          `**Optional Extensions** — uses these if the user has them installed: ${optional.join(", ")}.`,
        );
      }
    }
  }
...
function buildMarkdown(dirName, src) {
  ...
  const modTypes = [
    ...specModTypes,
    ...registeredModTypes.filter((mt) => !seenModTypeIds.has(mt.id)),
  ];
  ...
  const execName =
    table.get("EXEC") || table.get("EXEC_NAME") || table.get("EXEC_DEFAULT") || "N/A";
  ...
  const hasPaths =
    ...
    configSave.configRegistry ||
    configSave.savePath ||
    configSave.savePathXbox;
  if (hasPaths) {
    md += `## Config & Save Paths\n\n`;
    md += `| Type | Path |\n| --- | --- |\n`;
    if (configSave.configPath) md += `| Config | \`${configSave.configPath}\` |\n`;
    if (configSave.configRegistry)
      md += `| Config (Registry) | \`${configSave.configRegistry}\` |\n`;
...
const extDirs = entries
  ...
  .sort()
  .filter(
    (name) => gameArgs.length === 0 || gameArgs.includes(name.replace(/^(?:game|template)-/, "")),
  );
