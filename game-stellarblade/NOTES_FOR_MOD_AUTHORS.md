# Notes for Mod Authors - Stellar Blade

Packaging rules for Stellar Blade mods, so Vortex installs them to the right place.
...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `SB` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `SB\Content\Paks\LogicMods\LogicMods` |
...
Installs to: `SB\Content\Paks\LogicMods\LogicMods`
...
Installs to: `SB\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `SB`.
...
- Installed to `LOCALAPPDATA\SB\Saved\Config\WindowsNoEditor`.

Installs to: `LOCALAPPDATA\SB\Saved\Config\WindowsNoEditor`
