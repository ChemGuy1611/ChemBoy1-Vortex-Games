...
const UE4SS_REQUIREMENTS = [
  {
    archiveFileName: UE4SS_DLFILE_STRING, //a prefix rather than a file name, so the already-downloaded shortcut never fires - a stale local archive must not satisfy a rolling source
    modType: UE4SS_ID,
...
const spec = {
  ...
  discovery: {
    ids: [STEAMAPP_ID, EPICAPP_ID, STEAMAPP_ID_DEMO],
    names: [],
  },
};
...
//* Download UE4SS from the GitHub releases of the Stellar Blade-specific fork, through the
//shared auto-downloader module. check === false forces a re-download even when it is installed.
...
//pak installer's priority (UE5_SORTABLE_ID = 35): UE4SS Combo (25, always registered), LogicMods
//(27, only when logicModsLoadOrder), UE4SS (29, only when ue4ssLoadOrder), Scripts (31, only when
//ue4ssLoadOrder) and DLL (33, only when ue4ssLoadOrder) - this game's own ladder runs the whole
//UE4SS stack above pak, unlike template where only ModKit/UE4SSCombo/LogicMods sit above it and
//UE4SS/Scripts/DLL sit below - so all five have to be checked here, not just the usual three. An
//installer at or below priority 35 never gets a turn once a pak-type file is present, since
//testPak's own check is just "has a pak-type file" with no exclusivity - so lower-priority
//markers (Root and below) can't have "actually" claimed the archive and are not checked here.
//Used by retagFomodPakMod() below to work out, after the fact, what a FOMOD-installed mod would
//have been classified as had FOMOD not intercepted it first. Pass paths relative to the staging
//folder when calling this on a staged tree: an absolute path drags the staging root's own
//segments in, and a staging folder living under the game's root folder name would then satisfy
//that check for every mod.
function beatsPakInstaller(files) {
  ...
  const hasLogicModsExt = files.some(
    (file) => path.extname(file).toLowerCase() === LOGICMODS_EXT,
  );
  const hasDllExt = files.some((file) => path.extname(file).toLowerCase() === DLL_EXT);
  const hasRootFolder = segsLower.includes(ROOT_FOLDER.toLowerCase());
  if (hasScriptsExt && hasLogicModsExt && hasRootFolder) return true; //UE4SS Combo (25)
  if (logicModsLoadOrder && segsLower.includes(LOGICMODS_FOLDER.toLowerCase())) return true; //LogicMods (27)
  if (ue4ssLoadOrder && segsLower.includes(UE4SS_FILE.toLowerCase())) return true; //UE4SS (29)
  if (ue4ssLoadOrder && hasScriptsExt && segsLower.includes(SCRIPTS_FOLDER.toLowerCase()))
    return true; //Scripts (31)
  if (ue4ssLoadOrder && hasDllExt && segsLower.includes(DLL_FOLDER.toLowerCase())) return true; //DLL (33)
  return false;
}
...
function applyGame(context, gameSpec) {
  ...
  context.registerInstaller(DLL_ID, 33, testDll, (files, fileName) =>
    installDll(context.api, files, fileName),
  );
  context.registerInstaller(UE5_SORTABLE_ID, 35, testPak, (files) =>
    installPak(context.api, files),
  ); //Pak installer
  context.registerInstaller(ROOT_ID, 37, testRoot, installRoot);
  context.registerInstaller(CONFIG_ID, 39, testConfig, (files) =>
