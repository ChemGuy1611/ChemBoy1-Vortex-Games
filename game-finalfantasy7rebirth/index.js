...
const GAME_ID = "finalfantasy7rebirth"; //same as Nexus domain
const STEAMAPP_ID = "2909400"; //from steamdb.info
...
const EPIC_CODE_NAME = "End"; //Folder in root
const EXEC = "ff7rebirth.exe"; //This is true ~80% of the time. Change if different
...
const SAVE_EXTS = [".sav"];
const SAVE_COMPAT_VERSIONS = ["steam", "epic"]; //game versions with installable save mods (never Xbox) - no GOG release
...
const UE4SS_PAGE_NO = 267; //set these if there is a customized UE4SS Nexus page
const UE4SS_FILE_NO = 1351;
...
//config and save
const DATA_FOLDER = path.join("My Games", "FINAL FANTASY VII REBIRTH"); //nested Documents folder, not EPIC_CODE_NAME
...
const CONFIGMOD_LOCATION = DOCUMENTS; //this game keeps Config/Save under Documents\My Games, not the usual LocalAppData
...
const SAVE_PATH_DEFAULT = path.join(SAVEMOD_LOCATION, DATA_FOLDER, "Steam"); //store-specific save subfolder here, not the usual Saved/SaveGames
...
const UE5_SORTABLE_ID = `${GAME_ID}-ue5-sortable-modtype`; //kept at its pre-migration value (already game-specific) - do not rename, would re-tag every existing install
const UE5_SORTABLE_NAME = "UE Sortable Pak Mod";
const LEGACY_UE5_SORTABLE_ID = "ue5-sortable-modtype"; //pre-0.x bare shared modtype - this game already carries its own reinstall-to-sort notice for stragglers, kept as-is
...
let SHIPPING_EXE = path.join(BINARIES_PATH, "ff7rebirth_.exe"); //real shipping exe name - doesn't follow the ${SHIPEXE_PROJECTNAME}-Shipping.exe pattern
...
//FF7R Mod Loader (game-specific third-party loader, not a template feature - no equivalent elsewhere)
const MODLOADER_ID = `${GAME_ID}-modloader`;
const MODLOADER_NAME = "FF7R Mod Loader";
const MODLOADER_PATH = path.join(EPIC_CODE_NAME, "Mods");
const MODLOADER_FILE = "FF7RML";
const MODLOADER_PAGE_NO = 1061;
const MODLOADER_FILE_NO = 4918;

const MODLOADERMOD_ID = `${GAME_ID}-modloadermod`;
const MODLOADERMOD_NAME = "FF7RML Mod";
...
//Test for FF7R Mod Loader itself
...
//Test for FF7R Mod Loader mods (.uplugin)
function testModLoaderMod(files, gameId) {
  const isMod = files.some((file) => path.extname(file).toLowerCase() === MODLOADERMOD_EXT);
  let supported = gameId === spec.game.id && isMod;
...
function installModLoaderMod(files) {
  ...
  if (rootPath === ".") {
    //if the .uplugin file was not inside a folder, create the folder and index to the .uplugin file
    MOD_FOLDER = path.basename(modFile, MODLOADERMOD_EXT);
...
async function setup(discovery, api, gameSpec) {
  ...
  MODTYPE_FOLDERS.push(MODLOADERMOD_PATH); //also covers MODLOADER_PATH - same folder
...
function applyGame(context, gameSpec) {
  ...
  context.registerInstaller(MODLOADERMOD_ID, 28, testModLoaderMod, installModLoaderMod);
  ...
  //MODLOADER_ID must be tested before ROOT_ID: ROOT's testRoot matches ROOTSUB_FOLDERS, which
  //includes "Mods" - the same top-level folder name FF7R Mod Loader itself installs to - so a
  //lower priority number here would let ROOT's broader test claim it first.
  context.registerInstaller(MODLOADER_ID, 38, testModLoader, installModLoader);
