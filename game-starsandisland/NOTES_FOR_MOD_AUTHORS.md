# Notes for Mod Authors - Starsand Island

Packaging rules for Starsand Island mods, so Vortex installs them to the right place.
...
| Asset Replacement Mods | a `.assets` file | `StarsandIsland_Data` |
...
- Recognised by a folder named `StarsandIsland_Data` or `StarsandIsland_Data` in the archive.
...
Installs to: `StarsandIsland_Data`
