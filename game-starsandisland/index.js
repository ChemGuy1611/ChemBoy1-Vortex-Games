...
const XBOX_PUB_ID = "9hmq81wk0e54r"; //string after "ID_"
...
//BepInExConfigManager is bundled on this game's own Nexus page alongside its Nexus-hosted BepInEx
//build (page BEPINEX_PAGE_NO above), so it downloads from Nexus too, not the GitHub module.
...
const BEPINEX_PAGE_NO = 9; //Only specify if there is a Nexus page for BepInEx/MelonLoader
...
async function setup(discovery, api, gameSpec) {
  ...
  if (bepinexInstalled && allowBepCfgMan) {
    downloadBepCfgMan(api, gameSpec, true); //notification to download BepInExConfigManager
  } //*/
...
function applyGame(context, gameSpec) {
  ...
  //Nexus-hosted, so the manual download button stays available regardless of allowBepCfgMan
  //(which only gates the auto-install notification) - matches pre-port behavior.
  context.registerAction(
    "mod-icons",
    300,
    "open-ext",
...
//* Function to auto-download BepInExConfigManager from a Nexus Mods page (bundled alongside this
//game's own Nexus-hosted BepInEx build - see BEPCFGMAN_PAGE_NO above)
