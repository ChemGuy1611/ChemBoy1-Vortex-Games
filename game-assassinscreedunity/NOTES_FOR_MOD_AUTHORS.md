# Notes for Mod Authors - Assassin's Creed Unity

Packaging rules for Assassin's Creed Unity mods, so Vortex installs them to the right place.
...
- Recognised by any file named `version.dll` or `ACUFixes`.
...
- Recognised by a folder named `dlc_10` or `dlc_11` in the archive.
