# Notes for Mod Authors - Dishonored

Packaging rules for Dishonored mods, so Vortex installs them to the right place.
...
| Root / Game Folder Mods | a `DishonoredGame` folder | the game folder itself (no subfolder) |
| Cooked Content Mods | a `.upk` file | `DishonoredGame\CookedPCConsole` |
| Movie / Cutscene Replacements | a `.bik` file | `DishonoredGame\Movies` |
| Binaries / Injector Mods | a `Dishonored.exe` file or a `.dll` file | - |
...
- Recognised by a folder named `DishonoredGame`, `Binaries`, `Engine`, `Config`, `CookedPCConsole`, `DLC`, `Localization` or `Movies` in the archive.
...
- Recognised by any file with the `.upk` or `.pck` extensions.

Installs to: `DishonoredGame\CookedPCConsole`
...
Installs to: `DishonoredGame\Movies`
...
- Recognised by any file named `Dishonored.exe`.
