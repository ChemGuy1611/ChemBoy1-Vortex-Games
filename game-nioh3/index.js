...
const tools = [
  ...
  }, //*/
  {
    ...
    requiredFiles: [MOD_MANAGER_EXEC],
    relative: true,
    exclusive: false,
    //shell: true,
    //parameters: [],
  ...
  }, //*/
  {
    ...
    requiredFiles: [RDBEXPLORER_EXEC],
    relative: true,
    exclusive: false,
    //shell: true,
    //parameters: [],
...
function installYumiaMod(files, api) {
  ...
  if (!hasFolder) {
    //if no "package" folder, install normally
  ...
  const packageFolders = files.reduce((accum, file) => {
    if (path.basename(file).toLowerCase() === YUMIA_MOD_FOLDER) {
      ...
      accum[path.basename(file)] = exists ? accum[path.basename(file)].concat(file) : [file];
    }
    return accum;
  }, {});

  let filtered = files.filter(
    (file) => file.includes(YUMIA_MOD_FOLDER) && !file.endsWith(path.sep),
  );
    ...
    return Promise.map(package, (idx) => {
      return api
        .showDialog(
          ...
          {
            text:
              'This mod has several variants for the the "package" folder - please ' +
        ...
        .then((result) => {
          if (result.action === "Cancel")
            ...
            filtered = installAll ? filtered : createFileArray(result.input); //need to tease the data out to a simple array of all files that contain selcted folder(s)
            /*
...
function applyGame(context, gameSpec) {
  ...
  context.registerInstaller(YUMIA_MOD_ID, 35, testYumiaMod, (files) =>
    installYumiaMod(files, context.api),
  ); //.fdata "package"
...
async function resetRdb(api) {
  //on purge
  GAME_PATH = getDiscoveryPath(api);
  const folder = path.join(GAME_PATH, YUMIA_MOD_FOLDER);
  const rootFiles = ["root.rdb", "root.rdx"];
  const originalFiles = ["root.rdb.original", "root.rdx.original"];
  return copyRdbFiles(api, folder, originalFiles, rootFiles);
}

function runMerge(api) {
  //on deploy
  ...
  try {
    ...
    if (TOOL_PATH !== undefined) {
      return api
        .runExecutable(TOOL_PATH, [], { shell: true, detached: true, suggestDeploy: false })
        .catch((err) =>
      ...
      },
      {
        ...
        action: (dismiss) => {
          api.showDialog(
            ...
            {
              text:
                `You must use ModManager to enable most mods after installing with Vortex.\n` +
...
async function setManagerSettings(api) {
  //on deploy
  GAME_PATH = getDiscoveryPath(api);
...
function runModManager(api) {
  ...
  try {
    ...
    if (TOOL_PATH !== undefined) {
      return api
        .runExecutable(TOOL_PATH, [], { detached: true, suggestDeploy: false })
        .catch((err) =>
          api.showErrorNotification(`Failed to run ${TOOL_NAME}`, err, {
            allowReport: ["EPERM", "EACCESS", "ENOENT"].indexOf(err.code) !== -1,
          }),
