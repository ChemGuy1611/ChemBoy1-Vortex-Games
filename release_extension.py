...
Once per run, after every game has been processed:
    ...
    Drop every game that uploaded successfully from resources/lists/games-unreleased.txt.
    That list is generated by categorize_games.py from each index.js EXTENSION_URL, so it
    goes stale as soon as an extension is first published and stays stale until the next
    categorize run; a successful upload proves the Nexus page exists. --dry-run prints the
    entries it would remove instead of rewriting the file.
...
from vortex_utils import (
    REPO_ROOT, run_generate_explained_batch, run_generate_notes_batch,
...
def release(game_id, open_browser, dry_run=False, skip_eslint=False,
    ...
    if dry_run:
        ...
        # Only a game whose EXTENSION_URL parses could be uploaded for real, so this is
        # what decides whether the unreleased-list prune is previewed for it below.
...
def drop_from_unreleased_list(game_ids, dry_run=False):
    """Remove successfully uploaded games from resources/lists/games-unreleased.txt.

    The list is a categorize_games.py product: membership means index.js carries no
    real Nexus URL in EXTENSION_URL. Uploading requires that URL, so a successful
    upload is proof the extension is published and the entry is a leftover from
    before the const was filled in. Pruning it here keeps the list honest between
    categorize runs; a later categorize run reaches the same result independently.
    """
    path = os.path.join(LISTS_DIR, UNRELEASED_LIST)
    current = read_id_list(path)
    hits = sorted(set(game_ids) & set(current))
    if not hits:
        return
    label = ", ".join(hits)
    if dry_run:
        print(f"\n  [DRY RUN] Would remove from {UNRELEASED_LIST}: {label}")
        return
    write_id_list(path, [gid for gid in current if gid not in set(hits)])
    print(f"\n  Removed from {UNRELEASED_LIST}: {label}")
...
def main():
    ...
    try:
        ...
        # Regenerate every generated doc AFTER the release loop, for the games that
        ...
        # Safe to run after zipping: all of them are repo-only and are in ZIP_EXCLUDES.
        if not args.dry_run and saved:
            ...
            print(f"  Generating NOTES_FOR_MOD_AUTHORS files ({label})...")
            ok, err = run_generate_notes_batch(saved)
            if not ok:
        ...
        uploaded = [gid for gid, r in details.items() if r.get("uploaded")]
        if uploaded:
            drop_from_unreleased_list(uploaded)
        elif args.dry_run and upload:
            would_upload = [gid for gid in saved if details.get(gid, {}).get("upload_ready")]
            drop_from_unreleased_list(would_upload, dry_run=True)
