...
const BEP_LOADER_PATCH_ID = `${GAME_ID}-bepinex-loaderpatch`;
const BEP_LOADER_PATCH_NAME = "BepInEx Loader Patch";
const BEP_LOADER_PATCH_PATH = '.';
const BEP_LOADER_PATCH_FILE = 'doorstop_config.ini';
const BEP_LOADER_PATCH_EXCLUDE_FILE = 'winhttp.dll';
const BEP_LOADER_PATCH_PAGE_NO = 1;
const BEP_LOADER_PATCH_FILE_NO = 10;
const BEP_LOADER_PATCH_DOMAIN = GAME_ID;
...
const BEPINEX_ARCHIVE_NAME = 'BepInEx-Unity.IL2CPP-win-x64-6.0.0-be';
...
let BEPCFGMAN_ARCHIVE_NAME = `BepInEx.ConfigurationManager_IL2CPP_v`;
const BEPCFGMAN_URL_MAIN = `https://github.com/BepInEx/BepInEx.ConfigurationManager/releases`;
let BEPCFGMAN_URL_DIRECT = `https://github.com/BepInEx/BepInEx.ConfigurationManager/releases/download/v${BEPCFGMAN_VER}/BepInEx.ConfigurationManager_IL2CPP_v${BEPCFGMAN_VER}.zip`;
if (BEPINEX_BUILD === 'mono') {
  BEPCFGMAN_ARCHIVE_NAME = `BepInEx.ConfigurationManager_BepInEx5_v`;
  BEPCFGMAN_URL_DIRECT = `https://github.com/BepInEx/BepInEx.ConfigurationManager/releases/download/v${BEPCFGMAN_VER}/BepInEx.ConfigurationManager_BepInEx5_v${BEPCFGMAN_VER}.zip`;
}
...
let MODTYPE_FOLDERS = [BEPINEX_PATCHERS_PATH, BEPINEX_PLUGINS_PATH, BEPINEX_CONFIG_PATH, MELON_PLUGINS_PATH, MELON_MODS_PATH, MELON_CONFIG_PATH, MELON_USERLIB_PATH];
if (hasCustomMods) {
...
const spec = {
  ...
  "modTypes": [
    ...
    {
      "id": BEP_LOADER_PATCH_ID,
      "name": BEP_LOADER_PATCH_NAME,
      "priority": "low",
      "targetPath": path.join('{gamePath}', BEP_LOADER_PATCH_PATH)
    },
    {
      "id": MELON_ID,
      "name": MELON_NAME,
...
//Test for BepinEx files
function testBepLoaderPatch(files, gameId) {
  const isExcl = files.some(file => (path.basename(file) === BEP_LOADER_PATCH_EXCLUDE_FILE));
  const isFile = files.some(file => (path.basename(file) === BEP_LOADER_PATCH_FILE));
  let supported = (gameId === spec.game.id) && !isExcl && isFile;
...
//Install BepInEx files
function installBepLoaderPatch(files) {
  const MOD_TYPE = BEP_LOADER_PATCH_ID;
  const modFile = files.find(file => (path.basename(file) === BEP_LOADER_PATCH_FILE));
  const idx = modFile.indexOf(path.basename(modFile));
  const rootPath = path.dirname(modFile);
  const setModTypeInstruction = { type: 'setmodtype', value: MOD_TYPE };
...
async function installPlugin(api, gameSpec, files, workingDir) {
  ...
  if (unknown) { //warn user - installs to default location (root)
    ...
    actions: [
      {
        ...
        action: (dismiss) => {
          api.showDialog('question', MESSAGE, {
            text: `The mod you just installed contains dll files that don't appear to use any know mod loader for this game.\n`
  ...
  if (!loaderChoice) {
    if (recommendedLoader === 'bep') {
      if ((BEPINEX_PAGE_NO !== 0) && allowBepinexNexus) {
        ...
        if (bepinexLoaderPatch) await downloadBepLoaderPatch(api, gameSpec);
      }
    } else {
    ...
    api.sendNotification({
      ...
      actions: [
        ...
        },
        {
          ...
          action: (dismiss) => {
            api.showDialog('question', MESSAGE, {
              text: `${MOD_NAME} is a mod that allows you to configure MelonLoader mods with and in-game GUI.\n`
                  + `Click the button below to download and install ${MOD_NAME}.\n`
...
function applyGame(context, gameSpec) {
  ...
  context.registerInstaller(BEP_LOADER_PATCH_ID, 27, testBepLoaderPatch, installBepLoaderPatch);
  context.registerInstaller(MELON_ID, 28, testMelon, installMelon);
  context.registerInstaller(ROOT_ID, 29, testRoot, installRoot);
  context.registerInstaller(BEPCFGMAN_ID, 30, testBepCfgMan, installBepCfgMan);
  context.registerInstaller(MELONPREFMAN_ID, 31, testMelonPrefMan, installMelonPrefMan);
  context.registerInstaller(ASSEMBLY_ID, 32, testAssembly, installAssembly);
  ...
  context.registerAction('mod-icons', 300, 'open-ext', {}, 'Download BepInExConfigManager', () => {
    downloadBepCfgManManual(context.api, spec, false);
    }, () => {
      const state = context.api.getState();
      const gameId = selectors.activeGameId(state);
      return gameId === GAME_ID;
...
// Test if BepInEx Loader Patch is installed
function isBepLoaderPatchInstalled(api, spec) {
  const state = api.getState();
  const mods = state.persistent.mods[spec.game.id] || {};
  return Object.keys(mods).some(id => mods[id]?.type === BEP_LOADER_PATCH_ID);
}

//* Function to auto-download BepInEx Loader Patch from a Nexus Mods page
async function downloadBepLoaderPatch(api, gameSpec, check = true) {
  let isInstalled = isBepLoaderPatchInstalled(api, gameSpec);
  if (!isInstalled || !check) {
    const MOD_NAME = BEP_LOADER_PATCH_NAME;
    const MOD_TYPE = BEP_LOADER_PATCH_ID;
    const NOTIF_ID = `${MOD_TYPE}-installing`;
    const PAGE_ID = BEP_LOADER_PATCH_PAGE_NO;
    const FILE_ID = BEP_LOADER_PATCH_FILE_NO;  //If using a specific file id because "input" below gives an error
    const GAME_DOMAIN = BEP_LOADER_PATCH_DOMAIN;
    api.sendNotification({ //notification indicating install process
      id: NOTIF_ID,
      message: `Installing ${MOD_NAME}`,
      type: 'activity',
...
//* Download BepInEx BE from page (user browse for download)
async function downloadBepinexManual(api, gameSpec, check = true) {
  let isInstalled = isBepinexInstalled(api, gameSpec);
  const URL = BEPINEX_URL_ERR;
  const MOD_NAME = 'BepInEx BE';
  const MOD_TYPE = BEPINEX_ID;
...
async function downloadBepCfgMan(api, gameSpec, check = true) {
  ...
  if (!isInstalled || !check) {
    ...
    const NOTIF_ID = `${MOD_TYPE}-installing`;
    const GAME_DOMAIN = gameSpec.game.id;
    const URL = BEPCFGMAN_URL_DIRECT;
    const URL_ERR = BEPCFGMAN_URL_MAIN;
    api.sendNotification({ //notification indicating install process
      id: NOTIF_ID,
      message: `Installing ${MOD_NAME}`,
      type: 'activity',
...
//* Download BepInEx CfgMan from GitHub (user browse for download)
async function downloadBepCfgManManual(api, gameSpec, check = true) {
  let isInstalled = isBepCfgManInstalled(api, gameSpec);
  const URL = BEPCFGMAN_URL_MAIN;
  const MOD_NAME = BEPCFGMAN_NAME;
  const MOD_TYPE = BEPCFGMAN_ID;
  const ARCHIVE_NAME = BEPCFGMAN_ARCHIVE_NAME;
  const instructions = api.translate(`Click on Continue below to open the browser. - `
...
async function downloadMelonPrefMan(api, gameSpec, check = true) {
  ...
  if (!isInstalled || !check) {
    ...
    //try {
      ...
      //*Use promise to copy file to Mods folder directly (since it is a naked exe on GitHub and cannot be installed as a mod archive)
      return new Promise((resolve, reject) => {
        api.events.emit('start-download', [URL], {}, undefined,
          async (error, dlid) => { //callback function to check for errors and copy the dll to the Mods folder
            if (error !== null && (error.name !== 'AlreadyDownloaded')) {
              return reject(error);
            }
            try { //find file in Downloads and copy to Mods folder
              api.sendNotification({ //notification of copy process
                id: `${NOTIF_ID}-copy`,
                message: `Copying ${MOD_NAME} to MelonLoader Mods folder`,
                type: 'activity',
                noDismiss: true,
                allowSuppress: false,
              });
              let files = await fs.readdirAsync(DOWNLOAD_FOLDER);
              files = files.filter(file => (path.basename(file).toLowerCase().includes(MELONPREFMAN_STRING)))
              ...
              api.sendNotification({ //notification copy success
                id: `${NOTIF_ID}-success`,
                message: `Successfully copied ${MOD_NAME} executable to Mods folder`,
                type: 'success',
                noDismiss: false,
                allowSuppress: true,
              });
            } catch (err) { //Show the user the download page if the download and copy process fails
              api.showErrorNotification(`Failed to download and copy ${MOD_NAME}`, err, { allowReport: false });
              util.opn(URL_ERR).catch(() => null);
              return reject(err);
            }
            finally {
