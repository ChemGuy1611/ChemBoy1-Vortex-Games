...
| --- | --- | --- |
| Combo Mods (pak + UE4SS script/DLL together) | both a `Content` and a `Binaries` folder | `SystemShock` |
| Blueprint Mods (LogicMods) | a `LogicMods` folder | `SystemShock\Content\Paks\LogicMods` |
| Pak Mods | a `.pak` file | `SystemShock\Content\Paks\~mods` |
| UE4SS Itself | a `dwmapi.dll` file | `SystemShock\Binaries\Win64` |
| UE4SS Script Mods (Lua) | a `.lua` file and a `Scripts` folder | `SystemShock\Binaries\Win64\ue4ss\Mods` |
| UE4SS DLL Mods (C++) | a `.dll` file and a `dlls` folder | `SystemShock\Binaries\Win64\ue4ss\Mods` |
| Root / Game Folder Mods | a top-level folder such as `SystemShock`, `Engine` or `Content` | the game folder itself (no subfolder) |
| Config File Mods | a config file such as `engine.ini` or `game.ini` | - |
| Save Game Files | a `.sav` file | - |
| Fallback Installer | anything unrecognised with no pak file | `SystemShock\Binaries\Win64` |
...
Installs to: `SystemShock\Content\Paks\LogicMods`
...
Installs to: `SystemShock\Content\Paks\~mods`
...
Installs to: `SystemShock\Binaries\Win64`
...
Installs to: `SystemShock\Binaries\Win64\ue4ss\Mods`
...
- Recognised by a top-level folder matching any of: `SystemShock`, `Engine`, `Content`, `Binaries`, `Mods`, `Paks` or `Movies`.
